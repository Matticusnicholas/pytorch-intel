...
// Source: aten/src/ATen/native/cuda/Activation.cpp

#define TORCH_ASSERT_ONLY_METHOD_OPERATORS
#include <ATen/native/opencl/Activation.h>
...
Tensor& glu_backward_opencl_out(const Tensor& grad_output, const Tensor& input,
...
Tensor glu_backward_opencl(const Tensor& grad_output, const Tensor& input, int64_t dim) {
  auto grad_input = at::empty({0}, input.options());
  return glu_backward_opencl_out(grad_output, input, dim, grad_input);
}
...
std::tuple<Tensor&, Tensor&> log_sigmoid_forward_out_opencl(const Tensor& input, Tensor& result, Tensor& buffer) {
...
std::tuple<Tensor, Tensor> log_sigmoid_forward_opencl(const Tensor& input) {
  auto result = at::empty_like(input);
  auto buffer = at::empty({0}, input.options());
  log_sigmoid_forward_out_opencl(input, result, buffer);
  return std::forward_as_tuple(result, buffer);
}

TORCH_IMPL_FUNC(gelu_out_opencl) (
  const Tensor& /*self*/, std::string_view approximate, const Tensor& /*result*/
) {
  GeluSYCLKernelImpl(*this, get_gelutype_enum(approximate));
}

TORCH_IMPL_FUNC(gelu_backward_out_opencl) (
  const Tensor& /*grad*/, const Tensor& /*self*/, std::string_view approximate, const Tensor& /*grad_input*/
) {
  GeluBackwardSYCLKernelImpl(*this, get_gelutype_enum(approximate));
}
