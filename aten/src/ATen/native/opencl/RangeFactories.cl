...
// Source: aten/src/ATen/native/cuda/RangeFactories.cu
...
/* SYCL kernel */ void elementwise_kernel_with_index(index_t N, func_t f, typename function_traits<func_t>::result_type *data) {
  #pragma unroll
  for (int i = 0; i < thread_work_size; i++) {
    index_t idx = block_work_size * item.get_group(0) + num_threads() * i + item.get_local_id(0);
    if (idx < N) {
      data[idx] = f(idx);
    }
  }
...
void gpu_kernel_with_index(at::Tensor &output, func_t f) {
  ...
  if (N <= std::numeric_limits<int>::max()) {
    elementwise_kernel_with_index<int>/* SYCL: kernel launch with nd_range(grid, num_threads(), 0, stream) */(N, f, output.mutable_data_ptr<scalar_t>());
    // SYCL: kernel launch check handled by SYCL runtime;
  } else {
    elementwise_kernel_with_index<int64_t>/* SYCL: kernel launch with nd_range(grid, num_threads(), 0, stream) */(N, f, output.mutable_data_ptr<scalar_t>());
    // SYCL: kernel launch check handled by SYCL runtime;
  }
}
...
Tensor& linspace_cuda_out(const Scalar& start, const Scalar& end, int64_t steps, Tensor& result) {
  ...
  if (steps == 0) {
    ...
    AT_DISPATCH_INTEGRAL_TYPES(r.scalar_type(), "linspace_opencl", [&]() {
      ...
      gpu_kernel_with_index(r, [scalar_start, scalar_end, steps, step, halfway]SYCL_LAMBDA(int64_t ind) -> scalar_t {
        if (ind < halfway) {
    ...
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(kHalf, kBFloat16, r.scalar_type(), "linspace_opencl", [&]() {
      scalar_t scalar_start = start.to<scalar_t>();
      scalar_t scalar_end = end.to<scalar_t>();
...
Tensor& logspace_cuda_out(const Scalar& start, const Scalar& end, int64_t steps, double base, Tensor& result) {
  ...
  if (steps == 0) {
    ...
    AT_DISPATCH_INTEGRAL_TYPES(r.scalar_type(), "logspace_opencl", [&]() {
      ...
      const int64_t halfway = steps / 2;
      gpu_kernel_with_index(r, [scalar_start, scalar_end, scalar_base, steps, step, halfway]SYCL_LAMBDA(int64_t ind) -> scalar_t {
        if (ind < halfway) {
    ...
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(kHalf, kBFloat16, r.scalar_type(), "logspace_opencl", [&]() {
...
Tensor& range_cuda_out(const Scalar& start, const Scalar& end, const Scalar& step, Tensor& result) {
  AT_DISPATCH_ALL_TYPES_AND(at::ScalarType::Half, result.scalar_type(), "range_opencl", [&]() {
    ...
    gpu_kernel_with_index(r, [xstart, xstep]SYCL_LAMBDA(int64_t ind) -> scalar_t {
...
Tensor& arange_cuda_out(const Scalar& start, const Scalar& end, const Scalar& step, Tensor& result) {
  AT_DISPATCH_ALL_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16, result.scalar_type(), "arange_opencl", [&]() {
    using accscalar_t = at::acc_type<scalar_t, /*is_sycl*/true>;
    auto xstart = start.to<accscalar_t>();
    auto xend = end.to<accscalar_t>();
    auto xstep = step.to<accscalar_t>();
