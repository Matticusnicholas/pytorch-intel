...
// Source: aten/src/ATen/native/cuda/DistanceKernel.cu
#define TORCH_ASSERT_ONLY_METHOD_OPERATORS
#include <ATen/core/Tensor.h>
#include <ATen/Dispatch.h>
#include <ATen/sycl/Exceptions.h>
...
struct dists {
  ...
  // Zero norm
  struct zero {
    static inline void inc(scalar_t& agg, const scalar_t diff, const scalar_t /*p*/) {
      if (diff != diff) { // NaN
        agg = diff;
      } else if (diff != 0.0) {
        agg += 1.0;
    ...
    static inline void agg(scalar_t& update, const scalar_t other) { update += other; }
  };
  ...
  struct lt_two {
    static inline scalar_t backward(const scalar_t diff, const scalar_t grad, const scalar_t dist, const scalar_t p) {
  ...
  struct two {
    ...
    static inline scalar_t finish(const scalar_t agg, const scalar_t /*p*/) { return device_sqrt<scalar_t>(agg); }
    static inline void agg(scalar_t& update, const scalar_t other) { update += other; }
  ...
  struct p {
    ...
    static inline scalar_t finish(const scalar_t agg, const scalar_t p) { return std::pow(agg, static_cast<scalar_t>(1) / p); }
    static inline void agg(scalar_t& update, const scalar_t other) { update += other; }
  ...
  // Inf norm
  struct inf {
    static inline void inc(scalar_t& agg, const scalar_t diff, const scalar_t /*p*/) { if (diff > agg) { agg = diff; } }
    static inline scalar_t finish(const scalar_t agg, const scalar_t /*p*/) { return agg; }
    static inline void agg(scalar_t& update, const scalar_t other) { if (other > update) { update = other; } }
...
struct DistReduceOp {
    ...
    inline scalar_t warp_shfl_down(scalar_t data, int offset) const {
        return WARP_SHFL_DOWN(data, offset);
    }
};

template <typename scalar_t, typename F>
/* SYCL kernel */ static void pdist_kernel_cuda_impl(scalar_t * result, const scalar_t * self, const int64_t n, const int64_t m, const scalar_t p,
                                              const double n2, const double n2_squared_minus_1) {
  const int64_t k = item.get_group(0);
  ...
  const scalar_t * a = start + item.get_local_id(0);
  const scalar_t * b = self + j * m + item.get_local_id(0);
  scalar_t agg = 0.0;
  for (; a < end; a += stride, b += stride) {
    F::inc(agg, std::abs(*a - *b), p);
  }

  /* SYCL: use sycl::local_accessor */ scalar_t agg_smem[kCUDANumThreads];
...
template <typename scalar_t, typename F>
/* SYCL kernel */ static void cdist_backward_kernel_cuda_impl(scalar_t * buffer, const scalar_t * grad, const scalar_t * x1, const scalar_t * x2, const scalar_t * dist,
  ...
  const int y = (item.get_group(1) * item.get_group_range(2) + item.get_group(2)) * item.get_local_range(1) + item.get_local_id(1);
  const int init = item.get_group(0) * item.get_local_range(0) + item.get_local_id(0);
  if (y >= count || init >= m) {
    return;
  }
...
template <typename scalar_t, typename F>
/* SYCL kernel */ static void pdist_backward_kernel_cuda_impl(scalar_t * buffer, const scalar_t * grad, const scalar_t * self, const scalar_t * dist, int64_t gs, const int64_t n, const int64_t m, const int64_t combs, const scalar_t p,
                                                       const double n2, const double n2_squared_minus_1) {
  const int64_t k = ((int64_t) item.get_group(0)) * item.get_local_range(0) + item.get_local_id(0);
  const int init = item.get_group(1) * item.get_local_range(1) + item.get_local_id(1);
  const int stride = item.get_local_range(1) * item.get_group_range(1);
...
template <typename scalar_t, typename F>
/* SYCL kernel */ static void cdist_kernel_cuda_impl(scalar_t * result, const scalar_t * x1, const scalar_t * x2,
    const scalar_t p, const int64_t r2, const int64_t m, const int64_t r_size, const int64_t l1_size, const int64_t l2_size) {
  const int64_t l = item.get_group(0) / r_size;
  const int64_t k = item.get_group(0) % r_size;
  ...
  const scalar_t * const end = start + m;
  const scalar_t * a = start + item.get_local_id(0);
  const scalar_t * b = x2 + l * l2_size + j * m + item.get_local_id(0);
  ...
  agg = cuda_utils::BlockReduce(agg, DistReduceOp<scalar_t, F>{}, agg_init, agg_smem);
  if (item.get_local_id(0) == 0) {
    result[item.get_group(0)] = F::finish(agg, p);
  }
}
...
void cdist_kernel_impl(Tensor& result, const Tensor& x1, const Tensor& x2, double p) {
  ...
  const sycl::range<3> grid(result.numel());
  const sycl::range<3> block(kCUDANumThreads);

  AT_DISPATCH_FLOATING_TYPES(x1.scalar_type(), "cdist_opencl", [&] {
    ...
    impl_fptr/* SYCL: submit kernel with nd_range({grid=grid, block=block, smem=0, stream=at::sycl::getCurrentSYCLStream()}) */(result.mutable_data_ptr<scalar_t>(), x1.const_data_ptr<scalar_t>(), x2.const_data_ptr<scalar_t>(), p, r2, m, r_size, l1_size, l2_size);
...
void pdist_forward_kernel_impl(Tensor& result, const Tensor& self, double p) {
  ...
  AT_DISPATCH_FLOATING_TYPES(self.scalar_type(), "pdist_opencl", [&] {
    ...
    impl_fptr/* SYCL: submit kernel with nd_range({grid=grid, block=block, smem=0, stream=at::sycl::getCurrentSYCLStream()}) */(result.mutable_data_ptr<scalar_t>(), self.const_data_ptr<scalar_t>(), n, m, p, n2, n2_squared_minus_1);
    /* SYCL: kernel launch check handled by SYCL runtime */;
  });
}
...
void pdist_backward_kernel_impl(Tensor& result, const Tensor& grad, const Tensor& self, const double p, const Tensor& dist) {
  ...
  const sycl::range<3> grid(grid_x, grid_y);
  ...
  AT_DISPATCH_FLOATING_TYPES(self.scalar_type(), "pdist_cuda_backward", [&] {
    ...
    impl_fptr/* SYCL: submit kernel with nd_range({grid=grid, block=block, smem=0, stream=at::sycl::getCurrentSYCLStream()}) */(buffer.mutable_data_ptr<scalar_t>(), grad.const_data_ptr<scalar_t>(), self.const_data_ptr<scalar_t>(), dist.const_data_ptr<scalar_t>(), grad.stride(0), n, m, dist.numel(), p, n2, n2_squared_minus_1);
    /* SYCL: kernel launch check handled by SYCL runtime */;
  });
...
void cdist_backward_kernel_impl(Tensor& result, const Tensor& grad, const Tensor& x1, const Tensor& x2, const double p, const Tensor& dist) {
  ...
  AT_DISPATCH_FLOATING_TYPES(result.scalar_type(), "cdist_cuda_backward", [&] {
    ...
    impl_fptr/* SYCL: submit kernel with nd_range({grid=grid, block=block, smem=0, stream=at::sycl::getCurrentSYCLStream()}) */(buffer.mutable_data_ptr<scalar_t>(),
