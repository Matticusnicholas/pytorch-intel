...
// Source: aten/src/ATen/native/cuda/ForeachBinaryOpList.cu
#define TORCH_ASSERT_ONLY_METHOD_OPERATORS
#include <ATen/Dispatch.h>
#include <ATen/sycl/CUDAContext.h>
#include <ATen/native/ForeachUtils.h>
#include <ATen/native/sycl/ForeachFunctors.h>
#include <ATen/native/sycl/ForeachMinMaxFunctors.h>
#include <functional>
#include <type_traits>
...
struct Copy {
  inline dst_t operator()(const src_t& x) {
...
struct Copy<dst_t, c10::complex<double>> {
  inline dst_t operator()(const c10::complex<double>& x) {
...
struct Copy<dst_t, c10::complex<float>> {
  inline dst_t operator()(const c10::complex<float>& x) {
    if constexpr (!(std::is_same_v<dst_t, c10::complex<double>> ||
                    std::is_same_v<dst_t, c10::complex<float>>)) {
      return static_cast<dst_t>(x.real());
    } else {
...
#pragma unroll
        for (int ii = 0; ii < kILP; ii++) {
          const auto i = i_start + item.get_local_id(0) + ii * item.get_local_range(0);
          if (i < n && i < chunk_size) {
