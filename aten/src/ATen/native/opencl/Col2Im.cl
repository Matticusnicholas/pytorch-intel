...
// Source: aten/src/ATen/native/cuda/Col2Im.cu
// SYCL TODO: needs_review - uses col2im_batched from im2col.cuh which needs SYCL port.
...
void col2im_out_opencl_template(
    Tensor& output,
    const Tensor& input_,
  ...
  col2im_shape_check(input_, Tensor(), output_height, output_width,
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND3(kHalf, kBFloat16, kBool,
      input.scalar_type(), "col2im_out_opencl", [&] {
    ...
    col2im_batched(
        at::sycl::getCurrentSYCLQueue(),
        input.const_data_ptr<scalar_t>(),
        input_batch_stride, batch_size, n_output_plane,
        output_height, output_width, height_col, width_col,
        kernel_height, kernel_width, pad_height, pad_width,
        stride_height, stride_width, dilation_height, dilation_width,
        output.mutable_data_ptr<scalar_t>(), output_batch_stride);
  });
...
Tensor& col2im_out_opencl(const Tensor& input,
    IntArrayRef output_size, IntArrayRef kernel_size,
    IntArrayRef dilation, IntArrayRef padding, IntArrayRef stride,
    Tensor& output) {
  col2im_out_opencl_template(output, input, output_size, kernel_size, dilation, padding, stride);
  return output;
}

Tensor col2im_opencl(const Tensor& input,
    IntArrayRef output_size, IntArrayRef kernel_size,
    IntArrayRef dilation, IntArrayRef padding, IntArrayRef stride) {
