...
#include <ATen/native/sycl/Resize.h>
...
Tensor& eye_out_opencl(int64_t n, Tensor& result) {
  // the default value of `m` equals to `n`
  return at::native::eye_out_opencl(n, n, result);
}

Tensor& eye_out_opencl(int64_t n, int64_t m, Tensor& result) {
...
Tensor empty_opencl(IntArrayRef size, std::optional<ScalarType> dtype_opt, std::optional<Layout> layout_opt, std::optional<Device> device_opt, std::optional<bool> pin_memory_opt, std::optional<c10::MemoryFormat> memory_format_opt) {
  Tensor result = at::detail::empty_opencl(size, dtype_opt, layout_opt, device_opt, pin_memory_opt, memory_format_opt);
...
Tensor _efficientzerotensor_opencl(IntArrayRef size,
    ...
    if (!device_.has_index()) {
      device_.set_index(at::sycl::current_device());
    }
    auto allocator = at::native::ZeroTensorAllocator(device_);
    auto dtype_ = dtype_or_default(dtype);
    auto zero_ks = at::DispatchKeySet(c10::DispatchKey::OpenCL) | at::DispatchKeySet(c10::DispatchKey::ZeroTensor);
    auto out = at::detail::empty_generic(size, &allocator, zero_ks, dtype_, std::nullopt);
    return out;
}


Tensor empty_strided_opencl(IntArrayRef size, IntArrayRef stride, std::optional<ScalarType> dtype_opt, std::optional<Layout> layout_opt, std::optional<Device> device_opt, std::optional<bool> pin_memory_opt) {
  Tensor result = at::detail::empty_strided_opencl(size, stride, dtype_opt, layout_opt, device_opt, pin_memory_opt);
  // See Note [Enabling Deterministic Operations]
  if (C10_UNLIKELY(at::globalContext().deterministicAlgorithms() && at::globalContext().deterministicFillUninitializedMemory())) {
    fill_empty_deterministic_(result);
  }
...
// implementation, please enable them in test/test_opencl.py and make sure they
// pass on your local server.
Tensor tril_indices_opencl(
  ...
  auto tensor = empty_opencl({2, tril_size}, dtype_opt, layout_opt, device_opt, pin_memory_opt);
  ...
  if (tril_size > 0) {
    ...
    AT_DISPATCH_INDEX_TYPES(tensor.scalar_type(), "tril_indices_opencl", [&] {
      tril_indices_kernel/* SYCL: launch with nd_range(
          dim_grid, dim_block, 0, at::sycl::getCurrentSYCLStream()) */(
...
// implementation, please enable them in test/test_opencl.py and make sure they
// pass on your local server.
Tensor triu_indices_opencl(
    int64_t row, int64_t col, int64_t offset, std::optional<ScalarType> dtype_opt,
    std::optional<Layout> layout_opt, std::optional<Device> device_opt, std::optional<bool> pin_memory_opt) {
  check_args(row, col, layout_opt);

  auto triu_size = row * col - get_tril_size(row, col, offset - 1);
  auto tensor = empty_opencl({2, triu_size}, dtype_opt, layout_opt, device_opt, pin_memory_opt);
  ...
  if (triu_size > 0) {
    ...
    AT_DISPATCH_INDEX_TYPES(tensor.scalar_type(), "triu_indices_opencl", [&] {
      triu_indices_kernel/* SYCL: launch with nd_range(
          dim_grid, dim_block, 0, at::sycl::getCurrentSYCLStream()) */(
        tensor.mutable_data_ptr<index_t>(),
