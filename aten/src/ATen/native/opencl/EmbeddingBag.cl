...
// Source: aten/src/ATen/native/cuda/EmbeddingBag.cu
// SYCL TODO: needs_review - complex file (572 lines), kernel launch patterns need manual verification
...
#include <ATen/native/sycl/EmbeddingBackwardKernel.h>
...
/* SYCL kernel */ void EmbeddingBag_updateOutputKernel_max(
  ...
  int64_t chunksPerBag = ceil_div(featureSize, (int64_t)item.get_local_range(0));
  int64_t numChunks = numBags * chunksPerBag;
  int64_t chunkOffset = item.get_group(0) * item.get_local_range(1) + item.get_local_id(1);
  int64_t chunkStride = item.get_group_range(0) * item.get_local_range(1);
  ...
  for (int64_t chunk = chunkOffset; chunk < numChunks; chunk += chunkStride) {
    ...
    if (featureDim < featureSize) {
      ...
      SYCL_KERNEL_ASSERT(!has_invalid_index && "Invalid input index in EmbeddingBag: index out of range [0, numRows)");
...
// per_sample_weights are contiguous.
template <typename scalar_t, typename index_t>
/* SYCL kernel */ void EmbeddingBag_updateOutputKernel_sum_mean(
    const index_t *input, const index_t *offsets, const scalar_t *weight, scalar_t *output,
    index_t *offset2bag, int64_t numIndices, int64_t numBags,
    int64_t featureSize, int64_t weight_stride0, int64_t weight_stride1,
...
Tensor embedding_bag_backward_cuda_sum_avg(
  ...
  if (scale_grad_by_freq) {
    ...
    AT_DISPATCH_INDEX_TYPES(indices.scalar_type(), "embedding_bag_backward_cuda_sum_avg", [&] () {
      cudaStream_t stream = at::sycl::getCurrentSYCLStream();
  ...
  return embedding_backward_opencl_kernel(grad, orig_indices, sorted_indices,
      count, num_weights, padding_idx, mode == EmbeddingBagMode::MEAN, offset2bag,
      bag_size, per_sample_weights);
}

template <typename scalar_t, typename index_t>
/* SYCL kernel */ void EmbeddingBag_accGradParametersKernel_max(
  ...
  int64_t chunksPerBag = ceil_div(stride, (int64_t)item.get_local_range(0));
  int64_t numChunks = numBags * chunksPerBag;
  int64_t chunkOffset = item.get_group(0) * item.get_local_range(1) + item.get_local_id(1);
  int64_t chunkStride = item.get_group_range(0) * item.get_local_range(1);
...
#if defined(USE_ROCM)
  sycl::range<3> block = sycl::range<3>(64, 4);
#else
  sycl::range<3> block = sycl::range<3>(32, 8);
#endif
  int grid = 1024;
      ...
      grad.scalar_type(), "embedding_bag_backward_cuda_max", [&] {
        AT_DISPATCH_INDEX_TYPES(max_indices.scalar_type(), "embedding_bag_backward_cuda_max", [&] () {
          EmbeddingBag_accGradParametersKernel_max<
              scalar_t, index_t>/* SYCL: submit kernel with nd_range({grid=grid, block=block, smem=0, stream=stream}) */(
...
_embedding_bag_cuda(const Tensor &weight, const Tensor &indices_,
  ...
  checkScalarTypes("embedding_bag_opencl", indices_arg, {kLong, kInt});
  auto offsets_arg = TensorArg(offsets, "offsets", 1);
  checkScalarTypes("embedding_bag_opencl", offsets_arg, {kLong, kInt});
  checkSameType("embedding_bag_opencl", indices_arg, offsets_arg);
  auto weight_arg = TensorArg(weight, "weight", 1);
  checkSameGPU("embedding_bag_opencl", weight_arg, indices_arg);
  checkSameGPU("embedding_bag_opencl", weight_arg, offsets_arg);
...
#endif
  int grid = 1024;
  AT_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16, weight.scalar_type(), "embedding_bag_opencl", [&] {
    AT_DISPATCH_INDEX_TYPES(indices.scalar_type(), "embedding_bag_opencl", [&] () {
      if (mode == EmbeddingBagMode::MAX) {
        EmbeddingBag_updateOutputKernel_max<scalar_t, index_t>/* SYCL: submit kernel with nd_range({grid=grid, block=block, smem=0, stream=stream}) */(
        ...
        EmbeddingBag_updateOutputKernel_sum_mean<scalar_t, index_t>/* SYCL: submit kernel with nd_range({grid=grid, block=block, smem=0, stream=stream}) */(
            indices.const_data_ptr<index_t>(), offsets.const_data_ptr<index_t>(),
            weight.const_data_ptr<scalar_t>(), output.mutable_data_ptr<scalar_t>(),
            offset2bag.mutable_data_ptr<index_t>(), numIndices, numBags, featureSize,
...
Tensor _embedding_bag_dense_backward_cuda(const Tensor &grad_, const Tensor &indices,
  ...
  checkSameGPU("embedding_bag_opencl", grad_arg, indices_arg);
...
template <typename scalar_t, typename index_t>
/* SYCL kernel */ static void _embedding_bag_per_sample_weights_backward_kernel(
    ...
    index_t padding_idx) {
  using accscalar_t = acc_type<scalar_t, true>;
  const int idx = item.get_local_id(0) + item.get_group(0) * item.get_local_range(0);
  const int warp = idx / SYCL_SUB_GROUP_SIZE;
  const int thread_in_warp = idx % SYCL_SUB_GROUP_SIZE;
  const int num_warps = item.get_local_range(0) * item.get_group_range(0) / SYCL_SUB_GROUP_SIZE;
  ...
  for (int sample_idx = warp; sample_idx < num_samples; sample_idx += num_warps) {
    ...
    if (embedding_idx != padding_idx) {
      for (int feature_idx = thread_in_warp; feature_idx < embedding_features;
          feature_idx += SYCL_SUB_GROUP_SIZE) {
        result +=
...
Tensor _embedding_bag_per_sample_weights_backward_cuda(
  ...
  const int warps_per_block = threads_per_block / at::sycl::sub_group_size();

  sycl::range<3> block(threads_per_block);
  sycl::range<3> grid((num_samples + warps_per_block - 1) / warps_per_block);
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
    grad.scalar_type(), "_embedding_bag_per_sample_weights_backward_opencl", [&]() {
      AT_DISPATCH_INDEX_TYPES(indices.scalar_type(), "_embedding_bag_per_sample_weights_backward_opencl", [&]() {
        _embedding_bag_per_sample_weights_backward_kernel<scalar_t, index_t>
          /* SYCL: submit kernel with nd_range({grid=grid, block=block, smem=0, stream=at::sycl::getCurrentSYCLStream()}) */(
