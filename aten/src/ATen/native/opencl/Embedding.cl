...
// Source: aten/src/ATen/native/cuda/Embedding.cu
...
template
  <typename scalar_t,
   ...
   typename index_t>
/* SYCL kernel */ void embedding_backward_feature_kernel
  (const index_t* indices,
   ...
   int padding_idx)
{
  /* SYCL: use sycl::local_accessor for shared memory */ char buf[];
  accscalar_t* smem = (accscalar_t*)buf;
  accscalar_t* my_s = smem + SYCL_SUB_GROUP_SIZE*item.get_local_id(1);
  int* indices_batch = (int*)(buf + sizeof(accscalar_t)*SYCL_SUB_GROUP_SIZE*item.get_local_range(1));

  const int s = (int)stride; // OK to make int, we don't expect 2 billion+ embedding row size

  const int f = item.get_local_id(0) + item.get_group(0)*item.get_local_range(0); // feature_dim

  for(int batch_start = 0; batch_start < n; batch_start += item.get_local_range(0)*item.get_local_range(1))
  {
    // Entire block cooperates to load a batch of 1024 indices to process
    int tid = item.get_local_id(0) + item.get_local_id(1)*item.get_local_range(0);
    if(batch_start + tid < n)
      indices_batch[tid] = (int)indices[batch_start + tid];

    int batch_end = batch_start + item.get_local_range(0)*item.get_local_range(1) < n ?
                    batch_start + item.get_local_range(0)*item.get_local_range(1) : n;

    // Loop over the batch of <= 1024 loaded indices in chunks of item.get_local_range(1) = 32
    for(int chunk_start = batch_start; chunk_start < batch_end; chunk_start += item.get_local_range(1))
    {
      ...
      int n_this_chunk = (batch_end - chunk_start) < item.get_local_range(1) ?
                         (batch_end - chunk_start) : item.get_local_range(1);

      int src_row = chunk_start + item.get_local_id(1);
      ...
      if(src_row < n && f < s && dst_row != padding_idx)
        my_s[item.get_local_id(0)] = static_cast<accscalar_t>(grad[src_row*stride + f]);
      ...
      if(dst_row != padding_idx && src_row < n) // Per-warp exit condition, safe with ballot_sync
      {
        ...
        if(item.get_local_id(0) < n_this_chunk)
          match_found_this_thread = (dst_row == indices_batch[chunk_start - batch_start + item.get_local_id(0)]);
#if defined(USE_ROCM)
        ...
        if(item.get_local_id(1) == first_remaining_peer) // Nominate lowest-indexed warp as the leader
        {
...
#endif
            my_s[item.get_local_id(0)] += smem[item.get_local_id(0) + SYCL_SUB_GROUP_SIZE*first_remaining_peer];
            matchmask ^= (1 << first_remaining_peer);
          }
          if(f < s)
            grad_weight[dst_row*stride + f] += static_cast<scalar_t>(my_s[item.get_local_id(0)]);
        }
      }
    }
  }
...
/* SYCL kernel */ void embedding_backward_kernel(
  ...
  int idx = item.get_group(0) * 4 + item.get_local_id(1);
  ...
  if (idx < numel
    ...
    do {
      const int start_feature = item.get_local_id(0) + item.get_group(1) * item.get_local_range(0) * SZ;
      ...
      for (int ii = 0; ii < SZ; ii++) {
        int feature_dim = start_feature + ii * SYCL_SUB_GROUP_SIZE;
...
/* SYCL kernel */ void renorm_kernel(
  ...
  if (item.get_group(0) >= *num_unique_indices) {
    return;
  }

  // Some casting hacks since dynamic shared memory and templates don't work together:
  /* SYCL: use sycl::local_accessor for shared memory */ unsigned char smem[];
  auto sdata = reinterpret_cast<accscalar_t*>(smem);

  int tid = item.get_local_id(0);
  int base_index = indices[item.get_group(0)] * weights_stride0;

  accscalar_t v = 0;
  for (int i = tid; i < dim; i += item.get_local_range(0)) {
  ...
  v = sycl_utils::BlockReduceSum(v, sdata);
...
Tensor embedding_dense_backward_cuda(const Tensor & grad_, const Tensor & indices_,
  ...
  if (num_indices <= 3072 && !scale_grad_by_freq) {
    ...
    sycl::range<3> grid(ceil_div(stride, (int64_t)warp_size));
    sycl::range<3> block(warp_size, BLOCKDIMY);
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(
      ...
      grad.scalar_type(),
       ...
       {
          using accscalar_t = acc_type<scalar_t, true>;
          AT_DISPATCH_INDEX_TYPES(indices.scalar_type(), "embedding_dense_backward_opencl", [&] () {
  ...
  return embedding_backward_opencl_kernel(grad, orig_indices,
...
Tensor & embedding_renorm_cuda_(Tensor & self, const Tensor & indices,
  ...
  AT_DISPATCH_INDEX_TYPES(indices.scalar_type(), "embedding_renorm_cuda_", [&] () {
    ...
    sycl::range<3> grid = unique_indices.numel();
    sycl::range<3> block = num_threads();
    int dim = self.stride(0);

    AT_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16, self.scalar_type(), "embedding_renorm_cuda_", [&] {
      using accscalar_t = acc_type<scalar_t, true>;
      renorm_kernel/* SYCL: submit kernel with nd_range({grid=grid, block=block, smem=(block.x / warp_size) * sizeof(accscalar_t), stream=stream}) */(
