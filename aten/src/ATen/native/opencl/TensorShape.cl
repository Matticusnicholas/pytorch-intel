...
#include <c10/sycl/SYCLGraphsC10Utils.h>
...
// NOTE [SYCL fast path for split_with_sizes_copy.out]
...
static __inline__ void get_aligned_region(
    char* ptr,
...
static __inline__ void copy_chunk(
    char* dst,
    const char* src,
    int64_t chunk_size,
    int64_t thread_idx,
...
static // SYCL kernel
void split_with_sizes_copy_out_contiguous_no_cast_kernel(
    char** dst_base_addrs,
    char** src_base_addrs,
    ...
    int64_t src_stride,
    int64_t num_chunks) {
  const int64_t split_idx = block_idx_to_split_idx[item.get_group(0)];
  ...
  const int64_t split_thread_idx =
      (item.get_group(0) - blocks_cumsums[split_idx]) * item.get_local_range(0) + item.get_local_id(0);
  ...
  for (int64_t i = item.get_group(1); i < num_chunks; i += gridDim.y) {
    copy_chunk(
...
__inline__ void copy_chunk_with_pad(
    dst_t* dst_ptr,
    src_t* src_ptr,
...
// chunk_cat_opencl adopts a "jagged grid" strategy, inspired by NOTE [SYCL fast
// path for split_with_sizes_copy.out]. In addition, chunk_cat_opencl supports
// padding via copy_chunk_with_pad when src chunk size is less than dst chunk
// size.
template <typename dst_t, typename src_t>
static // SYCL kernel
void chunk_cat_opencl_kernel(
    src_t** src,
    dst_t* dst,
  ...
  const int64_t tensor_idx = block_idx_to_tensor_idx[item.get_group(0)];
  const int64_t tile_idx =
      item.get_group(0) - start_block_idx_per_tensor_chunk[tensor_idx];
...
// See [SYCL kernel for chunk_cat_opencl]
template <typename dst_t, typename src_t>
void _chunk_cat_out_opencl_contiguous(
    TensorList tensors,
    int64_t dim,
    int64_t num_chunks,
    Tensor& out,
  ...
  sycl::range<3> blocks(num_blocks_per_chunk, num_chunks, leading_dim);
  sycl::range<3> threads(detail::BLOCK_SIZE, 1, 1);
  detail::chunk_cat_opencl_kernel/* SYCL: launch with nd_range(
      blocks,
      threads,
      0,
...
void split_with_sizes_copy_out_opencl_contiguous_no_cast(
  ...
  // latency.
  const auto num_sms =
      at::sycl::getCurrentDeviceProperties()->multiProcessorCount;
  const auto max_threads_per_sm =
      at::sycl::getCurrentDeviceProperties()->maxThreadsPerMultiProcessor;
  ...
  sycl::range<3> blocks(blocks_cumsums.back(), num_chunks / chunks_per_block, 1);
  sycl::range<3> threads(detail::BLOCK_SIZE, 1, 1);
  ...
  detail::split_with_sizes_copy_out_contiguous_no_cast_kernel/* SYCL: launch with nd_range(
      blocks,
      threads,
      0,
      at::sycl::getCurrentSYCLStream()) */(
...
void split_with_sizes_copy_out_opencl(
    const Tensor& self,
  ...
  if (contiguous_no_cast) {
    ...
    split_with_sizes_copy_out_opencl_contiguous_no_cast(
...
Tensor _chunk_cat_opencl(TensorList tensors, int64_t dim, int64_t num_chunks) {
  ...
  if (detail::all_contiguous(tensors)) {
    ...
    detail::_chunk_cat_out_opencl_contiguous<char, char>(
...
Tensor& _chunk_cat_out_opencl(
  ...
  TORCH_CHECK(
      ...
      "_chunk_cat_out_opencl: mismatch between input and out tensor devices");
  ...
  if (both_input_output_contiguous &&
    ...
    // _chunk_cat_out_opencl_contiguous should also support other types, thanks to
    // static_cast_with_inter_type. Here, we dispatch to BFloat16 in and float32
    // out since it is the only known use case.
    detail::_chunk_cat_out_opencl_contiguous<float, BFloat16>(
        tensors,
        dim,
        num_chunks,
        out,
