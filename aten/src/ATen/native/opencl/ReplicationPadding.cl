...
// Source: aten/src/ATen/native/cuda/ReplicationPadding.cu
...
/* SYCL kernel */ void replication_pad_forward_kernel1d(
    PackedTensorAccessor64<const scalar_t, 3> input,
    PackedTensorAccessor64<scalar_t, 3> output,
    const int padL,
    const int y_shift,
    const int z_shift) {
  const int64_t outputPointId = item.get_local_id(0) + ((int64_t) item.get_group(0)) * item.get_local_range(0);
  const int64_t plane = item.get_group(1) + y_shift;
  const int64_t batch = item.get_group(2) + z_shift;
...
template <typename scalar_t>
/* SYCL kernel */ void replication_pad_backward_kernel(
  ...
  sycl_atomic_add_no_return(&gradInput[batch][plane][inputPointX], valueToCopy);
}

template <typename scalar_t>
/* SYCL kernel */ void replication_pad_forward_kernel2d(
    ...
    const int y_shift,
    const int z_shift) {
  const int outputPointId = item.get_local_id(0) + item.get_group(0) * item.get_local_range(0);
  const int plane = item.get_group(1) + y_shift;
  const int batch = item.get_group(2) + z_shift;
...
/* SYCL kernel */ void replication_pad_backward_kernel(
  ...
  sycl_atomic_add_no_return(&gradInput[batch][plane][inputPointY][inputPointX], valueToCopy);
}

template <typename scalar_t>
/* SYCL kernel */ void replication_pad_forward_kernel3d(
...
/* SYCL kernel */ void replication_pad_backward_kernel(
  ...
  sycl_atomic_add_no_return(&gradInput[batch][plane][inputPointZ][inputPointY][inputPointX],
      valueToCopy);
}

void replication_pad2d_backward_out_opencl_template(
    Tensor& gradInput,
    ...
    IntArrayRef paddingSize)
{
  ...
  TORCH_CHECK(at::sycl::detail::canUse32BitIndexMath(input),
      "input tensor must fit into 32-bit index math");
  TORCH_CHECK(at::sycl::detail::canUse32BitIndexMath(gradOutput),
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(kHalf, kBFloat16,
      input.scalar_type(), "replication_pad2d_backward_opencl", [&] {
        ...
        for (int64_t block_y = 0; block_y < size1; block_y += 65535) {
          ...
          for (int64_t block_z = 0; block_z < size0; block_z += 65535) {
            ...
            sycl::range<3> gridSize(ceil_div(outputPlaneSize, static_cast<int64_t>(256)), block_y_size, block_z_size);
            sycl::range<3> blockSize(outputPlaneSize > 256 ? 256 : outputPlaneSize);

            replication_pad_backward_kernel /* SYCL: kernel launch with nd_range(gridSize, blockSize, 0, at::sycl::getCurrentSYCLStream()) */(
...
void replication_pad3d_backward_out_opencl_template(
    Tensor& gradInput,
    const Tensor& gradOutput,
    const Tensor& input,
    IntArrayRef paddingSize)
{
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(kHalf, kBFloat16,
    input.scalar_type(), "replication_pad3d_backward_opencl", [&] {
      auto gradInput_ = gradInput;
      auto gradOutput_ = gradOutput;
      if (numInputDims == 4) {
        gradInput_ = gradInput.unsqueeze(0);
...
TORCH_IMPL_FUNC(replication_pad1d_out_opencl) (
  ...
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND2(kHalf, kBFloat16,
    input.scalar_type(), "replication_pad1d_opencl", [&] {
      ...
      for (int64_t block_y = 0; block_y < size1; block_y += 65535) {
        ...
        for (int64_t block_z = 0; block_z < size0; block_z += 65535) {
          ...
          replication_pad_forward_kernel1d /* SYCL: kernel launch with nd_range(gridSize, blockSize, 0,
            at::sycl::getCurrentSYCLStream()) */(devInput, devOutput, padL, block_y, block_z);
          // SYCL: kernel launch check handled by SYCL runtime;
        }
      }
    }
...
TORCH_IMPL_FUNC(replication_pad1d_backward_out_opencl) (
  const Tensor& gradOutput,
  const Tensor& input,
  IntArrayRef paddingSize,
  ...
  globalContext().alertNotDeterministic("replication_pad1d_backward_opencl");
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(kHalf, kBFloat16,
      input.scalar_type(), "replication_pad1d_backward_opencl", [&] {
...
TORCH_IMPL_FUNC(replication_pad2d_out_opencl) (
  const Tensor& input, IntArrayRef paddingSize, const Tensor& output
) {
  TORCH_CHECK(at::sycl::detail::canUse32BitIndexMath(input),
      "input tensor must fit into 32-bit index math");
  ...
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND2(kHalf, kBFloat16,
    input.scalar_type(), "replication_pad2d_opencl", [&] {
      at::Tensor input_ = input;
      at::Tensor output_ = output;
      if (input.dim() == 3) {
        input_ = input.unsqueeze(0);
      ...
      for (int64_t block_y = 0; block_y < size1; block_y += 65535) {
        ...
        for (int64_t block_z = 0; block_z < size0; block_z += 65535) {
          ...
          sycl::range<3> gridSize(ceil_div(outputPlaneSize, static_cast<int64_t>(256)), block_y_size, block_z_size);
          sycl::range<3> blockSize(outputPlaneSize > 256 ? 256 : outputPlaneSize);
          replication_pad_forward_kernel2d /* SYCL: kernel launch with nd_range(gridSize, blockSize, 0, at::sycl::getCurrentSYCLStream()) */(
...
Tensor& replication_pad2d_backward_out_opencl(const Tensor& gradOutput,
    const Tensor& input,
    IntArrayRef paddingSize,
    Tensor& gradInput)
{
  // See Note [Writing Nondeterministic Operations]
  // Nondeterministic because of sycl_atomic_add usage
  globalContext().alertNotDeterministic("replication_pad2d_backward_out_opencl");
  replication_pad2d_backward_out_opencl_template(
      gradInput, gradOutput, input, paddingSize);
  return gradInput;
}

Tensor replication_pad2d_backward_opencl(
    const Tensor& gradOutput,
    const Tensor& input,
    IntArrayRef paddingSize)
{
  // See Note [Writing Nondeterministic Operations]
  // Nondeterministic because of sycl_atomic_add usage
  globalContext().alertNotDeterministic("replication_pad2d_backward_opencl");
...
TORCH_IMPL_FUNC(replication_pad3d_out_opencl) (
  const Tensor& input, IntArrayRef paddingSize, const Tensor& output
) {
  const auto pleft = paddingSize[0];
  ...
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND2(kHalf, kBFloat16,
    input.scalar_type(), "replication_pad3d_opencl", [&] {
      at::Tensor input_ = input;
      at::Tensor output_ = output;
      if (numInputDims == 4) {
        input_ = input.unsqueeze(0);
      ...
      for (int64_t block_y = 0; block_y < size1; block_y += 65535) {
        ...
        for (int64_t block_z = 0; block_z < size0; block_z += 65535) {
          ...
          replication_pad_forward_kernel3d /* SYCL: kernel launch with nd_range(gridSize, blockSize, 0, at::sycl::getCurrentSYCLStream()) */(
...
Tensor& replication_pad3d_backward_out_opencl(const Tensor& gradOutput,
    const Tensor& input,
    IntArrayRef paddingSize,
    Tensor& gradInput)
{
  // See Note [Writing Nondeterministic Operations]
  // Nondeterministic because of sycl_atomic_add usage
  globalContext().alertNotDeterministic("replication_pad3d_backward_out_opencl");
  replication_pad3d_backward_out_opencl_template(
      gradInput, gradOutput, input, paddingSize);
  return gradInput;
}

Tensor replication_pad3d_backward_opencl(
    const Tensor& gradOutput,
    const Tensor& input,
    IntArrayRef paddingSize)
{
  // See Note [Writing Nondeterministic Operations]
  // Nondeterministic because of sycl_atomic_add usage
  globalContext().alertNotDeterministic("replication_pad3d_backward_opencl");
  auto gradInput = at::empty_like(input, LEGACY_CONTIGUOUS_MEMORY_FORMAT);
  replication_pad3d_backward_out_opencl_template(
      gradInput, gradOutput, input, paddingSize);
  return gradInput;
