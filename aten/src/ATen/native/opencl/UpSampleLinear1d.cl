...
// Source: aten/src/ATen/native/cuda/UpSampleLinear1d.cu
...
void upsample_linear1d_out_frame_kernel(
    sycl::nd_item<1> item,
    const int n,
...
template <typename scalar_t, typename accscalar_t>
void upsample_linear1d_out_frame_backward_kernel(
    sycl::nd_item<1> item,
    const int n,
    const accscalar_t rwidth,
    const bool align_corners,
  ...
  if (index < n) {
    ...
    for (int n = 0; n < batchsize; n++) {
      for (int c = 0; c < channels; ++c) {
        ...
        sycl::atomic_ref<scalar_t, sycl::memory_order::relaxed, sycl::memory_scope::device, sycl::access::address_space::global_space> ref_w0(idata[n][c][w1]);
        ref_w0 += static_cast<scalar_t>(w0lambda * d2val);
        sycl::atomic_ref<scalar_t, sycl::memory_order::relaxed, sycl::memory_scope::device, sycl::access::address_space::global_space> ref_w1(idata[n][c][w1 + w1p]);
        ref_w1 += static_cast<scalar_t>(w1lambda * d2val);
      }
    }
  }
}

static void upsample_linear1d_out_opencl_template(
    const Tensor& output,
    const Tensor& input,
      ...
      input.scalar_type(), "upsample_linear1d_out_frame", [&] {
        ...
        stream.submit([&](sycl::handler& cgh) {
          cgh.parallel_for(
              ...
              [=](sycl::nd_item<1> item) {
                upsample_linear1d_out_frame_kernel<scalar_t, accscalar_t>(
                    item, num_kernels, rwidth, align_corners, idata, odata);
              });
        });
      });
}

static void upsample_linear1d_backward_out_opencl_template(
      ...
      grad_output.scalar_type(), "upsample_linear1d_out_frame_backward", [&] {
        ...
        stream.submit([&](sycl::handler& cgh) {
          cgh.parallel_for(
              sycl::nd_range<1>(groups * num_threads, num_threads),
              [=](sycl::nd_item<1> item) {
                upsample_linear1d_out_frame_backward_kernel<scalar_t, accscalar_t>(
                    item, num_kernels, rwidth, align_corners, idata, odata);
              });
        });
      });
...
TORCH_IMPL_FUNC(upsample_linear1d_out_opencl) (
    const Tensor& input,
    IntArrayRef output_size,
    bool align_corners,
    std::optional<double> scales,
  ...
  upsample_linear1d_out_opencl_template(output, input, output_size, align_corners, scales);
}

TORCH_IMPL_FUNC(upsample_linear1d_backward_out_opencl) (
  ...
  globalContext().alertNotDeterministic("upsample_linear1d_backward_out_opencl");
  upsample_linear1d_backward_out_opencl_template(
