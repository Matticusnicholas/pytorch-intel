...
// Source: aten/src/ATen/native/cuda/UpSampleNearest3d.cu
...
void upsample_nearest3d_out_frame_kernel(
    sycl::nd_item<1> item,
  ...
  int64_t dst_idx = static_cast<int64_t>(item.get_group(0)) * item.get_local_range(0) + item.get_local_id(0);
...
void upsample_nearest3d_backward_out_frame_kernel(
    sycl::nd_item<1> item,
...
static void upsample_nearest3d_out_opencl_template(
    const Tensor& output,
    const Tensor& input_,
  ...
  AT_DISPATCH_FLOATING_TYPES_AND3(ScalarType::Half, ScalarType::BFloat16, ScalarType::Byte, input.scalar_type(), "upsample_nearest3d_out_frame", [&] {
        ...
        stream.submit([&](sycl::handler& cgh) {
          cgh.parallel_for(
              ...
              [=](sycl::nd_item<1> item) {
                upsample_nearest3d_out_frame_kernel<scalar_t, nn_compute_source_index_fn>(
                    item, idata, nbatch, channels, input_depth, input_height, input_width,
                    output_depth, output_height, output_width, odata,
...
static void upsample_nearest3d_backward_out_opencl_template(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND3(ScalarType::Half, ScalarType::BFloat16, ScalarType::Byte, grad_output.scalar_type(), "upsample_nearest3d_backward_out_frame", [&] {
        ...
        stream.submit([&](sycl::handler& cgh) {
          cgh.parallel_for(
              sycl::nd_range<1>(gdim * bdim, bdim),
              [=](sycl::nd_item<1> item) {
                upsample_nearest3d_backward_out_frame_kernel<scalar_t, accscalar_t, nn_bw_compute_source_index_fn>(
                    item, odata, nbatch, channels, output_depth, output_height, output_width,
                    input_depth, input_height, input_width, idata,
                    depth_scale, height_scale, width_scale);
              });
        });
      });
...
TORCH_IMPL_FUNC(upsample_nearest3d_out_opencl) (
  ...
  upsample_nearest3d_out_opencl_template<nearest_neighbor_compute_source_index>(
      output, input, output_size, scales_d, scales_h, scales_w);
}

TORCH_IMPL_FUNC(_upsample_nearest_exact3d_out_opencl) (
    const Tensor& input, IntArrayRef output_size,
    std::optional<double> scales_d, std::optional<double> scales_h,
    std::optional<double> scales_w, const Tensor& output) {
  upsample_nearest3d_out_opencl_template<nearest_neighbor_exact_compute_source_index>(
      output, input, output_size, scales_d, scales_h, scales_w);
}

TORCH_IMPL_FUNC(upsample_nearest3d_backward_out_opencl) (
    const Tensor& grad_output, IntArrayRef output_size,
    IntArrayRef input_size, std::optional<double> scales_d,
    std::optional<double> scales_h, std::optional<double> scales_w,
    const Tensor& grad_input) {
  upsample_nearest3d_backward_out_opencl_template<nearest_neighbor_bw_compute_source_index>(
      grad_input, grad_output, output_size, input_size, scales_d, scales_h, scales_w);
}

TORCH_IMPL_FUNC(_upsample_nearest_exact3d_backward_out_opencl) (
    const Tensor& grad_output, IntArrayRef output_size,
    IntArrayRef input_size, std::optional<double> scales_d,
    std::optional<double> scales_h, std::optional<double> scales_w,
    const Tensor& grad_input) {
  upsample_nearest3d_backward_out_opencl_template<nearest_neighbor_exact_bw_compute_source_index>(
      grad_input, grad_output, output_size, input_size, scales_d, scales_h, scales_w);
}

using at::native::upsample::compute_output_size;
using at::native::upsample_opencl::get_scale_value;
