...
// Source: aten/src/ATen/native/cuda/FractionalMaxPool3d.cu
...
/* SYCL kernel */ void fractional_max_pool3d_out_frame(
  ...
  int64_t poolSizeT, int64_t poolSizeH, int64_t poolSizeW) {
    ...
    int64_t ourOutputPoint = item.get_local_id(0) + ((int64_t) item.get_group(0)) * item.get_local_range(0);
    int64_t plane = item.get_group(1);
    int64_t batch = item.get_group(2);
...
template <typename scalar_t>
/* SYCL kernel */ void fractional_max_pool3d_backward_out_frame(
...
void fractional_max_pool3d_backward_out_cuda_template(
  ...
  const Tensor& indices) {
    ...
    sycl::range<3> grid(
      ...
      gradInput_.size(1),
      gradInput_.size(0));
    sycl::range<3> block(outputPlaneSize > 128 ? 128 : outputPlaneSize);
...
Tensor& fractional_max_pool3d_backward_out_cuda(const at::Tensor& gradOutput_,
  ...
  at::Tensor& gradInput) {
    ...
    globalContext().alertNotDeterministic("fractional_max_pool3d_backward_out_opencl");
    fractional_max_pool3d_backward_out_cuda_template(
      gradInput,
      gradOutput_,
      input,
...
Tensor fractional_max_pool3d_backward_cuda(
  ...
  const at::Tensor& indices) {
    // See Note [Writing Nondeterministic Operations]
    // Nondeterministic because of atomicAdd usage
    globalContext().alertNotDeterministic("fractional_max_pool3d_backward_opencl");
