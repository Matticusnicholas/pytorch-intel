...
// Source: aten/src/ATen/native/cuda/DepthwiseConv3d.cu
// SYCL TODO: needs_review - complex file (707 lines), kernel launch patterns need manual verification
...
/* SYCL kernel */ void conv_depthwise3d_opencl_kernel(
    ...
    int dilationT_, int dilationH_, int dilationW_)
{
  ...
  for (int index = item.get_global_id(0); index < num_output; index += item.get_global_range(0)) {
...
conv_depthwise3d_cuda_backward_input_kernel(
  ...
  for (int index = item.get_global_id(0); index < num_input; index += item.get_global_range(0)) {
...
conv_depthwise3d_cuda_backward_weight_kernel(
  ...
  /* SYCL: use sycl::local_accessor for shared memory */ int sdata_raw[];
  ...
  const int warpid = item.get_local_id(0) / SYCL_SUB_GROUP_SIZE;
  ...
  SYCL_KERNEL_ASSERT(__popc(item.get_local_range(0)) == 1);
#pragma unroll
...
#define DWCONV3D_FORWARD_DISPATCH_SPECIALIZATION(kt, kh, kw, dilt, dilh, dilw) \
  if (NODEF_OR_EQUAL_3(kernel_size, (kt), (kh), (kw)) &&                    \
    ...
    conv_depthwise3d_opencl_kernel                                            \
    <scalar_t, accscalar_t, (kt), (kh), (kw), (dilt), (dilh), (dilw)>       \
      /* SYCL: submit kernel with nd_range({grid=grid, block=block, smem=(smem), stream=at::sycl::getCurrentSYCLStream()}) */(          \
...
Tensor conv_depthwise3d_opencl(
    const Tensor& input,
    const Tensor& weight,
  ...
  if (output_mask[1]) {
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(
        ...
        [&] {
          ...
          int64_t warp_size = at::sycl::sub_group_size();
...
REGISTER_OPENCL_DISPATCH(conv_depthwise3d_backward_stub, &conv_depthwise3d_backward_opencl)
