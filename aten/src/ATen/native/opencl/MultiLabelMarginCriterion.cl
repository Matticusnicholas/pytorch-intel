...
// Source: aten/src/ATen/native/cuda/MultiLabelMarginCriterion.cu
...
/* SYCL kernel */ void multilabel_margin_loss_forward_kernel(
    scalar_t* output,
  ...
  /* SYCL: use sycl::local_accessor */ accscalar_t smem[MULTILABELMARGIN_THREADS];
  accscalar_t total_sum = sycl_utils::BlockReduceSum(sum, smem);
  if (item.get_local_id(0) == 0) {
    if (size_average) {
...
// SYCL: launch bounds
C10_LAUNCH_BOUNDS_1(MULTILABELMARGIN_THREADS)
/* SYCL kernel */ void multilabel_margin_loss_backward_kernel(
    scalar_t* grad_input,
...
void multilabel_margin_loss_forward_out_opencl_template(
    const Tensor& input,
    const Tensor& target,
    int64_t reduction,
    Tensor& output,
  ...
  if (input.dim() <= 1) {
    ...
    sycl::range<3> threads(MULTILABELMARGIN_THREADS);
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(
        ...
        [&] {
          ...
          multilabel_margin_loss_forward_kernel<scalar_t, accscalar_t>
              /* SYCL: kernel launch with nd_range(blocks, threads, 0, at::sycl::getCurrentSYCLStream()) */(
    ...
    sycl::range<3> blocks(input.size(0));
    sycl::range<3> threads(MULTILABELMARGIN_THREADS);
...
void multilabel_margin_loss_backward_cuda_out_template(
  ...
  if (grad_input.dim() <= 1) {
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(
        ...
        [&] {
          ...
          multilabel_margin_loss_backward_kernel<scalar_t, accscalar_t>
              /* SYCL: kernel launch with nd_range(blocks, threads, 0, c10::sycl::getCurrentSYCLStream()) */(
    ...
    sycl::range<3> blocks(grad_input.size(0));
    sycl::range<3> threads(MULTILABELMARGIN_THREADS);
...
std::tuple<Tensor&, Tensor&> multilabel_margin_loss_forward_out_opencl(
    const Tensor& self,
    const Tensor& target,
    int64_t reduction,
    Tensor& output,
    Tensor& is_target) {
  multilabel_margin_loss_forward_out_opencl_template(
      self, target, reduction, output, is_target);
  return std::tuple<Tensor&, Tensor&>(output, is_target);
}

std::tuple<Tensor, Tensor> multilabel_margin_loss_forward_opencl(
    const Tensor& self,
    const Tensor& target,
    int64_t reduction) {
...
Tensor multilabel_margin_loss_backward_opencl(
    const Tensor& grad_output,
    const Tensor& self,
    const Tensor& target,
    int64_t reduction,
