...
// Source: aten/src/ATen/native/cuda/NaiveDilatedConvolution.cu
...
/*
  slow_conv_dilated_all_opencl_template
...
void slow_conv_dilated_all_opencl_template(
    Tensor& output,
    const Tensor& input,
    const Tensor& weight,
    const Tensor& bias,
          ...
          if (output.defined()) {
            ...
            if (bias.defined()) {
              /* For gemm argument derivation, see
                 slow_conv_dilated_all_opencl_template in
...
Tensor slow_conv_dilated2d_cuda(
  ...
  slow_conv_dilated_all_opencl_template<2>(
      output_,
      input_,
      weight_,
      bias_,
...
std::tuple<Tensor, Tensor, Tensor> slow_conv_dilated2d_backward_opencl(
    const Tensor& grad_output,
    const Tensor& input,
    const Tensor& weight,
    IntArrayRef kernel_size,
...
Tensor slow_conv_dilated3d_cuda(
  ...
  slow_conv_dilated_all_opencl_template<3>(
      output,
      input_,
      weight_,
      bias_,
...
std::tuple<Tensor, Tensor, Tensor> slow_conv_dilated3d_backward_opencl(
    const Tensor& grad_output,
    const Tensor& input,
    const Tensor& weight,
    IntArrayRef kernel_size,
...
REGISTER_OPENCL_DISPATCH(slow_conv_dilated2d_backward_stub, &slow_conv_dilated2d_backward_opencl)
REGISTER_OPENCL_DISPATCH(slow_conv_dilated3d_backward_stub, &slow_conv_dilated3d_backward_opencl)
