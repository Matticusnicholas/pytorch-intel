...
void bitwise_not_kernel_opencl(TensorIteratorBase& iter) {
  if (iter.dtype() == ScalarType::Bool) {
    sycl_kernel(iter, []SYCL_LAMBDA(bool a) {
      return !a;
    });
  } else {
    AT_DISPATCH_INTEGRAL_TYPES(iter.dtype(), "bitwise_not_opencl", [&]() {
...
void exp_kernel_opencl(TensorIteratorBase& iter) {
  ...
  if (at::isComplexType(common_dtype)) {
      ...
      AT_DISPATCH_COMPLEX_TYPES_AND(kComplexHalf, common_dtype, "exp_opencl", [&]() {
          jitted_gpu_kernel<
              /*name=*/exp_name,
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16, common_dtype, "exp_opencl", [&]() {
      sycl_kernel(iter, []SYCL_LAMBDA(scalar_t a) -> scalar_t {
        return std::exp(a);
      });
    });
...
void expm1_kernel_opencl(TensorIteratorBase& iter) {
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(
      ScalarType::BFloat16, ScalarType::Half,
      iter.common_dtype(), "expm1_opencl",
      [&]() {
        sycl_kernel(iter, []SYCL_LAMBDA(scalar_t a) -> scalar_t {
          return ::expm1(a);
        });
...
void rsqrt_kernel_opencl(TensorIteratorBase& iter) {
  auto common_dtype = iter.common_dtype();
  if (at::isComplexType(common_dtype)) {
    #if AT_USE_JITERATOR()
      ...
      AT_DISPATCH_COMPLEX_TYPES_AND(kComplexHalf, common_dtype, "rsqrt_opencl", [&]() {
          jitted_gpu_kernel<
              /*name=*/rsqrt_name,
              /*return_dtype=*/scalar_t,
              /*common_dtype=*/scalar_t,
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(
      ScalarType::BFloat16, ScalarType::Half,
      iter.common_dtype(), "rsqrt_opencl",
      [&]() {
        sycl_kernel(iter, []SYCL_LAMBDA(scalar_t a) -> scalar_t {
...
void sqrt_kernel_opencl(TensorIteratorBase& iter) {
  auto common_dtype = iter.common_dtype();
  if (at::isComplexType(common_dtype)) {
    #if AT_USE_JITERATOR()
      ...
      AT_DISPATCH_COMPLEX_TYPES_AND(kComplexHalf, common_dtype, "sqrt_opencl", [&]() {
          jitted_gpu_kernel<
              /*name=*/sqrt_name,
              /*return_dtype=*/scalar_t,
              /*common_dtype=*/scalar_t,
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(ScalarType::Half, ScalarType::BFloat16, common_dtype, "sqrt_opencl", [&]() {
      sycl_kernel(iter, []SYCL_LAMBDA(scalar_t a) -> scalar_t {
        return std::sqrt(a);
      });
    });
...
void clamp_kernel_opencl(TensorIteratorBase& iter, const Scalar& min_value, const Scalar& max_value) {
  AT_DISPATCH_ALL_TYPES_AND2(kHalf, kBFloat16, iter.dtype(), "clamp_opencl", [&]() {
    auto lower = min_value.to<scalar_t>();
    auto upper = max_value.to<scalar_t>();
    sycl_kernel(iter, [=]SYCL_LAMBDA(scalar_t v) -> scalar_t {
...
void clamp_min_kernel_opencl(TensorIteratorBase& iter, const Scalar& min_value) {
  AT_DISPATCH_ALL_TYPES_AND2(kHalf, kBFloat16, iter.dtype(), "clamp_min_opencl", [&]() {
    auto lower = min_value.to<scalar_t>();
    sycl_kernel(iter, [=]SYCL_LAMBDA(scalar_t v) -> scalar_t {
      // Propagate nan, which doesn't propagate automatically for ROCm
      if (_isnan(v)) {
...
void clamp_max_kernel_opencl(TensorIteratorBase& iter, const Scalar& max_value) {
  AT_DISPATCH_ALL_TYPES_AND2(kHalf, kBFloat16, iter.dtype(), "clamp_max_opencl", [&]() {
    auto upper = max_value.to<scalar_t>();
    sycl_kernel(iter, [=]SYCL_LAMBDA(scalar_t v) -> scalar_t {
      // Propagate nan, which doesn't propagate automatically for ROCm
      if (_isnan(v)) {
...
void nan_to_num_kernel_opencl(
  ...
  if (isComplexType(iter.dtype())) {
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(kHalf, kBFloat16, iter.dtype(), "nan_to_num_opencl", [&]() {
...
void frexp_kernel_opencl(TensorIteratorBase& iter) {
  AT_DISPATCH_FLOATING_TYPES_AND2(kHalf, kBFloat16,
    ...
    iter.dtype(),
    "frexp_opencl", [&]() {
      sycl_kernel_multiple_outputs(iter, [=] SYCL_LAMBDA (scalar_t a) -> oneapi::dpl::tuple<scalar_t, int32_t> {
        int32_t exponent;
...
REGISTER_DISPATCH(bitwise_not_stub, &bitwise_not_kernel_opencl)
REGISTER_DISPATCH(exp_stub, &exp_kernel_opencl)
REGISTER_DISPATCH(expm1_stub, &expm1_kernel_opencl)
REGISTER_DISPATCH(rsqrt_stub, &rsqrt_kernel_opencl)
REGISTER_DISPATCH(sqrt_stub, &sqrt_kernel_opencl)
REGISTER_DISPATCH(nan_to_num_stub, &nan_to_num_kernel_opencl)
REGISTER_DISPATCH(frexp_stub, &frexp_kernel_opencl)
