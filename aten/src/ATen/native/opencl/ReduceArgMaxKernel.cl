...
// Source: aten/src/ATen/native/cuda/ReduceArgMaxKernel.cu
...
void argmax_kernel_opencl_impl(TensorIterator& iter) {
...
void argmax_kernel_opencl(TensorIterator& iter) {
  ...
  if (iter.dtype(1) == kHalf) {
    argmax_kernel_opencl_impl<at::Half, float>(iter);
  } else if (iter.dtype(1) == kBFloat16) {
    argmax_kernel_opencl_impl<at::BFloat16, float>(iter);
  } else {
    AT_DISPATCH_ALL_TYPES(iter.dtype(1), "argmax_opencl", [&]() {
      argmax_kernel_opencl_impl<scalar_t>(iter);
    });
  }
}

REGISTER_DISPATCH(argmax_stub, &argmax_kernel_opencl)
