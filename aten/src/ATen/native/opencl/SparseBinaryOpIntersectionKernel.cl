...
// SYCL kernel
void apply_kernel(int n, loop_t loop) {
...
void launch_kernel(int64_t n, const loop_t& loop) {
  ...
  apply_kernel<nt, vt, loop_t>/* SYCL: launch with nd_range(grid, block, 0, stream) */(n, loop);
...
void mul_sparse_sparse_out_opencl_kernel(
    Tensor& result,
    const Tensor& x,
    const Tensor& y) {
...
void sparse_mask_intersection_out_opencl_kernel(
    Tensor& result,
    const Tensor& x,
    const Tensor& y,
...
void sparse_mask_projection_out_opencl_kernel(
    Tensor& result,
    const Tensor& x,
    const Tensor& y,
...
REGISTER_CUDA_DISPATCH(mul_sparse_sparse_out_stub, &mul_sparse_sparse_out_opencl_kernel)
REGISTER_CUDA_DISPATCH(sparse_mask_intersection_out_stub, &sparse_mask_intersection_out_opencl_kernel)
REGISTER_CUDA_DISPATCH(sparse_mask_projection_out_stub, &sparse_mask_projection_out_opencl_kernel)
