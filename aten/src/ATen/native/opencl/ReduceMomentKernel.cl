...
// Source: aten/src/ATen/native/cuda/ReduceMomentKernel.cu
...
void std_var_kernel_impl(TensorIterator& iter, double correction, bool take_sqrt) {
  ...
  sycl_reduce_kernel<scalar_t, out_t, 2>(iter, ops, typename ops_t::acc_t{});
}

static void std_var_kernel_opencl(TensorIterator& iter, double correction, bool take_sqrt) {
...
void mean_kernel_impl(TensorIterator& iter) {
  ...
  if constexpr (is_16_bits) {
    sycl_reduce_kernel<scalar_t, out_t, /*vt0=*/4, /*input_vec_size=*/8>(iter, MeanOps<scalar_t, acc_t, factor_t, out_t> {factor});
  } else {
    sycl_reduce_kernel<scalar_t, out_t>(iter, MeanOps<scalar_t, acc_t, factor_t, out_t> {factor});
  }
}

static void mean_kernel_opencl(TensorIterator& iter) {
  if (iter.dtype() == kHalf) {
    ...
    AT_DISPATCH_ALL_TYPES_AND_COMPLEX(iter.dtype(), "mean_opencl", [&]() {
      mean_kernel_impl<scalar_t>(iter);
    });
  }
}

REGISTER_DISPATCH(std_var_stub, &std_var_kernel_opencl)
REGISTER_DISPATCH(mean_stub, &mean_kernel_opencl)
