...
struct HermitianSymmetryOffsetCalculator {
  ...
  dim_type dims;
  at::sycl::detail::IntDivider<index_t> sizes_[MAX_DIMS];
  ...
  HermitianSymmetryOffsetCalculator(
    ...
    using at::sycl::detail::IntDivider;
...
void _fft_fill_with_conjugate_symmetry_opencl_(
  ...
  AT_DISPATCH_COMPLEX_TYPES_AND(kComplexHalf, dtype, "_fft_fill_with_conjugate_symmetry", [&] {
      ...
      _fft_conjugate_copy_kernel/* SYCL: launch with nd_range(
        GET_BLOCKS(numel), CUDA_NUM_THREADS, 0, at::sycl::getCurrentSYCLStream()) */(
            numel,
...
REGISTER_DISPATCH(fft_fill_with_conjugate_symmetry_stub, &_fft_fill_with_conjugate_symmetry_opencl_)
