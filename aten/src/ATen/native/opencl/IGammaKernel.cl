...
// Source: aten/src/ATen/native/cuda/IGammaKernel.cu
// SYCL TODO: needs_review - complex file (555 lines), kernel launch patterns need manual verification
...
template <typename scalar_t>
[[gnu::noinline]] scalar_t calc_igammac(scalar_t a, scalar_t x) {
...
// NOTE: this [[gnu::noinline]] is important -- otherwise, observed compile times significantly
// increase.  The same kernel seems to get recompiled multiple times via gpu_kernel_with_scalars,
// multiple dtypes, etc.
template <typename scalar_t>
[[gnu::noinline]] scalar_t calc_igamma(scalar_t a, scalar_t x) {
...
void igamma_kernel_opencl(TensorIteratorBase& iter) {
  AT_DISPATCH_FLOATING_TYPES(iter.common_dtype(), "igamma_opencl", [&]() {
    sycl_kernel(iter, CalcIgamma<scalar_t>(false));
  });
}

void igammac_kernel_opencl(TensorIteratorBase& iter) {
  AT_DISPATCH_FLOATING_TYPES(iter.common_dtype(), "igammac_opencl", [&]() {
    sycl_kernel(iter, CalcIgamma<scalar_t>(true));
  });
}

REGISTER_DISPATCH(igamma_stub, &igamma_kernel_opencl)
REGISTER_DISPATCH(igammac_stub, &igammac_kernel_opencl)
