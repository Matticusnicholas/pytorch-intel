...
// Source: aten/src/ATen/native/cuda/DilatedMaxPool2d.cu
// SYCL TODO: needs_review - complex file (797 lines), kernel launch patterns need manual verification
...
static inline index_t p_start(index_t size, int pad, int kernel, int dilation, int stride) {
...
template <typename index_t>
static inline index_t p_end(index_t size, int pad, index_t pooled_size, int stride) {
...
/* SYCL kernel */ void max_pool_forward_nchw(
    ...
    scalar_t* top_data,
    int64_t* top_mask) {
  for (index_t index = item.get_global_id(0); index < nthreads; index += item.get_global_range(0)) {
...
template <typename scalar_t, typename index_t>
/* SYCL: reqd_work_group_size(CUDA_MAX_THREADS) */
/* SYCL kernel */ void max_pool_forward_nhwc(
    const scalar_t* bottom_data,
    const int nbatch,
  ...
  /* SYCL: use sycl::local_accessor for shared memory */ unsigned char smem_raw[];
  index_t *out_mask_cached = reinterpret_cast<index_t*>(smem_raw);
  scalar_t *out_cached = reinterpret_cast<scalar_t*>(
      out_mask_cached + kernel_size_C*item.get_local_range(0)*item.get_local_range(1)*item.get_local_range(2));

  // flattening cta for pre-computation & smem initialization;
  int thread_id = item.get_local_id(0) + item.get_local_range(0) * (item.get_local_id(1) + item.get_local_range(1) * item.get_local_id(2));
  int block_size = item.get_local_range(0) * item.get_local_range(1) * item.get_local_range(2);

  // use shared memory to store temporary output value. This is simply to
  // reduce register usage.
  for (int i = thread_id; i < kernel_size_C*item.get_local_range(0)*item.get_local_range(1)*item.get_local_range(2); i+= block_size) {
  ...
  int batch_id = item.get_group(0) % nbatch;
  int channel_id = item.get_group(0) / nbatch;
  int channel_offset = item.get_local_id(0) + channel_id * item.get_local_range(0);
  ...
  out_cached += (item.get_local_id(2) * item.get_local_range(1) + item.get_local_id(1)) * kernel_size_C*item.get_local_range(0);
  out_mask_cached  += (item.get_local_id(2) * item.get_local_range(1) + item.get_local_id(1)) * kernel_size_C*item.get_local_range(0);

  int oH = (static_cast<int>(pooled_height) + item.get_group_range(2) - 1) / item.get_group_range(2);
  int oW = (static_cast<int>(pooled_width)  + item.get_group_range(1) - 1) / item.get_group_range(1);
  int ostartH = item.get_local_id(2) + item.get_group(2)*oH;
  int oendH = ::min(ostartH+oH, static_cast<int>(pooled_height));
  int ostartW = item.get_local_id(1) + item.get_group(1)*oW;
  int oendW = ::min(ostartW+oW, static_cast<int>(pooled_width));

  for (int oh = ostartH; oh < oendH; oh+=item.get_local_range(2)) {
    ...
    for (int ow = ostartW; ow < oendW; ow+=item.get_local_range(1)) {
...
#define MAXc 1
      ...
      if (kernel_h<=MAXh &&
          kernel_w<=MAXw &&
          channels<=MAXc*(item.get_local_range(0)*kernel_stride_C)) {
        ...
        for (int ih = 0; ih < MAXh; ih++) {
          ...
          for (int iw = 0; iw < MAXw; iw++) {
            ...
            for(int c = 0; c < MAXc; c++) {
              int c_ = c*item.get_local_range(0)*kernel_stride_C+channel_offset;
      ...
      for (index_t ih = hstart; ih < hend; ih += dilation_h) {
        for (index_t iw = wstart; iw < wend; iw += dilation_w) {
          ...
          for (index_t c = channel_offset; c < channels; c += static_cast<index_t>(item.get_local_range(0)) * kernel_stride_C) {
...
#endif
/* SYCL kernel */ void max_pool_backward_nchw(
  ...
  CUDA_KERNEL_LOOP_TYPE(index, height*width, index_t) {
    ...
    for (index_t n = item.get_group(1); n < num; n += item.get_group_range(1)) {
      for (index_t c = item.get_group(2); c < channels; c += item.get_group_range(2)) {
...
/* SYCL: reqd_work_group_size(CUDA_MAX_THREADS) */
/* SYCL kernel */ void max_pool_backward_nhwc(const scalar_t* top_diff,
                                    ...
                                    const int kernel_stride_C, const int kernel_size_C,
                                    scalar_t* bottom_diff) {
  /* SYCL: use sycl::local_accessor for shared memory */ int smem[];
  ...
  out_cached = &out_cached[(item.get_local_id(2) * item.get_local_range(1) + item.get_local_id(1)) * kernel_size_C*item.get_local_range(0)];
  ...
  int iH = (height + item.get_group_range(2)-1) / item.get_group_range(2);
  int iW = (width + item.get_group_range(1)-1) / item.get_group_range(1);
  int istartH = item.get_local_id(2) + item.get_group(2)*iH;
  int iendH = ::min(static_cast<int64_t>(istartH)+iH, height);
  int istartW = item.get_local_id(1) + item.get_group(1)*iW;
  int iendW = ::min(static_cast<int64_t>(istartW)+iW, width);

  for (int ih = istartH; ih < iendH; ih+=item.get_local_range(2)) {
    int phstart = p_start(ih, pad_h, kernel_h, dilation_h, stride_h);
    int phend = p_end(ih, pad_h, pooled_height, stride_h);
    for (int iw = istartW; iw < iendW; iw+=item.get_local_range(1)) {
...
#define _MAXw 2
        if (phend-phstart<=_MAXh && pwend-pwstart<=_MAXw) {
          ...
          int cached_index = item.get_local_id(0);
#pragma unroll
          for (int c = channel_offset; c < channels; c += item.get_local_range(0)*kernel_stride_C) {
#pragma unroll
      ...
      switch (memory_format) {
        case MemoryFormat::ChannelsLast: {
          ...
          int block_x = std::min<int>(
              maxThreadsDim[0], std::min<int>(lastPow2(nInputPlane), at::sycl::sub_group_size()));
          ...
          const sycl::range<3> block(block_x, block_y, block_z);
          ...
          if (use_int32) {
            ...
            max_pool_forward_nhwc<scalar_t, int32_t>
              /* SYCL: submit kernel with nd_range({grid=grid, block=block, smem=shmem_size, stream=at::sycl::getCurrentSYCLStream()}) */(
        ...
        case MemoryFormat::Contiguous: {
          ...
          if (use_int32) {
            max_pool_forward_nchw<scalar_t, int32_t>
                /* SYCL: submit kernel with nd_range({grid=blocks, block=threads, smem=0, stream=stream}) */(
    ...
    [&] {
      ...
      switch (memory_format) {
        ...
        case MemoryFormat::Contiguous: {
          ...
          sycl::range<3> grid(blocks_x, static_cast<unsigned>(std::min<int64_t>(nbatch, maxGridY)), static_cast<unsigned>(std::min<int64_t>(nInputPlane, maxGridZ)));
          bool use_int32 = can_use_int32_nchw(
              nbatch, nInputPlane, inputHeight, inputWidth, outputHeight, outputWidth);
          auto stream = at::sycl::getCurrentSYCLStream();
          if (use_int32) {
            max_pool_backward_nchw<scalar_t, accscalar_t, int32_t>
                /* SYCL: submit kernel with nd_range({grid=grid, block=threads, smem=0, stream=stream}) */(
                    gradOutput_data,
                    indices_data,
