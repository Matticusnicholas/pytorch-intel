...
#include <ATen/native/sycl/TensorTopK.h>
...
// SYCL kernel
void gatherTopK(at::sycl::detail::TensorInfo<const T, IndexType> input,
                           ...
                           at::sycl::detail::TensorInfo<T, IndexType> topK,
                           IndexType topKWithinSliceStride,

                           at::sycl::detail::TensorInfo<int64_t, IndexType> indices,
  ...
  // SYCL: use sycl::local_accessor IndexType smem[32];
  ...
  IndexType sliceStartIndex =
    at::sycl::detail::IndexToOffset<const T, IndexType, Dim>::get(slice, input);
  IndexType topKSliceStartIndex =
    at::sycl::detail::IndexToOffset<T, IndexType, Dim>::get(slice, topK);
  IndexType indicesSliceStartIndex =
    at::sycl::detail::IndexToOffset<int64_t, IndexType, Dim>::get(slice, indices);
  ...
  IndexType numIterations = round_up(inputSliceSize, (IndexType) item.get_local_range(0));
  IndexType writeIndexStart = 0;

  for (IndexType i = item.get_local_id(0); i < numIterations; i += item.get_local_range(0)) {
    ...
    IndexType index;
    IndexType carry;
    at::sycl::exclusiveBinaryPrefixScan<IndexType, true>(
        smem, hasTopK, &index, &carry, AddOp<IndexType>());

    if (hasTopK) {
      IndexType writeIndex = writeIndexStart + index;
      SYCL_KERNEL_ASSERT(writeIndex < outputSliceSize);
  ...
  SYCL_KERNEL_ASSERT(outputSliceSize >= writeIndexStart);
...
inline void reserveWarpSpace(bool hasTopK,
  ...
  if (warp_count > 0 && lane_id == 0) {
    start_index = sycl::atomic_ref_add /* sycl::atomic_ref */(&writeIndexStart, (IndexType)warp_count);
  }
...
inline void writeResult(T* topKSliceStart,
                                            ...
                                            T v,
                                            IndexType i){
  SYCL_KERNEL_ASSERT(writeIndex < outputSliceSize); // assert that the write index is within the output slice size.
...
void gatherTopK(at::sycl::detail::TensorInfo<const T, IndexType> input,
  ...
  // SYCL: use sycl::local_accessor IndexType smem[256];
  // SYCL: use sycl::local_accessor IndexType writeIndexStart; // index to track where to write results. This is shared by all threads in the block. Increases atomically.
  ...
  T v = (item.get_local_id(0) < inputSliceSize) ? doLdg(&inputSliceStart[item.get_local_id(0) * inputWithinSliceStride]) : static_cast<T>(0);
  for (IndexType i = item.get_local_id(0); i < numIterations; i += item.get_local_range(0)) {
    T v_next = (i + item.get_local_range(0) < inputSliceSize) ? doLdg(&inputSliceStart[(i + item.get_local_range(0)) * inputWithinSliceStride]) : static_cast<T>(0);
...
void launch(
    at::sycl::detail::TensorInfo<const T, IndexType> input,
    IndexType inputSliceSize,
    IndexType outputSliceSize, // aka `k`
    bool largest,
    ...
    sycl::range<3> block(std::min(at::ceil_div((int64_t)inputSliceSize, (int64_t)warp_size) * (int64_t)warp_size, (int64_t)1024));
    gatherTopK<T, IndexType, Dim, /* WithKthValues= */false>/* SYCL: launch with nd_range(grid, block, 0, c10::sycl::getCurrentSYCLStream()) */(
        input,
        inputSliceSize,
        outputSliceSize,
        largest,
...
// SYCL kernel
void warpMergeSortTopK(
    at::sycl::detail::TensorInfo<const scalar_t, IndexType> input,
    IndexType inputSliceSize,
    IndexType k,
    IndexType numInputSlices,
    IndexType inputWithinSliceStride,
    at::sycl::detail::TensorInfo<scalar_t, IndexType> topK,
    IndexType topKWithinSliceStride,
    at::sycl::detail::TensorInfo<int64_t, IndexType> indices,
    IndexType indicesWithinSliceStride) {
  ...
  const IndexType linearIndex = blockIndex * item.get_local_range(1) + item.get_local_id(1);
  ...
  const IndexType inputStartOffset =
      at::sycl::detail::IndexToOffset<const scalar_t, IndexType, KeyDims>::get(linearIndex, input);
  const IndexType topKStartOffset =
      at::sycl::detail::IndexToOffset<scalar_t, IndexType, ValueDims>::get(linearIndex, topK);
  const IndexType indicesStartOffset =
      at::sycl::detail::IndexToOffset<int64_t, IndexType, ValueDims>::get(linearIndex, indices);
  ...
  namespace cub = SYCL_ONEDPL(at_sycl_detail::cub);

  SYCL_KERNEL_ASSERT(item.get_local_range(0) == C10_WARP_SIZE);
  SYCL_KERNEL_ASSERT(item.get_local_range(1) <= max_block_dim_y);
  ...
  // SYCL: use sycl::local_accessor union {
  ...
  auto& warp_storage = tmp_storage[item.get_local_id(1)];
  ...
  for (int i = 0; i < items_per_thread; ++i) {
    local_indices[i] = item.get_local_id(0) * items_per_thread + i;
  }
...
void launch(
  ...
  const auto stream = c10::sycl::getCurrentSYCLStream();
  ...
  const int block_x = at::sycl::warp_size();
  sycl::range<3> block(block_x, max_block_dim_y);
  ...
  #define LAUNCH_KERNEL(SORT_SIZE, IS_DESCENDING) \
    warpMergeSortTopK<Dim, Dim, SORT_SIZE, max_block_dim_y, scalar_t, IndexType, IS_DESCENDING> \
      /* SYCL: launch with nd_range(grid, block, 0, stream) */( \
...
template <typename T, typename IndexType>
// SYCL kernel
void fill(T* x, T value, IndexType size) {
  IndexType idx = item.get_group(0) * item.get_local_range(0) + item.get_local_id(0);
  for (IndexType i = idx; i < size; i += item.get_group_range(0) * item.get_local_range(0)) {
    x[i] = value;
  }
}
...
void computeBlockDigitCounts(
  ...
  IndexType slice_start_index = at::sycl::detail::IndexToOffset<const T, IndexType, Dim>::get(slice_idx, input);
  ...
  // SYCL: use sycl::local_accessor TempStorage temp_storage;
  ...
  for (int i = 0; i < items_per_thread; ++i) {
    ...
    if (idx < slice_size) {
      ...
      Bitwise digit = at::sycl::Bitfield<Bitwise>::getBitfield(val, current_bit, RADIX_BITS);
      if (has_val) {
        sycl::atomic_ref_add /* sycl::atomic_ref */(&temp_storage.digit_counters[digit], 1);
      }
    }
  }
...
// SYCL kernel
void computeDigitCumSum(
  short* counts,
  uint32_t* digit_cum_sum,
  uint32_t blocks_per_slice) {
  int tidx = item.get_local_id(0) + item.get_group(0) * item.get_local_range(0);
  int digit_idx = item.get_local_id(0);
  uint32_t slice_idx = item.get_group(0);

  typedef cub::BlockScan<uint32_t, RADIX_DIGITS> BlockScan;
  // SYCL: use sycl::local_accessor typename BlockScan::TempStorage scan_storage;
  // accumulates counters from multiple blocks
  uint32_t digit_count = 0;
  if (item.get_local_id(0) < RADIX_DIGITS) {
...
// SYCL kernel
void computeBlockwiseWithinKCounts(
  ...
  // elements, we will use both item.get_group(0) and item.get_group(1), and maybe blockIdx.z
  ...
  // SYCL: use sycl::local_accessor Bitwise desired;
  ...
  if (tidx < RADIX_DIGITS) {
    ...
    if (digit_count_cumsum_left < k_to_find && k_to_find <= digit_count_cumsum) {
      ...
      desired = at::sycl::Bitfield<Bitwise>::setBitfield(desired, tidx, current_bit, RADIX_BITS);
  ...
  Bitwise desired_digit = at::sycl::Bitfield<Bitwise>::getBitfield(desired, current_bit, RADIX_BITS);
  ...
  // SYCL: use sycl::local_accessor uint32_t warp_counts[num_warps];
...
// SYCL kernel
void computeBlockwiseKthCounts(
  Bitwise* desires,            // size: num_slices
  short* counts,               // size: num_slices * blocks_per_slice * radix_digits
  ...
  SYCL_KERNEL_LOOP_TYPE(idx, num_blocks, uint32_t) {
    uint32_t slice_idx = idx / blocks_per_slice;
    Bitwise desired = doLdg(desires + slice_idx);
    Bitwise desired_digit = at::sycl::Bitfield<Bitwise>::getBitfield(desired, 0, RADIX_BITS);
...
void computeBlockwiseWithinKCounts(
  ...
  typedef SYCL_ONEDPL(at_sycl_detail::cub)::BlockScan<uint32_t, RADIX_DIGITS> BlockScan;
  // SYCL: use sycl::local_accessor typename BlockScan::TempStorage scan_storage;

  // Build per-slice digit totals in shared memory and compute inclusive cumsum
  // SYCL: use sycl::local_accessor uint32_t digit_totals[RADIX_DIGITS];
  if (tidx < RADIX_DIGITS) {
    uint32_t sum = 0;
  ...
  SYCL_KERNEL_ASSERT(RADIX_DIGITS < C10_WARP_SIZE * C10_WARP_SIZE);
  if (warp != 0) {
    return;
  }
  count = 0;
  ...
  if (tidx == 0 && current_bit == 0) {
    Bitwise desired_digit0 = at::sycl::Bitfield<Bitwise>::getBitfield(desired, 0, RADIX_BITS);
...
void gatherTopK(at::sycl::detail::TensorInfo<const T, IndexType> input,
  ...
  uint32_t tidx = item.get_local_id(0);
  uint32_t block_idx = getLinearBlockId<uint32_t>();
  ...
  // Find the start offset for our slice
  IndexType sliceStartIndex =
    at::sycl::detail::IndexToOffset<const T, IndexType, Dim>::get(slice_idx, input);
  IndexType topKSliceStartIndex =
    at::sycl::detail::IndexToOffset<T, IndexType, Dim>::get(slice_idx, topK);
  IndexType indicesSliceStartIndex =
    at::sycl::detail::IndexToOffset<int64_t, IndexType, Dim>::get(slice_idx, indices);
  ...
  // SYCL: use sycl::local_accessor uint32_t slice_prefix_within;
  // SYCL: use sycl::local_accessor uint32_t slice_prefix_kth;
  // SYCL: use sycl::local_accessor uint32_t slice_total_within;
  if (item.get_local_id(0) == 0) {
  ...
  // SYCL: use sycl::local_accessor typename BlockScan::TempStorage temp_storage;
  for (int i = 0; i < items_per_thread; ++i) {
    // Find the start offset for this slice
    IndexType idx = blk_idx_in_slice * items_per_block + i * BLOCK_THREADS + tidx;
    T val;
...
int get_items_per_thread(uint64_t num_slices, uint64_t slice_size) {
  ...
  cudaDeviceProp* prop = at::sycl::getCurrentDeviceProperties();
...
public:
  ...
  inline uint32_t operator()(uint32_t blk) const {
...
void launch(
    ...
    at::sycl::detail::TensorInfo<int64_t, IndexType> indices,
    IndexType indicesWithinSliceStride) {
  auto stream = c10::sycl::getCurrentSYCLStream();
  ...
  // temporary storage
  auto& allocator = *c10::sycl::CUDACachingAllocator::get();
  ...
  fill<uint32_t>/* SYCL: launch with nd_range(std::min(((int64_t)numInputSlices + 511) / 512, (int64_t)1073741824), 512, 0, stream) */(
  ...
  sycl::range<3> block(BLOCK_THREADS);
  ...
  for (int current_bit = sizeof(T) * 8 - RADIX_BITS; current_bit >= 0; current_bit -= RADIX_BITS) {
    computeBlockDigitCounts<T, IndexType, Bitwise, Dim>/* SYCL: launch with nd_range(grid, block, 0, stream) */(
        input,
        inputSliceSize,
    ...
    computeDigitCumSum/* SYCL: launch with nd_range(numInputSlices, RADIX_DIGITS, 0, stream) */(counts, digit_cum_sum, blocks_per_slice);
    // SYCL: kernel launch check handled by SYCL runtime;

    // CUDA path: Call computeBlockwiseWithinKCounts with digit_cum_sum, without kthCounts
    computeBlockwiseWithinKCounts<Bitwise, T>/* SYCL: launch with nd_range(grid, RADIX_DIGITS, 0, stream) */(
    ...
    desiredMask = at::sycl::Bitfield<Bitwise>::setBitfield(desiredMask, RADIX_MASK, current_bit, RADIX_BITS);
  }
  desired = desired_in;

#if !defined(USE_ROCM)
  // CUDA path: Separate kernel for kthCounts
  computeBlockwiseKthCounts<Bitwise>/* SYCL: launch with nd_range(std::min(((int64_t)numInputSlices + 255) / 256, (int64_t)1073741824), 256, 0, stream) */(
  ...
  at::sycl::cub::inclusive_sum_by_key(slice_idx_iter, withinKCounts, withinKCounts, num_blocks);
  at::sycl::cub::inclusive_sum_by_key(slice_idx_iter, kthCounts, kthCounts, num_blocks);
#else
  ...
  gatherTopK<T, IndexType, Dim>/* SYCL: launch with nd_range(grid, block, 0, stream) */(
...
#define RUN_T(INDEX_T)                                                    \
  AT_DISPATCH_ALL_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16, input.scalar_type(), "topk_out_opencl", [&] { \
    at::sycl::detail::TensorInfo<const scalar_t, INDEX_T> inputInfo =     \
      at::sycl::detail::getTensorInfo<const scalar_t, INDEX_T>(input);    \
    at::sycl::detail::TensorInfo<scalar_t, INDEX_T> topKInfo =            \
      at::sycl::detail::getTensorInfo<scalar_t, INDEX_T>(values);         \
    at::sycl::detail::TensorInfo<int64_t, INDEX_T> indicesInfo =          \
      at::sycl::detail::getTensorInfo<int64_t, INDEX_T>(indices);         \
  ...
  if (input.numel() > 0) {
    if (at::sycl::detail::canUse32BitIndexMath(input) &&
        at::sycl::detail::canUse32BitIndexMath(values) &&
        at::sycl::detail::canUse32BitIndexMath(indices)) {
      RUN_T(uint32_t);
    } else {
      RUN_T(uint64_t);
    }
