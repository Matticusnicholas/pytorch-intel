...
// Source: aten/src/ATen/native/cuda/IndexKernel.cu
...
/* SYCL kernel */ void index_elementwise_kernel(const int64_t N, const func_t f) {
...
static void launch_kernel(const int64_t N, const func_t& f) {
  ...
  index_elementwise_kernel<nt, vt, func_t>/* SYCL: kernel launch with nd_range(grid, block, 0, stream) */(N, f);
...
void gpu_index_kernel(TensorIteratorBase& iter, const IntArrayRef index_size, const IntArrayRef index_stride, const func_t& f, const bool is_gather_like) {
  ...
  launch_kernel<launch_size_nd, launch_bound2>(iter.numel(), [=]__device__(int idx) {
    ...
    for (int i = 0; i < num_indices; i++) {
      ...
      SYCL_KERNEL_ASSERT(-sizes[i] <= index && index < sizes[i] && "index out of bounds");
      if (index < 0) {
        index += sizes[i];
      }
...
void index_fill_kernel_impl(
  ...
  const auto loop = [=]C10_DEVICE(int i) {
    ...
    SYCL_KERNEL_ASSERT(idx >= -self_dim_size && idx < self_dim_size && "index out of bounds");
    if (idx < 0) {
      idx += self_dim_size;
    }
...
void index_copy_kernel_impl(
  ...
  const auto loop = [=]C10_DEVICE(int i) {
    ...
    SYCL_KERNEL_ASSERT(idx >= 0 && idx < self_dim_size && "index_copy_(): index out of bounds");
...
void cuda_take_put_kernel(
  ...
  const auto loop = [=]C10_DEVICE(int i) {
    ...
    SYCL_KERNEL_ASSERT(idx < numel && idx >= -numel && "cuda_take_put_kernel() index out of bounds");
...
void put_kernel(TensorIterator& iter, const TensorBase& output, const bool accumulate) {
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND3(at::ScalarType::Half, at::ScalarType::Bool, at::ScalarType::BFloat16, iter.dtype(), "put_opencl", [&] {
    // Cannot use `OpaqueType`, as we need the actual type for `fastSpecializedgpuAtomicAdd`
    AT_DISPATCH_INDEX_TYPES(sycl::detail::canUse32BitIndexMath(output) ? ScalarType::Int : ScalarType::Long,
        "put_cuda_index", [&] {
           ...
           if (accumulate) {
             ...
             cuda_take_put_kernel<scalar_t, index_t>(iter, output,
                 [numel, indexed_ptr] __device__(scalar_t& iterated, const index_t offset) {
                   sycl_fast_specialized_atomic_add(indexed_ptr, offset, numel, iterated);
                 });
           }
           else {
             cuda_take_put_kernel<scalar_t, index_t>(iter, output,
...
void take_kernel(
  ...
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND3(at::ScalarType::Half, at::ScalarType::Bool, at::ScalarType::BFloat16, iter.dtype(), "take_opencl", [&] {
    // Cannot use `OpaqueType`, as Tensor::data_ptr<OpaqueType<N>> is not implemented
    AT_DISPATCH_INDEX_TYPES(sycl::detail::canUse32BitIndexMath(input) ? ScalarType::Int : ScalarType::Long,
      "take_cuda_index", [&] {
...
/* SYCL kernel */ void masked_scatter_size_check(
  ...
  SYCL_KERNEL_ASSERT(totalElements <= srcSize);
}
...
void launch_masked_scatter_kernel(
  ...
  at::sycl::onedpl::mask_exclusive_sum(
  ...
  masked_scatter_size_check/* SYCL: kernel launch with nd_range(1, 1, 0, at::sycl::getCurrentSYCLStream()) */(
  ...
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND3(
      ...
      [&]() {
        ...
        sycl_kernel(
            iter, [=] SYCL_LAMBDA(const scalar_t a, const bool mask, const int64_t maskPrefixSum) -> scalar_t {
              if (mask) {
                return source_ptr[maskPrefixSum];
              }
              return a;
            });
        // SYCL: error check handled by SYCL runtime;
      });
}
...
void flip_kernel(TensorIterator& iter, const bool quantized) {
  if (quantized) {
    AT_DISPATCH_QINT_AND_SUB_BYTE_TYPES(iter.dtype(), "flip_quantized_opencl",
    [&] {
      using dtype = OpaqueType<sizeof(scalar_t)>;
...
REGISTER_OPENCL_DISPATCH(index_put_kernel_quantized_stub, &index_put_kernel_quantized_cuda)
