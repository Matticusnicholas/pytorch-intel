...
// Source: aten/src/ATen/native/cuda/LogAddExpKernel.cu
...
c10::complex<scalar_t> _logaddexp_minmax(const c10::complex<scalar_t>& x, const c10::complex<scalar_t>& y) {
...
void logaddexp_kernel_opencl(TensorIteratorBase& iter) {
  if (at::isComplexType(iter.dtype())) {
// SYCL: JIT path disabled, using direct dispatch
#if 0 /* AT_USE_JITERATOR - disabled for SYCL */
    AT_DISPATCH_COMPLEX_TYPES_AND(at::ScalarType::ComplexHalf, iter.dtype(), "logaddexp_opencl", [&]() {
      jitted_gpu_kernel<
...
void logaddexp2_kernel_opencl(TensorIteratorBase& iter) {
  AT_DISPATCH_FLOATING_TYPES_AND2(
      ScalarType::BFloat16, ScalarType::Half,
      iter.dtype(), "logaddexp2_opencl",
      [&]() {
        using opmath_t = at::opmath_type<scalar_t>;
        const auto inv_log_2 = static_cast<opmath_t>(1.0 / c10::ln_2<double>);
        sycl_kernel(iter, [inv_log_2] SYCL_LAMBDA (scalar_t a_, scalar_t b_) -> scalar_t {
          const auto a = static_cast<opmath_t>(a_);
          const auto b = static_cast<opmath_t>(b_);
          if (::isinf(a) && a == b) {
            return a;
...
REGISTER_DISPATCH(logaddexp_stub, &logaddexp_kernel_opencl)
REGISTER_DISPATCH(logaddexp2_stub, &logaddexp2_kernel_opencl)
