...
// Source: aten/src/ATen/native/cuda/MixedDtypesLinear.cu
...
#else
// SYCL: cuda_fp16 not needed, using sycl::half
// #include <cuda_fp16.h>
// SYCL: cuda_runtime not needed
...
mixed_dtypes_linear_cutlass(
  ...
  status = gemm_op.run(at::sycl::getCurrentSYCLStream());
  CUTLASS_STATUS_CHECK(status);
...
_mixed_dtypes_linear(const Tensor& input, const Tensor& weight,
  ...
  const auto dprops = at::sycl::getCurrentDeviceProperties();
