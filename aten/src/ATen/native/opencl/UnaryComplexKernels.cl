...
#include <ATen/native/sycl/Copy.h>
...
static inline scalar_t angle_wrapper(scalar_t v) {
  if (at::_isnan(v)){
    return v;
  }
  return v < 0 ? M_PI : 0;
...
template<typename T>
static inline c10::complex<T> angle_wrapper(c10::complex<T> v) {
...
void angle_kernel_opencl(TensorIteratorBase& iter) {
  ...
  if (at::isComplexType(dtype)) {
    ...
    AT_DISPATCH_COMPLEX_TYPES_AND(kComplexHalf, dtype, "angle_opencl", [&]() {
        jitted_gpu_kernel<
          /*name=*/ angle_name,
    ...
    AT_DISPATCH_FLOATING_TYPES(dtype, "angle_opencl", [&]() {
        sycl_kernel(iter, []SYCL_LAMBDA(scalar_t a) -> scalar_t {
          return angle_wrapper(a);
        });
    });
...
void conj_kernel_opencl(TensorIteratorBase& iter) {
  ...
  AT_DISPATCH_SWITCH(iter.common_dtype(), "conj_opencl",
...
REGISTER_DISPATCH(angle_stub, &angle_kernel_opencl)
REGISTER_DISPATCH(conj_physical_stub, &conj_kernel_opencl)
