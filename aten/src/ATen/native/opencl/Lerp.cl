...
// Source: aten/src/ATen/native/cuda/Lerp.cu
...
// SYCL: JIT path disabled, using direct dispatch
  ...
  AT_DISPATCH_COMPLEX_TYPES_AND(kComplexHalf, dtype, "lerp_opencl", [&] {
...
// SYCL: JIT path disabled, using direct dispatch
  ...
  AT_DISPATCH_COMPLEX_TYPES_AND(kComplexHalf, dtype, "lerp_opencl", [&] {
    ...
    at::native::sycl_kernel(
        iter,
        [=] SYCL_LAMBDA(scalar_t self_val, scalar_t end_val) {
          opmath_t self_val_f = self_val;
          opmath_t end_val_f = end_val;
          return lerp(self_val, end_val, weight_val);
        });
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
      ...
      [&]{
        ...
        auto weight_val = weight.to<opmath_t>();
        at::native::sycl_kernel(
            iter, [=] SYCL_LAMBDA(scalar_t self_val, scalar_t end_val) {
              return lerp(self_val, end_val, weight_val);
            });
      });
    }
