...
// Source: aten/src/ATen/native/cuda/RowwiseScaledMM.cu
...
void f8f8bf16_rowwise_impl(
  ...
  status = gemm.initialize(arguments, workspace.data_ptr(), at::sycl::getCurrentSYCLStream());
  if (status != cutlass::Status::kSuccess) {
    throw std::runtime_error("cutlass cannot initialize");
  }

  status = gemm(at::sycl::getCurrentSYCLStream());
  if (status != cutlass::Status::kSuccess) {
...
void dispatch_fp8_rowwise_kernel_on_sm(
    ...
    std::optional<at::Tensor> bias,
    at::Tensor out) {
  cudaDeviceProp* properties = at::sycl::getCurrentDeviceProperties();
