...
// SYCL kernel
void upsample_bicubic2d_out_frame(
...
C10_LAUNCH_BOUNDS_1(1024)
// SYCL kernel
void upsample_bicubic2d_out_frame_parallel(
    const int num_elements,
    const accscalar_t height_scale,
    const accscalar_t width_scale,
    const bool align_corners,
...
static void upsample_bicubic2d_out_opencl_template(
    const Tensor& output,
    const Tensor& input,
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
      ...
      input.scalar_type(), "upsample_bicubic2d_out_frame", [&] {
        ...
        if (num_output_elements <= 18432 && input.size(0) * input.size(1) > 0) {
          ...
          sycl::range<3> grid(num_blocks, 1, grid_z);
          upsample_bicubic2d_out_frame_parallel<scalar_t, accscalar_t>
              /* SYCL: launch with nd_range(grid, max_threads, 0, stream) */(
                  num_output_elements,
                  rheight,
                  rwidth,
                  align_corners,
          ...
          upsample_bicubic2d_out_frame<scalar_t, accscalar_t>
              /* SYCL: launch with nd_range(num_blocks, max_threads, 0, stream) */(
                  num_output_elements,
                  rheight,
                  rwidth,
                  align_corners,
...
static void upsample_bicubic2d_backward_out_opencl_template(
...
TORCH_IMPL_FUNC(upsample_bicubic2d_out_opencl) (
    const Tensor& input,
    IntArrayRef output_size,
    bool align_corners,
    std::optional<double> scales_h,
    std::optional<double> scales_w,
    const Tensor& output) {
  upsample_bicubic2d_out_opencl_template(output, input, output_size, align_corners, scales_h, scales_w);
}

TORCH_IMPL_FUNC(upsample_bicubic2d_backward_out_opencl) (
  ...
  globalContext().alertNotDeterministic("upsample_bicubic2d_backward_out_opencl");
  upsample_bicubic2d_backward_out_opencl_template(
