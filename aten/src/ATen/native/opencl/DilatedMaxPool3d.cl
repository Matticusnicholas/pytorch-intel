...
// Source: aten/src/ATen/native/cuda/DilatedMaxPool3d.cu
// SYCL TODO: needs_review - complex file (653 lines), kernel launch patterns need manual verification
...
/* SYCL kernel */ static void max_pool3d_with_indices_single_out_frame(
  ...
  bool channels_last)
{
  int oColumn = item.get_group(0) * item.get_local_range(0) + item.get_local_id(0);
  int oRow = item.get_group(1) * item.get_local_range(1) + item.get_local_id(1);
  int oFrame = 0;
  ...
  if (!channels_last) {
    ...
    oFrame = (item.get_group(2) * item.get_local_range(2) + item.get_local_id(2) + offsetZ) % otime; // output frame/time
    slice = (item.get_group(2) * item.get_local_range(2) + item.get_local_id(2) + offsetZ) / otime; // output slice/feature
  } else {
    // indexing order: batch, time, channel
    channel = (item.get_group(2) * item.get_local_range(2) + item.get_local_id(2) + offsetZ) % features; // output feature (channel)
    slice = (item.get_group(2) * item.get_local_range(2) + item.get_local_id(2) + offsetZ) / features; // output slice (batch + time)
    batch = slice / otime;
    oFrame = slice % otime;
  }
  ...
  bool channels_last)
{
  ...
  sycl::range<3> block(threadX, threadY, threadZ);

  while (totalZ > 0) {
    sycl::range<3> grid(ceil_div(owidth, static_cast<int>(block.x)),
...
template <typename scalar_t>
/* SYCL kernel */ static void max_pool3d_with_indices_backward_single_out_frame(
  ...
  Tensor& gradInput)
{
  ...
  globalContext().alertNotDeterministic("max_pool3d_with_indices_backward_out_opencl");
  max_pool3d_with_indices_backward_out_cuda_template(
    gradInput,
    gradOutput,
    input,
  ...
  const Tensor& indices)
{
  // See Note [Writing Nondeterministic Operations]
  // Nondeterministic because of atomicAdd usage
  globalContext().alertNotDeterministic("max_pool3d_with_indices_backward_opencl");
