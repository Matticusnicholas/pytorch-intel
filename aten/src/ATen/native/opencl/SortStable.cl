...
#include <ATen/native/sycl/SortStable.h>
...
void sort_postprocess_kernel(
    ...
    int nsegments,
    int nsort) {
  SYCL_KERNEL_LOOP(i, nsegments * nsort) {
    int segment = i / nsort;
    int j = i % nsort;
...
// SYCL kernel
void fill_index_and_segment_kernel(
    int2* data,
    int numel,
    at::sycl::detail::IntDivider<uint32_t> nsort_divider) {
  SYCL_KERNEL_LOOP(idx, numel) {
...
C10_LAUNCH_BOUNDS_1(at::sycl::detail::CUDA_NUM_THREADS)
// SYCL kernel
void fill_reverse_indices_kernel(
    int64_t* data,
    int numel,
    at::sycl::detail::IntDivider<uint32_t> nsort_divider) {
  SYCL_KERNEL_LOOP(idx, numel) {
...
inline void segmented_sort_large_segments(
  ...
  sycl::range<3> grid = GET_BLOCKS(nsort);
  c10::DeviceArray<int64_t> indices(*allocator, nsort);
  at::sycl::detail::IntDivider<uint32_t> nsort_divider(nsort);
  fill_reverse_indices_kernel/* SYCL: launch with nd_range(grid, block, 0, stream) */(
      indices.get(), nsort, nsort_divider);
  const int64_t* initial_indices = indices.get();

  for (auto i : c10::irange(nsegments)) {
    at::sycl::cub::radix_sort_pairs<scalar_t, int64_t>(
...
inline void segmented_sort_pairs_by_full_sort(
  ...
  using namespace at::sycl::detail;
  sycl::range<3> block = CUDA_NUM_THREADS;
  sycl::range<3> grid = GET_BLOCKS(numel);
  auto stream = c10::sycl::getCurrentSYCLStream();
  at::sycl::detail::IntDivider<uint32_t> nsort_divider(nsort);
  fill_index_and_segment_kernel/* SYCL: launch with nd_range(grid, block, 0, stream) */(
  ...
  at::sycl::cub::radix_sort_pairs<scalar_t, int2>(
  ...
  at::sycl::cub::radix_sort_keys<int64_t>(
  ...
  sort_postprocess_kernel/* SYCL: launch with nd_range(
      (n + 511) / 512,
      512,
      0,
...
void segmented_sort_pairs(
  ...
  at::sycl::cub::segmented_sort_pairs(
      self_ptr,
      values_ptr,
      reverse_indices_ptr,
      indices_ptr,
