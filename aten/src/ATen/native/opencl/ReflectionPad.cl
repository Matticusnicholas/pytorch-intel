...
// Source: aten/src/ATen/native/cuda/ReflectionPad.cu
...
using at::sycl::detail::canUse32BitIndexMath;
...
inline std::pair<int64_t, int64_t> get_index_mapping1d(
  ...
  auto input_offset =
    (item.get_group(1) + item.get_group(2) * item.get_group_range(1)) * input_w;
  auto output_offset =
    (item.get_group(1) + item.get_group(2) * item.get_group_range(1)) * output_w;
...
inline std::pair<int64_t, int64_t>  get_index_mapping2d(
  ...
  // 3D grid of 1D blocks
  auto input_offset =
    ((item.get_group(1) + y_shift) + (item.get_group(2) + z_shift) * nplane) * input_dim_x * input_dim_y;
  auto output_offset =
    ((item.get_group(1) + y_shift) + (item.get_group(2) + z_shift) * nplane) * output_dim_x * output_dim_y;
...
inline int64_t reflect_index(int64_t x, int64_t len) {
...
/* SYCL kernel */ void reflection_pad1d_out_kernel(
    const scalar_t * input, scalar_t * output,
    int64_t input_w,
    int64_t pad_l, int64_t pad_r) {
  auto output_x = item.get_local_id(0) + item.get_group(0) * item.get_local_range(0);
...
template <typename scalar_t>
/* SYCL kernel */ void reflection_pad1d_flat(
    const scalar_t* __restrict__ input,
    scalar_t* __restrict__ output,
    int64_t input_w, int64_t pad_l, int64_t pad_r,
    int64_t out_w, int64_t plane_count) {

  const int64_t bx = item.get_local_range(0);
  const int64_t tx = item.get_local_id(0);

  const int64_t total = plane_count * out_w;
  const int64_t grid_stride = static_cast<int64_t>(bx) * item.get_group_range(0);
  int64_t linear = static_cast<int64_t>(item.get_group(0)) * bx + tx;
...
template <typename scalar_t>
/* SYCL kernel */ void reflection_pad1d_backward_out_kernel(
  ...
  if (output_x < output_w) {
    auto index_pair = get_index_mapping1d(input_w, output_w, output_x, pad_l);
    sycl_atomic_add_no_return(
      &grad_input[index_pair.first], grad_output[index_pair.second]);
  }
}

template<typename scalar_t>
/* SYCL kernel */ void reflection_pad2d_out_kernel(
    const scalar_t * input, scalar_t * output,
    int64_t input_dim_x, int64_t input_dim_y,
    int pad_t, int pad_b, int pad_l, int pad_r, int y_shift, int z_shift, int nplane) {
  auto output_xy = item.get_local_id(0) + item.get_group(0) * item.get_local_range(0);
...
template <typename scalar_t>
/* SYCL kernel */ void reflection_pad2d_backward_out_kernel(
    scalar_t * grad_input, const scalar_t * grad_output,
    int64_t input_dim_x, int64_t input_dim_y,
    int pad_t, int pad_b, int pad_l, int pad_r, int y_shift, int z_shift, int nplane) {
  auto output_xy = item.get_local_id(0) + item.get_group(0) * item.get_local_range(0);
  ...
  if (output_xy < output_dim_x * output_dim_y) {
    ...
    sycl_atomic_add_no_return(&grad_input[index_pair.first], grad_output[index_pair.second]);
  }
}

template <typename scalar_t>
/* SYCL kernel */ void reflection_pad2d_backward_det_out_kernel(
    scalar_t* grad_input,
    ...
    int channels,
    int) {
  const int64_t input_xy_ = item.get_local_id(0) + item.get_group(0) * item.get_local_range(0);
  ...
  const int64_t stride =
      static_cast<int64_t>(item.get_group_range(0)) * static_cast<int64_t>(item.get_local_range(0));
  const int64_t end =
...
inline void parallel_reflection_pad3d(
    ...
    int64_t z_shift,
    const F& f) {
  int64_t output_id = item.get_local_id(0) + ((int64_t) item.get_group(0)) * item.get_local_range(0);
  ...
  int64_t plane = item.get_group(1) + y_shift;
  int64_t batch = item.get_group(2) + z_shift;
  f(plane, batch, output_z, output_y, output_x, input_z, input_y, input_x);
}

template<typename scalar_t>
/* SYCL kernel */ void reflection_pad3d_out_kernel(
...
template <typename scalar_t>
/* SYCL kernel */ void reflection_pad3d_backward_out_kernel(
  ...
  parallel_reflection_pad3d(
      ...
      [&] __device__(
        ...
        sycl_atomic_add_no_return(target, value_to_add);
      });
}
...
void reflection_pad2d_out_template(
  ...
  sycl::range<3> block_size(output_plane_size > 256 ? 256 : output_plane_size);
    ...
    input.scalar_type(), "reflection_pad2d_out_template", [&] {
      ...
      for (int64_t block_y = 0; block_y < size_y; block_y += 65535) {
        ...
        for (int64_t block_z = 0; block_z < size_z; block_z += 65535) {
          ...
          sycl::range<3> grid_size(at::ceil_div(output_plane_size, static_cast<int64_t>(256)), block_y_size, block_z_size);

          reflection_pad2d_out_kernel/* SYCL: kernel launch with nd_range(
            grid_size, block_size, 0, at::sycl::getCurrentSYCLStream()) */(
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(
      ...
      [&] {
        if (at::globalContext().deterministicAlgorithms()) {
          ...
          reflection_pad2d_backward_det_out_kernel/* SYCL: kernel launch with nd_range(
              grid_size,
              block_size,
              0,
          ...
          for (int64_t block_y = 0; block_y < size_y; block_y += 65535) {
            ...
            for (int64_t block_z = 0; block_z < size_z; block_z += 65535) {
              ...
              reflection_pad2d_backward_out_kernel/* SYCL: kernel launch with nd_range(
                  grid_size,
                  block_size,
                  0,
                  at::sycl::getCurrentSYCLStream()) */(
...
TORCH_IMPL_FUNC(reflection_pad1d_out_opencl)
(const Tensor& input_, IntArrayRef padding, const Tensor& output) {
  ...
  const cudaDeviceProp* prop = at::sycl::getCurrentDeviceProperties();
  ...
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND2(kHalf, kBFloat16, input.scalar_type(), "reflection_pad1d_out", [&] {
    ...
    if (fits3d) {
      sycl::range<3> block(block_x, 1, 1);
      sycl::range<3> grid(gx, static_cast<unsigned>(nplane), static_cast<unsigned>(nbatch));
      reflection_pad1d_out_kernel<scalar_t>/* SYCL: kernel launch with nd_range(grid, block, 0, stream) */(
      ...
      reflection_pad1d_flat<scalar_t>/* SYCL: kernel launch with nd_range(grid, block, 0, stream) */(
          input.const_data_ptr<scalar_t>(),
          output.mutable_data_ptr<scalar_t>(),
...
TORCH_IMPL_FUNC(reflection_pad1d_backward_out_opencl)(const Tensor& grad_output_,
    const Tensor& input,
    IntArrayRef padding,
    const Tensor& grad_input) {
  // See Note [Writing Nondeterministic Operations]
  // Nondeterministic because of sycl_atomic_add usage
  globalContext().alertNotDeterministic("reflection_pad1d_backward_out_opencl");
  grad_input.zero_();
  ...
  sycl::range<3> block_size(output_w > 256 ? 256 : output_w);
  sycl::range<3> grid_size((int) ::ceil(output_w / 256.0), nplane, nbatch);

  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(kHalf, kBFloat16,
    grad_input.scalar_type(), "reflection_pad1d_backward_out_opencl", [&] {
      reflection_pad1d_backward_out_kernel/* SYCL: kernel launch with nd_range(
        grid_size, block_size, 0, at::sycl::getCurrentSYCLStream()) */(
...
Tensor& reflection_pad2d_out_opencl(const Tensor& input, IntArrayRef padding,
    Tensor& output) {
...
Tensor& reflection_pad2d_backward_out_opencl(const Tensor& grad_output,
    const Tensor& input,
    IntArrayRef padding,
    Tensor& grad_input) {
...
Tensor reflection_pad2d_backward_opencl(
...
TORCH_IMPL_FUNC(reflection_pad3d_out_opencl) (
  const Tensor& input_, IntArrayRef padding, const Tensor& output
  ) {
  TORCH_CHECK(
      canUse32BitIndexMath(input_),
  ...
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND2(kHalf, kBFloat16,
      input.scalar_type(), "reflection_pad3d_out_opencl", [&] {
        ...
        for (int64_t block_y = 0; block_y < size_y; block_y += 65535) {
          ...
          for (int64_t block_z = 0; block_z < size_z; block_z += 65535) {
            ...
            sycl::range<3> grid_size(at::ceil_div(output_plane_size, static_cast<int64_t>(256)), \
                           block_y_size, block_z_size);

            reflection_pad3d_out_kernel/* SYCL: kernel launch with nd_range(
                grid_size, block_size,0, at::sycl::getCurrentSYCLStream()) */(
...
TORCH_IMPL_FUNC(reflection_pad3d_backward_out_opencl) (
  const Tensor& grad_output, const Tensor& input, IntArrayRef padding,
  const Tensor& grad_input) {
  globalContext().alertNotDeterministic("reflection_pad3d_backward_out_opencl");
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(kHalf, kBFloat16,
      input.scalar_type(), "reflection_pad3d_backward_out_opencl", [&] {
        ...
        for (int64_t block_y = 0; block_y < size_y; block_y += 65535) {
          ...
          for (int64_t block_z = 0; block_z < size_z; block_z += 65535) {
            ...
            reflection_pad3d_backward_out_kernel/* SYCL: kernel launch with nd_range(
                grid_size, block_size,0, at::sycl::getCurrentSYCLStream()) */(
