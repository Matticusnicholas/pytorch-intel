...
// SYCL: cuda_runtime.h not needed
...
#if !(defined(_WIN32) && SYCL_VERSION == 12090)
...
__global__ static void post_sum_div_kernel(
    ...
    bool is_initial_set,
    scalar_t initial) {
  SYCL_KERNEL_LOOP(index, segment_count) {
...
Tensor _segment_reduce_lengths_offsets_backward_opencl_kernel(
  ...
  AT_DISPATCH_INDEX_TYPES(
      lengths_or_offsets_contig.scalar_type(), "_segment_reduce_opencl_lengths_offsets_backward_kernel1", ([&] {
        ...
        AT_DISPATCH_FLOATING_TYPES_AND2(
            ...
            ([&]() {
              ...
              segment_reduce_backward_kernel<scalar_t>
                  /* SYCL: launch with nd_range(num_blocks,
                     threads_per_block,
                     0,
...
Tensor _segment_reduce_lengths_backward_opencl_kernel(
  const Tensor& grad_contig,
  const Tensor& output_contig,
  const Tensor& data_contig,
  ReductionType reduction,
  ...
  int64_t axis,
  const std::optional<Scalar>& initial) {
  return _segment_reduce_lengths_offsets_backward_opencl_kernel(
    grad_contig, output_contig, data_contig, reduction, lengths_contig, axis, initial, /*is_offsets_like=*/false);
}

Tensor _segment_reduce_offsets_backward_opencl_kernel(
  const Tensor& grad_contig,
  const Tensor& output_contig,
  const Tensor& data_contig,
  ReductionType reduction,
...
Tensor _segment_reduce_lengths_offsets_opencl_kernel(
  ReductionType reduction,
  const Tensor& data,
  ...
  AT_DISPATCH_INDEX_TYPES(
      lengths_or_offsets.scalar_type(), "_segment_reduce_opencl_kernel1", ([&] {
        ...
        AT_DISPATCH_FLOATING_TYPES_AND2(
            ...
            [&]() {
              ...
              if (output_shape.size() > 1) {
                ...
                if (reduction == ReductionType::MAX) {
                  CustomMax max_op{};
                  // SYCL: Use oneDPL segmented reduce instead of CUB
    // CUB_WRAPPER(
...
Tensor _segment_reduce_lengths_opencl_kernel(
  ReductionType reduction,
  const Tensor& data,
  const Tensor& lengths,
  int64_t axis,
  const std::optional<Scalar>& initial) {
  return _segment_reduce_lengths_offsets_opencl_kernel(
    reduction, data, lengths, axis, initial, /*is_offsets_like=*/false);
}

Tensor _segment_reduce_offsets_opencl_kernel(
  ReductionType reduction,
  const Tensor& data,
...
REGISTER_DISPATCH(_segment_reduce_lengths_stub, &_segment_reduce_lengths_opencl_kernel)
REGISTER_DISPATCH(_segment_reduce_offsets_stub, &_segment_reduce_offsets_opencl_kernel)
REGISTER_DISPATCH(
    _segment_reduce_lengths_backward_stub,
    &_segment_reduce_lengths_backward_opencl_kernel);
REGISTER_DISPATCH(
  _segment_reduce_offsets_backward_stub,
  &_segment_reduce_offsets_backward_opencl_kernel);
