...
void triu_tril_kernel(
    sycl::detail::TensorInfo<scalar_t, IndexType> result_info,
    const sycl::detail::TensorInfo<const scalar_t, IndexType> self_info,
    const int64_t k,
    const int64_t N_padded,
    const IndexType last_dim_padded) {
  int64_t linear_idx = (((int64_t)item.get_group(0)) * item.get_local_range(0) + item.get_local_id(0)) * elements_per_thread;
...
template <bool upper>
void triu_tril_opencl_template(const Tensor& result, const Tensor& self, int64_t k, const char* name) {
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND4(
      at::ScalarType::ComplexHalf,
      at::ScalarType::Half,
      at::ScalarType::BFloat16,
      at::ScalarType::Bool,
      self.scalar_type(), "triu_tril_opencl_template", [&] {
    ...
    sycl::range<3> dim_block = block_size;
    sycl::range<3> dim_grid((N_padded / elements_per_thread + dim_block.x - 1) / dim_block.x);
    if (sycl::detail::canUse32BitIndexMath(result) && sycl::detail::canUse32BitIndexMath(self)) {
      auto result_info = sycl::detail::getTensorInfo<scalar_t, int32_t>(result);
      auto self_info = sycl::detail::getTensorInfo<const scalar_t, int32_t>(self);
      BOOL_SWITCH(self.is_same(result), inplace, [&] {
        triu_tril_kernel<scalar_t, int32_t, upper, elements_per_thread, inplace>
          /* SYCL: launch with nd_range(dim_grid, dim_block, 0, at::sycl::getCurrentSYCLStream()) */(
            result_info, self_info, k, N_padded, last_dim_padded);
      });
      // SYCL: kernel launch check handled by SYCL runtime;
    } else {
      auto result_info = sycl::detail::getTensorInfo<scalar_t, int64_t>(result);
      auto self_info = sycl::detail::getTensorInfo<const scalar_t, int64_t>(self);
      BOOL_SWITCH(self.is_same(result), inplace, [&] {
...
TORCH_IMPL_FUNC(tril_opencl)(const Tensor& self, int64_t k, const Tensor &result) {
  if (self.numel() != 0) {
    triu_tril_opencl_template<false>(result, self, k, "tril");
  }
}

TORCH_IMPL_FUNC(triu_opencl)(const Tensor& self, int64_t k, const Tensor &result) {
  if (self.numel() != 0) {
    triu_tril_opencl_template<true>(result, self, k, "triu");
  }
}

Tensor trace_opencl(const Tensor& self) {
