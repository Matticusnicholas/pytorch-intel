...
// Source: aten/src/ATen/native/cuda/Dropout.cu
...
fused_dropout_kernel_vec(at::cuda::detail::TensorInfo<const scalar_t, IndexType> a,
  ...
  for (IndexType linearIndex = idx * VEC;
      ...
      linearIndex += item.get_group_range(0) * item.get_local_range(0) * VEC) {
    // local storage
    scalar_t src[VEC];
...
fused_dropout_kernel(cuda::detail::TensorInfo<const scalar_t, IndexType> a,
  ...
  IndexType rounded_size = ((totalElements - 1)/(item.get_local_range(0) * item.get_group_range(0) * UNROLL)+1) *
        item.get_local_range(0) * item.get_group_range(0) * UNROLL;
  for (IndexType linearIndex = idx;
       linearIndex < rounded_size;
       linearIndex += item.get_group_range(0) * item.get_local_range(0)*UNROLL) {
//curand_uniform_double was pure evil anyway, not doing what it promises, and there's nothing for Halfs, so generate float for everything
       ...
       for (int ii = 0; ii < UNROLL; ii++) {
           IndexType li = linearIndex + item.get_local_range(0) * item.get_group_range(0) * ii;
           if (li < totalElements) {
...
void masked_scale_kernel(at::Tensor& ret, const at::Tensor& src, const at::Tensor& mask, accscalar_t scale){
   ...
   at::native::sycl_kernel(
       iter,
       [=]SYCL_LAMBDA(const scalar_t src_val, const mask_t mask_val) -> scalar_t {
...
inline void launcher(
    ...
    double p,
    const int64_t nelem,
    const PhiloxSYCLState rng_engine_inputs,
    sycl::range<3> grid,
    sycl::range<3> dim_block) {
          ...
          switch (vec_size) {
            case 16:
              fused_dropout_kernel_vec<
                  ...
                  1,
                  16>
                  /* SYCL: submit kernel with nd_range({grid=grid, block=dim_block, smem=0, stream=at::sycl::getCurrentSYCLStream()}) */(
                      self_info,
                      ret_info,
                      mask_info,
                      nelem,
...
dropout_cuda(SYCLGeneratorImpl* gen, const Tensor& self, double p){
  ...
  sycl::range<3> grid((nelem + block_size -1)/block_size);
...
native_dropout_cuda(const Tensor& self, double p, std::optional<bool> train){
  ...
  auto gen = get_generator_or_default<SYCLGeneratorImpl>(std::nullopt, sycl::detail::getDefaultSYCLGenerator());
  double p1m = 1. - p;
