...
// Source: aten/src/ATen/native/cuda/EmbeddingBackwardKernel.cu
...
void krn_partials_per_segment(index_t *ret, const index_t *segment_offsets,
  ...
  const int id = item.get_group(0) * item.get_local_range(0) + item.get_local_id(0);
...
/* SYCL kernel */ void compute_grad_weight_bags(
  ...
  int64_t num_of_segments = *num_of_segments_ptr;
  const int64_t gid = static_cast<int64_t>(item.get_group(0)) * item.get_local_range(0) + item.get_local_id(0);
...
template <typename scalar_t, typename index_t>
/* SYCL kernel */ void compute_grad_weight(
...
/* SYCL kernel */ void compute_grad_weight_atomic_accumulate(
  ...
  using accscalar_t = acc_type<scalar_t, true>;
  const int32_t gid = item.get_group(0) * item.get_local_range(0) + item.get_local_id(0);
...
template <typename index_t>
/* SYCL kernel */ void krn_partial_to_segment_idx(
    index_t *partial_to_segment_idx,
    const index_t *partials_per_segment,
    const index_t *partials_per_segment_offset,
    const int64_t *num_of_segments_ptr) {
  int64_t num_of_segments = *num_of_segments_ptr;
  const int32_t id = item.get_group(0) * item.get_local_range(0) + item.get_local_id(0);
  if (id < num_of_segments) {
    index_t idx = partials_per_segment_offset[id];
    const index_t num_partials = partials_per_segment[id];
...
template <typename scalar_t, typename index_t>
/* SYCL kernel */ void sum_and_scatter(
...
template<typename index_t>
/* SYCL kernel */ void compute_num_of_partial_segments(const index_t *partials_per_segment, const index_t *partials_per_segment_offset, const int64_t *num_of_segments_ptr, int64_t *output) {
  int64_t num_of_segments = *num_of_segments_ptr;
...
Tensor embedding_backward_opencl_kernel(
        const Tensor &grad,
  ...
  AT_DISPATCH_INDEX_TYPES(orig_indices.scalar_type(), "embedding_backward_opencl_kernel", [&] () {
  ...
  AT_DISPATCH_INDEX_TYPES(orig_indices.scalar_type(), "embedding_backward_opencl_kernel", [&] () {
    ...
    {
      krn_partials_per_segment/* SYCL: submit kernel with nd_range({grid=ceil_div(max_segments, block=32), smem=32, stream=0, stream}) */ (
    ...
    compute_num_of_partial_segments<index_t>/* SYCL: submit kernel with nd_range({grid=1, block=1, smem=0, stream=c10::sycl::getCurrentSYCLStream()}) */(
      partials_per_segment.const_data_ptr<index_t>(),
    ...
    {
      krn_partial_segment_offset/* SYCL: submit kernel with nd_range({grid=ceil_div(max_segments, block=32), smem=32, stream=0, stream}) */ (
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16,
      grad.scalar_type(), "embedding_bag_backward_cuda_compute_grad_weight", [&] {
        ...
        if (use_fused_kernel) {
          ...
          {
            krn_partial_to_segment_idx/* SYCL: submit kernel with nd_range({grid=ceil_div(max_segments, block=32), smem=32, stream=0, stream}) */(
          ...
          if constexpr (!std::is_same_v<scalar_t, opmath_type<scalar_t>>) {
            ...
            compute_grad_weight_atomic_accumulate<scalar_t, partial_weight_t>/* SYCL: submit kernel with nd_range({grid=grid, block=block, smem=0, stream=stream}) */(
            ...
            // For float/double, accumulate directly into grad_weight
            compute_grad_weight_atomic_accumulate<scalar_t, scalar_t>/* SYCL: submit kernel with nd_range({grid=grid, block=block, smem=0, stream=stream}) */(
                orig_indices.const_data_ptr<index_t>(),
                grad.const_data_ptr<scalar_t>(),
                count.defined() ? count.const_data_ptr<index_t>() : nullptr,
                numel, stride,
          ...
          if (offset2bag.defined()) {
                compute_grad_weight_bags<scalar_t>/* SYCL: submit kernel with nd_range({grid=grid, block=block, smem=0, stream=stream}) */(
                  orig_indices.const_data_ptr<index_t>(),
                  grad.const_data_ptr<scalar_t>(),
                ...
                compute_grad_weight<scalar_t>/* SYCL: submit kernel with nd_range({grid=grid, block=block, smem=0, stream=stream}) */(
                  orig_indices.const_data_ptr<index_t>(),
                  grad.const_data_ptr<scalar_t>(),
                  count.defined() ? count.const_data_ptr<index_t>() : nullptr,
                  numel, stride,
          ...
          const int grid2 = ceil_div(max_segments*stride_warped, block);
              sum_and_scatter<scalar_t>/* SYCL: submit kernel with nd_range({grid=grid2, block=block, smem=0, stream=stream}) */(
                sorted_indices.const_data_ptr<index_t>(),
                grad_weight.mutable_data_ptr<scalar_t>(),
                stride,
                segment_offsets.const_data_ptr<index_t>(),
