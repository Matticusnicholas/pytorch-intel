...
// Source: aten/src/ATen/native/cuda/Loss.cu
...
void binary_cross_entropy_backward_out_kernel(Tensor& grad_input, const Tensor& grad, const Tensor& input, const Tensor& target) {
  at::TensorIterator iter = TensorIteratorConfig()
      ...
      .add_input(target)
      .build();
  AT_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16, iter.common_dtype(), "binary_cross_entropy_backward_out_opencl", [&]() {
    at::native::sycl_kernel(iter, [] SYCL_LAMBDA (
        scalar_t grad_val,
        scalar_t input_val,
        scalar_t target_val
      ) -> scalar_t {
...
Tensor binary_cross_entropy_cuda(const Tensor& input, const Tensor& target, const std::optional<Tensor>& weight_opt, int64_t reduction) {
    ...
    return at::native::binary_cross_entropy_out_opencl(
        input, target, weight_opt, reduction, loss);
}

Tensor& binary_cross_entropy_out_opencl(const Tensor& input, const Tensor& target, const std::optional<Tensor>& weight_opt, int64_t reduction, Tensor& loss) {
  ...
  TensorIterator iter = TensorIteratorConfig()
      ...
      .build();
  AT_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16, iter.common_dtype(), "binary_cross_entropy_out_opencl", [&]() {
    sycl_kernel(iter,
      [] SYCL_LAMBDA (scalar_t input_val, scalar_t target_val) -> scalar_t {
...
Tensor binary_cross_entropy_backward_opencl(const Tensor& grad, const Tensor& input, const Tensor& target, const std::optional<Tensor>& weight_opt, int64_t reduction) {
  Tensor grad_input = at::empty_like(input);
  return at::native::binary_cross_entropy_backward_out_opencl(
      grad, input, target, weight_opt, reduction, grad_input);
}

Tensor& binary_cross_entropy_backward_out_opencl(const Tensor& grad, const Tensor& input, const Tensor& target, const std::optional<Tensor>& weight_opt, int64_t reduction, Tensor& grad_input) {
  // See [Note: hacky wrapper removal for optional tensor]
  c10::MaybeOwned<Tensor> weight_maybe_owned = at::borrow_from_optional_tensor(weight_opt);
  const Tensor& weight = *weight_maybe_owned;
...
/* SYCL kernel */ void nll_loss_forward_no_reduce_opencl_kernel(
    int64_t batch_size,
  ...
  for (int64_t index = item.get_global_id(0); index < batch_size; index += item.get_global_range(0)) {
...
template <typename scalar_t, typename index_t>
/* SYCL kernel */ void nll_loss_forward_reduce_cuda_kernel_1d(
    scalar_t* output,
    ...
    int64_t n_classes,
    int64_t ignore_index) {
  SYCL_KERNEL_ASSERT(item.get_local_id(0) == 0 && item.get_local_id(1) == 0 && item.get_local_id(2) == 0);
...
/* SYCL kernel */ void nll_loss_forward_reduce_cuda_kernel_2d(
    scalar_t* output,
    scalar_t* total_weight,
    const scalar_t* input,
    const index_t* target,
  ...
  /* SYCL: use sycl::local_accessor for dynamic local memory */ unsigned char shmem[];
  accscalar_t* sh_inputs = reinterpret_cast<accscalar_t*>(shmem);
  accscalar_t* acc_weight = reinterpret_cast<accscalar_t*>(shmem + item.get_local_range(0) * sizeof(accscalar_t));

  sh_inputs[item.get_local_id(0)] = static_cast<accscalar_t>(0);
  acc_weight[item.get_local_id(0)] = static_cast<accscalar_t>(0);
  for (int i = item.get_local_id(0); i < nframe; i += item.get_local_range(0)) {
    index_t t = target[i];
    if (t != ignore_index) {
      CHECK_INDEX_IN_CLASS(t, n_classes);
      scalar_t cur_weight =
          weights != nullptr ? weights[t] : static_cast<scalar_t>(1);
      sh_inputs[item.get_local_id(0)] -= input[i * ndim + t] * cur_weight;
      acc_weight[item.get_local_id(0)] += cur_weight;
    }
  }

  item.barrier(sycl::access::fence_space::local_space);

  for (int stride = item.get_local_range(0)/2; stride > 0; stride >>= 1) {
    if (item.get_local_id(0) < stride) {
      sh_inputs[item.get_local_id(0)] += sh_inputs[item.get_local_id(0) + stride];
      acc_weight[item.get_local_id(0)] += acc_weight[item.get_local_id(0) + stride];
    }
    item.barrier(sycl::access::fence_space::local_space);
  }
...
void nll_loss_forward_out_opencl_template(
    const Tensor& output,
  ...
  if (reduction == Reduction::None && n_dims == 2) {
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(
        ...
        input.scalar_type(),
        "nll_loss_forward_no_reduce_opencl_kernel",
        [&] {
          AT_DISPATCH_NLL_LOSS_INDEX_TYPES(
              target.scalar_type(),
              "nll_loss_forward_no_reduce_cuda_kernel_index",
              [&] {
                nll_loss_forward_no_reduce_opencl_kernel<scalar_t, index_t>
                    /* SYCL: kernel launch with nd_range(at::sycl::detail::SYCL_GET_BLOCKS(batch_size),
                       at::sycl::detail::SYCL_NUM_THREADS,
                       0,
          ...
          AT_DISPATCH_NLL_LOSS_INDEX_TYPES(
              ...
              [&] {
                ...
                nll_loss_forward_reduce_cuda_kernel_2d<scalar_t, accscalar_t, index_t>
                    /* SYCL: kernel launch with nd_range(1,
                       nthreads,
...
template <typename scalar_t, typename index_t>
/* SYCL kernel */ void nll_loss_backward_no_reduce_opencl_kernel(
  int batch_size,
  const index_t *target,
...
template <typename scalar_t, typename index_t>
/* SYCL kernel */ void nll_loss_backward_reduce_cuda_kernel_1d(
  scalar_t *grad_input,
...
template <typename scalar_t, typename index_t>
/* SYCL kernel */ void nll_loss_backward_reduce_cuda_kernel_2d(
    scalar_t* grad_input,
    const scalar_t* grad_output,
    const index_t* target,
    const scalar_t* weights,
...
void nll_loss_backward_out_opencl_template(
  ...
  if (reduction == at::Reduction::None && n_dims == 2) {
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(
        ...
        at::ScalarType::BFloat16,
        input.scalar_type(),
        "nll_loss_backward_no_reduce_opencl_kernel",
        [&] {
          AT_DISPATCH_NLL_LOSS_INDEX_TYPES(
              target.scalar_type(),
              "nll_loss_backward_no_reduce_cuda_kernel_index",
              [&] {
                nll_loss_backward_no_reduce_opencl_kernel<scalar_t, index_t>
                    /* SYCL: kernel launch with nd_range(at::sycl::detail::SYCL_GET_BLOCKS(batch_size),
                       at::sycl::detail::SYCL_NUM_THREADS,
                       0,
                       at::sycl::getCurrentSYCLStream()) */(
        ...
        [&] {
          AT_DISPATCH_NLL_LOSS_INDEX_TYPES(
            ...
            nll_loss_backward_reduce_cuda_kernel_2d<scalar_t, index_t>
                /* SYCL: kernel launch with nd_range(1, nll_loss_threads(input.size(0)), 0, at::sycl::getCurrentSYCLStream()) */(
                    grad_input.mutable_data_ptr<scalar_t>(),
                    grad_output.const_data_ptr<scalar_t>(),
                    target.const_data_ptr<index_t>(),
                    weight.defined() ? weight_.const_data_ptr<scalar_t>() : nullptr,
...
TORCH_IMPL_FUNC(nll_loss_forward_out_opencl)
(const Tensor& self,
 const Tensor& target,
 ...
 const Tensor& total_weight) {
  ...
  nll_loss_forward_out_opencl_template(
      output, total_weight, self, target, weight, reduction, ignore_index);
}

TORCH_IMPL_FUNC(nll_loss_backward_out_opencl)
(const Tensor& grad_output,
 ...
 const Tensor& grad_input) {
  const Tensor& weight = weight_opt.getTensorRef();
  grad_input.zero_();
  nll_loss_backward_out_opencl_template(
      grad_input,
      grad_output,
      self,
      target,
