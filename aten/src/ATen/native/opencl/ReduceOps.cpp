...
// Source: aten/src/ATen/native/cuda/ReduceOps.cpp

#define TORCH_ASSERT_ONLY_METHOD_OPERATORS
#include <ATen/native/opencl/ReduceOps.h>
...
void norm_kernel_opencl(TensorIterator& iter, const Scalar& val) {
  double p = 0;
  if (val.isIntegral(false)) {
    ...
    TORCH_CHECK(false, "norm_kernel_opencl_impl expects norm to be integer or float");
  }
  if (iter.numel() == 0) {
...
void powsum_kernel_opencl(TensorIterator& iter, const Scalar& val) {
  double p = 0;
  if (val.isIntegral(false)) {
    p = static_cast<double>(val.to<int64_t>());
  } else if (val.isFloatingPoint()) {
    ...
    TORCH_CHECK(false, "powsum_kernel_opencl expects ord to be integer or float");
  }
  if (iter.numel() == 0) {
    iter.output().fill_(0);
    return;
...
void aminmax_kernel_impl(
  ...
  at::TensorIterator iter = make_reduction("aminmax_opencl", min_result,
...
void aminmax_allreduce_kernel_impl(const Tensor& input, Tensor& min_result, Tensor& max_result) {
  auto dtype = input.scalar_type();
  auto iter = make_reduction("aminmax_opencl", min_result, max_result, input,
...
REGISTER_OPENCL_DISPATCH(min_stub, &min_kernel_impl)
REGISTER_OPENCL_DISPATCH(max_stub, &max_kernel_impl)
REGISTER_OPENCL_DISPATCH(min_all_stub, &min_all_kernel_impl)
REGISTER_OPENCL_DISPATCH(max_all_stub, &max_all_kernel_impl)
REGISTER_OPENCL_DISPATCH(aminmax_allreduce_stub, &aminmax_allreduce_kernel_impl)
REGISTER_OPENCL_DISPATCH(aminmax_stub, &aminmax_kernel_impl)

REGISTER_OPENCL_DISPATCH(norm_stub, &norm_kernel_opencl)
REGISTER_OPENCL_DISPATCH(powsum_stub, &powsum_kernel_opencl)
