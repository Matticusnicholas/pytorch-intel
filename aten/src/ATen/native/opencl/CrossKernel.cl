...
// Source: aten/src/ATen/native/cuda/CrossKernel.cu
...
/* SYCL kernel */ void cross_kernel(
  ...
  for (int i = item.get_global_id(0); i < numel; i += item.get_global_range(0)) {
...
void launch_cross_kernel(const TensorIteratorBase& iter, int64_t ostride,
  ...
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND2(kHalf, kBFloat16, iter.common_dtype(), "cross_opencl", [&] {
    ...
    if (ostride * 2 > int_max || x1stride * 2 > int_max || x2stride * 2 > int_max) {
      cross_kernel/* SYCL: submit kernel with nd_range({grid=grid, block=num_threads(), smem=0, stream=stream}) */(
