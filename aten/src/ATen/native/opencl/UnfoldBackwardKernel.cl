...
static void _launch_unfold_backward_kernel(int total_n_elems, func_t f) {
  ...
  _unfold_backward_elementwise_kernel<n_threads, n_elems_per_thread, func_t>
    /* SYCL: launch with nd_range(grid, block, 0, stream) */(total_n_elems, f);
...
void unfold_backward_opencl_kernel(
  Tensor& grad_out,
...
REGISTER_DISPATCH(unfold_backward_stub, &unfold_backward_opencl_kernel)
