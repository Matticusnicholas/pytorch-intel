...
// Source: aten/src/ATen/native/cuda/FractionalMaxPool2d.cu
...
inline int get_interval(accscalar_t sample,
...
/* SYCL kernel */ void fractional_max_pool2d_out_cuda_frame(
  ...
  int ourOutputPoint = item.get_local_id(0) + item.get_group(0) * item.get_local_range(0);
...
template <typename scalar_t>
/* SYCL kernel */ void fractional_max_pool2d_backward_out_cuda_frame(
...
TORCH_IMPL_FUNC(fractional_max_pool2d_out_cuda) (
  ...
  int outputPlaneSize = output_.size(2) *
    output_.size(3);
  sycl::range<3> grid((outputPlaneSize + 127) / 128, // ceil(outputPlaneSize / 128)
            input_.size(1),
            input_.size(0));
  ...
  const Tensor& gradInput)
{
  ...
  globalContext().alertNotDeterministic("fractional_max_pool2d_backward_opencl");
