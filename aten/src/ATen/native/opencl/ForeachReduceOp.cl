...
// Source: aten/src/ATen/native/cuda/ForeachReduceOp.cu
// SYCL TODO: needs_review - complex file (668 lines), kernel launch patterns need manual verification
...
struct LpMaxFunctor {
  inline void operator()(
    ...
    const auto tensor_loc = tl.block_to_tensor[item.get_group(0)];
    const auto chunk_idx = tl.block_to_chunk[item.get_group(0)];
    ...
    /* SYCL: use sycl::local_accessor */ T s_vals[512];
    T vals[kILP];
    T r_x[kILP];
    ...
    if (n % kILP == 0 && (chunk_size & kILP) == 0 && is_aligned(x)) {
      for (int64_t i_start = item.get_local_id(0);
           i_start * kILP < n && i_start * kILP < chunk_size;
           i_start += item.get_local_range(0)) {
        // load
...
#pragma unroll
      ...
      for (int64_t i_start = 0; i_start < n && i_start < chunk_size;
           i_start += item.get_local_range(0) * kILP) {
#pragma unroll
        for (int ii = 0; ii < kILP; ii++) {
          int i = i_start + item.get_local_id(0) + ii * item.get_local_range(0);
          if (i < n && i < chunk_size) {
...
template <typename T>
/* SYCL kernel */ void lpmax_cleanup(
    const T* output_per_tensor,
    TensorListAddresses addr_struct,
    int max_chunks_per_tensor) {
  /* SYCL: use sycl::local_accessor */ T vals[512];
  const T* output_this_tensor =
      output_per_tensor + item.get_group(0) * max_chunks_per_tensor;
  T val = std::numeric_limits<T>::lowest();
  for (size_t i = item.get_local_id(0); i < max_chunks_per_tensor; i += item.get_local_range(0)) {
    val = max_propagate_nan(val, output_this_tensor[i]);
  }
  T final_val = at::native::cuda_utils::BlockReduceMax(val, vals);
  if (item.get_local_id(0) == 0) {
    *(T*)addr_struct.addresses[item.get_group(0)] = final_val;
  }
}
  ...
  AT_DISPATCH_ALL_TYPES_AND3(
      ...
      [&]() {
        ...
        for (const auto i : c10::irange(num_kernels)) {
          ...
          lpmax_cleanup<scalar_t>/* SYCL: submit kernel with nd_range({grid=num_tensors_this_kernel, block=512, smem=0, stream=stream}) */(
...
struct LpNormFunctor {
  ...
  inline void operator()(
    ...
    /* SYCL: use sycl::local_accessor */ out_opmath_t s_vals[512];
...
#pragma unroll
    ...
    auto final_val = norm_type == NormType::L1 || norm_type == NormType::L2
        ? at::native::sycl_utils::BlockReduceSum(val, s_vals)
...
/* SYCL kernel */ void lpnorm_cleanup(
    const out_opmath_t* output_per_tensor,
    TensorListAddresses addr_struct,
    int max_chunks_per_tensor) {
  /* SYCL: use sycl::local_accessor */ out_opmath_t vals[512];
  ...
  out_opmath_t final_val =
      ...
      ? at::native::sycl_utils::BlockReduceSum<out_opmath_t>(val, vals)
  ...
  if (item.get_local_id(0) == 0) {
    ...
    *(out_t*)addr_struct.addresses[item.get_group(0)] = result;
  }
}
...
struct ForeachNormDispatchName<true> {
  static constexpr const char* value = "foreach_tensor_norm_opencl";
};

template <>
struct ForeachNormDispatchName<false> {
  static constexpr const char* value = "foreach_tensor_powsum_opencl";
};
              ...
              for (const auto i : c10::irange(num_kernels)) {
                ...
                if (p == static_cast<double>(1)) {
                  lpnorm_cleanup<scalar_t, NormType::L1, out_t, apply_root>
                      /* SYCL: submit kernel with nd_range({grid=num_tensors_this_kernel, block=512, smem=0, stream=stream}) */(
                          output_per_tensor
...
std::vector<Tensor> foreach_tensor_norm_cuda(
  ...
  check_foreach_norm_dtype(
      dtype, tensors[0].scalar_type(), "_foreach_tensor_norm_opencl");
...
std::vector<Tensor> foreach_tensor_powsum_cuda(
  ...
  check_foreach_norm_dtype(
      dtype, tensors[0].scalar_type(), "_foreach_tensor_powsum_opencl");
