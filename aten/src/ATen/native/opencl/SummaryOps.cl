...
#define FOR_KERNEL_LOOP(i, lim)                                      \
  for (IndexType i = item.get_group(0) * item.get_local_range(0) + item.get_local_id(0); i < lim; \
       i += item.get_group_range(0) * item.get_local_range(0))

/*
  Memory types used for the 3 histogram implementations.
  See `SYCL_tensor_histogram` below.
 */
enum class SYCLHistogramMemoryType { SHARED, GLOBAL };
namespace {
template <typename input_t, typename IndexType>
static IndexType getBin(
    input_t bVal,
    at::acc_type<input_t, /*is_sycl=*/true> minvalue,
    at::acc_type<input_t, /*is_sycl=*/true> maxvalue,
    int64_t nbins) {
...
template <
    ...
    int PDims,
    int BDims,
    SYCLHistogramMemoryType MemoryType,
    typename Op>
...
void kernelHistogram1D(
    ...
    Op getOp) {
  // SYCL: use sycl::local_accessor for dynamic shared memory unsigned char my_smem[];
  output_t* smem = nullptr;

  if (MemoryType == SYCLHistogramMemoryType::SHARED) {
    ////////////////////////// Shared memory //////////////////////////
    ...
    for (IndexType i = item.get_local_id(0); i < a.sizes[0]; i += item.get_local_range(0)) {
      smem[i] = 0;
    }
...
#define HANDLE_CASE(MEMORY_TYPE, WEIGHTS_OP, SHARED_MEM)                 \
  kernelHistogram1D<                                                     \
      ...
      2,                                                                 \
      -1,                                                                \
      MEMORY_TYPE>/* SYCL: launch with nd_range(grid, block, SHARED_MEM, getCurrentSYCLStream()) */( \
      aInfo,                                                             \
      pInfo,                                                             \
      bInfo,                                                             \
      nbins,                                                             \
...
#define HANDLE_SWITCH_CASE(mType, getOp)                                   \
  switch (mType) {                                                         \
    case SYCLHistogramMemoryType::SHARED:                                  \
      HANDLE_CASE(SYCLHistogramMemoryType::SHARED, getOp, sharedMem);      \
      break;                                                               \
    default:                                                               \
      HANDLE_CASE(SYCLHistogramMemoryType::GLOBAL, getOp, 0);              \
  }
...
bool SYCL_tensor_histogram(
  ...
  checkBackend("SYCL_tensor_histogram", {a, b}, Backend::CUDA);
  if (HasWeights) {
    checkBackend("SYCL_tensor_histogram", {c}, Backend::CUDA);
  }
  ...
  const sycl::range<3> block = getApplyBlock();
  sycl::range<3> grid;
  ...
  SYCLHistogramMemoryType memType = SYCLHistogramMemoryType::GLOBAL;
  ...
  if (sharedMem < maxSharedMem) {
    // Solve equations:
    // (1) #(smem sycl::atomic_ref_add /* sycl::atomic_ref */ per SM) = totalElements / min(grid.x, #SM)
    // (2) #(gmem sycl::atomic_ref_add /* sycl::atomic_ref */) = grid.x * nbins
    // (3) RATIO_OF_GMEM_ATOMIC_ADD_TO_SMEM_ATOMIC_ADD = #(gmem sycl::atomic_ref_add /* sycl::atomic_ref */) / #(smem sycl::atomic_ref_add /* sycl::atomic_ref */ per SM)
    ...
    memType = SYCLHistogramMemoryType::SHARED;
  }
...
Tensor _bincount_opencl_template(
    const Tensor& self,
  ...
  using bounds_t = at::acc_type<input_t, /*is_sycl=*/true>;
  ...
  if (has_weights) {
    ...
    cuda::SYCL_tensor_histogram<weights_t, input_t, true>(
    ...
    output = at::zeros(
        ...
        DeviceType::CUDA,
        std::nullopt /* pin_memory */);
    cuda::SYCL_tensor_histogram<int64_t, input_t, false>(
        output, self, weights, nbins, minvalue, maxvalue);
  }
  return output;
}
...
Tensor _histc_opencl_template(
    const Tensor& self,
    int64_t nbins,
    at::acc_type<input_t, /*is_sycl=*/true> min,
    at::acc_type<input_t, /*is_sycl=*/true> max) {
  if (nbins <= 0) {
...
#define STL_CAST_BUG(value) value
  ...
  cuda::SYCL_tensor_histogram<input_t, input_t, false>(
...
Tensor _bincount_opencl(
  ...
  if (weights_opt.has_value()) {
    ...
    // sycl::atomic_ref_add /* sycl::atomic_ref */ usage
    globalContext().alertNotDeterministic("_bincount_opencl");
  }
  return AT_DISPATCH_INTEGRAL_TYPES(self.scalar_type(), "bincount_opencl", [&] {
    const auto scalar = weights.scalar_type();
    if (scalar == ScalarType::Undefined || scalar == ScalarType::Float)
      return _bincount_opencl_template<scalar_t, float>(self, weights, minlength);
    return _bincount_opencl_template<scalar_t, double>(
        self, weights.to(kDouble), minlength);
  });
}

Tensor _histc_opencl(
    const Tensor& self,
    int64_t nbins,
    const Scalar& min,
    const Scalar& max) {
  ...
  // See Note [Writing Nondeterministic Operations]
  // Nondeterministic for floating types because of sycl::atomic_ref_add /* sycl::atomic_ref */ usage
  if (at::isFloatingType(self.scalar_type())){
    globalContext().alertNotDeterministic("_histc_opencl with floating point input");
  }
  return AT_DISPATCH_ALL_TYPES(self.scalar_type(), "histc", [&] {
    using bounds_t = at::acc_type<scalar_t, /*is_sycl=*/true>;
    return _histc_opencl_template<scalar_t>(
        self, nbins, min.to<bounds_t>(), max.to<bounds_t>());
  });
}

Tensor& _histc_out_opencl(const Tensor& self, int64_t bins, const Scalar& min, const Scalar& max, Tensor& result) {
  auto ret = _histc_opencl(self, bins, min, max);
