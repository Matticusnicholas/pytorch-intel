...
// Source: aten/src/ATen/native/cuda/Indexing.cu
...
// SYCL: CUB not available, using oneDPL or manual implementation
#include <ATen/sycl/oneDPL.h>
#include <ATen/sycl/detail/IntegerDivider.h>
...
/* SYCL kernel */ void indexing_backward_kernel_many_indices(
  ...
  /* SYCL: use sycl::local_accessor for dynamic local memory */ unsigned char smem[];
  auto smem_dups_cache = reinterpret_cast<int64_t*>(smem);

  int smem_offset = item.get_local_id(1) * SYCL_SUB_GROUP_SIZE;

  int laneIdx = item.get_local_id(0) % SYCL_SUB_GROUP_SIZE;
  int64_t grad_row = 0;

  for (int64_t z = item.get_group(2); z < outer_dim; z += item.get_group_range(2)) {
    // Init duplicates every time we compute a new set of entries:
    smem_dups_cache[smem_offset + laneIdx] = 0;
    sycl::group_barrier(item.get_sub_group());

    int64_t base_idx = item.get_group(0) * item.get_local_range(1) * SYCL_SUB_GROUP_SIZE + item.get_local_id(1) * SYCL_SUB_GROUP_SIZE;
    ...
    for (int subwarp = 0; subwarp < SYCL_SUB_GROUP_SIZE; subwarp++) {
      ...
      if (!accumulate) {
        ...
        int64_t feature_dim = item.get_group(1) * item.get_local_range(0) + item.get_local_id(0);
        while (feature_dim < stride) {
          grad_weight[new_weight_row + feature_dim] = grad_output[grad_row + feature_dim];
          feature_dim += item.get_group_range(1) * item.get_local_range(0);
        }
        continue;
      }
...
/* SYCL kernel */ void indexing_backward_kernel_stride_1(
  const int64_t* sorted_indices, const int64_t* indices, const scalar_t* grad_output, scalar_t* grad_weight,
  int64_t numel, int64_t stride, int64_t stride_before, int64_t outer_dim, bool accumulate) {
  using opmath_t = at::opmath_type<scalar_t>;
  ...
  for (int64_t z = item.get_group(2); z < outer_dim; z += item.get_group_range(2)) {
    ...
    if (idx < numel) {
      ...
      if (idx == 0 || crnt_sorted_idx != sorted_indices[idx - 1]) {
        ...
        if (num_duplicates < SYCL_SUB_GROUP_SIZE) {
    ...
    for (int subwarp = 0; subwarp < SYCL_SUB_GROUP_SIZE; subwarp++) {
      ...
      int64_t num_warp_passes = new_num_duplicates / SYCL_SUB_GROUP_SIZE;
      // Parallel reduction across the array of duplicates using all the lanes in the warp:
      for (int64_t i = 0; i < num_warp_passes; ++i) {
        grad_row = ((int64_t) indices[new_idx + i * SYCL_SUB_GROUP_SIZE + laneIdx]) * stride + z * numel * stride;
        gradient += static_cast<opmath_t>(grad_output[grad_row]) * scale;
      }

      // Reduce across the lanes of the warp:
      sycl::group_barrier(item.get_sub_group());
      for (int offset = SYCL_SUB_GROUP_SIZE / 2; offset > 0; offset /= 2) {
        gradient += sycl::shift_group_left(item.get_sub_group(), gradient, offset);
      }

      if (laneIdx == 0) {
        for (int64_t i = num_warp_passes * SYCL_SUB_GROUP_SIZE; i < new_num_duplicates; ++i) {
...
template <typename scalar_t, int SZ>
/* SYCL kernel */ void indexing_backward_kernel(
  const int64_t* sorted_indices, const int64_t* indices, const scalar_t* grad_output, scalar_t* grad_weight,
  int64_t numel, int64_t stride, int64_t stride_before, int64_t outer_dim, bool accumulate) {
...
//if indexing starts from the 0th dimension, stride_before does not matter because item.get_group(2) will be 0 in this case
//outer_dim is number of elements in the first unindexed dimensions
  ...
  // Number of values processed by each thread (grain size)
  for (int64_t z = item.get_group(2); z < outer_dim; z += item.get_group_range(2)){
    int64_t idx = item.get_group(0) * item.get_local_range(1) + item.get_local_id(1);
    if (idx < numel
        && (idx == 0 || sorted_indices[idx] != sorted_indices[idx - 1])){
      do {
        int64_t start_feature = item.get_local_id(0) + item.get_group(1) * item.get_local_range(0) * SZ;
        ...
        while (start_feature < stride) {
          ...
          for (int ii = 0; ii < SZ; ii++) {
            int64_t feature_dim = start_feature + ii * SYCL_SUB_GROUP_SIZE;
          ...
          start_feature += item.get_group_range(1) * item.get_local_range(0) * SZ;
        }
  ...
  for (int64_t z = item.get_group(2); z < outer_dim; z += item.get_group_range(2)){
        ...
        (idx == 0 || crnt_sorted_idx != sorted_indices[idx - 1]))
    {
      ...
      if (!accumulate) {
        ...
        int laneIdx = item.get_local_id(0) % SYCL_SUB_GROUP_SIZE;
        int64_t num_warp_passes = num_duplicates / SYCL_SUB_GROUP_SIZE;
        for (int64_t i = 0; i < num_warp_passes; ++i) {
            grad_row = ((int64_t) indices[idx + i * SYCL_SUB_GROUP_SIZE + laneIdx]) * stride + z * numel * stride;
            gradient += static_cast<opmath_t>(grad_output[grad_row]) * scale;
        }
        sycl::group_barrier(item.get_sub_group());
        for (int offset = SYCL_SUB_GROUP_SIZE / 2; offset > 0; offset /= 2) {
        ...
        if (laneIdx == 0) {
          for (int64_t i = num_warp_passes * SYCL_SUB_GROUP_SIZE; i < num_duplicates; ++i) {
            grad_row = ((int64_t) indices[idx + i]) * stride + z * numel * stride;
            gradient += static_cast<opmath_t>(grad_output[grad_row]) * scale;
          }
...
template <typename scalar_t>
/* SYCL kernel */ void indexing_backward_kernel_small_stride(
  const int64_t* sorted_indices, const int64_t* indices, const scalar_t* grad_output, scalar_t* grad_weight,
  int64_t numel, int64_t stride, int64_t stride_before, int64_t outer_dim, bool accumulate) {
  using opmath_t = at::opmath_type<scalar_t>;
  ...
  for (int64_t z = item.get_group(2); z < outer_dim; z += item.get_group_range(2)){
    int64_t idx = item.get_group(0) * item.get_local_range(1) + item.get_local_id(1);
    int64_t tidx = item.get_local_id(0);
    int64_t crnt_sorted_idx = sorted_indices[idx];
...
template <typename scalar_t, int SZ>
/* SYCL kernel */ void indexing_backward_kernel_quantized(
...
public:
  ...
  constexpr C10_DEVICE void operator() (scalar_t* self_data_start, int64_t index, int64_t numel, const scalar_t * src_data) const {
    ...
    sycl_atomic_mul(self_data_start + index, *src_data);
  }
};
...
public:
  ...
  constexpr C10_DEVICE void operator() (scalar_t* self_data_start, int64_t index, int64_t numel, const scalar_t * src_data) const {
    ...
    sycl_fast_atomic_add(self_data_start, index, numel, *src_data);
#else
    sycl_fast_atomic_add(self_data_start, index, numel, *src_data, true);
#endif
  }
};
...
public:
  ...
  constexpr C10_DEVICE void operator() (scalar_t* self_data_start, int64_t index, int64_t numel, const scalar_t * src_data) const {
    (void)numel; // suppress unused warning
    sycl_atomic_min(self_data_start + index, *src_data);
  }
};
...
public:
  ...
  constexpr C10_DEVICE void operator() (scalar_t* self_data_start, int64_t index, int64_t numel, const scalar_t * src_data) const {
    (void)numel; // suppress unused warning
    sycl_atomic_max(self_data_start + index, *src_data);
  }
};
...
void index_put_with_sort_kernel(Tensor & self, const c10::List<std::optional<Tensor>>& indices, const Tensor & value, bool accumulate, bool unsafe) {
  ...
  if (num_indices > 0 && sliceSize > 0) {
      ...
      int64_t nbits = sycl::onedpl::get_num_bits(largestIndex(self_) / sliceSize);
      sycl::onedpl::radix_sort_pairs(
      ...
      const int warp_size = at::sycl::sub_group_size();
      sycl::range<3> grid(ceil_div(num_indices, (int64_t) indices_per_block),
           std::min<int>(at::sycl::getCurrentDeviceProperties()->maxGridSize[1], ceil_div(sliceSize, (int64_t) (warp_size*UNROLL))),
           std::min(std::max<int>(1,nElemBefore), at::sycl::getCurrentDeviceProperties()->maxGridSize[2]));
      sycl::range<3> block(warp_size, indices_per_block);

#ifdef USE_ROCM
      sycl::range<3> new_grid_many_indices(ceil_div(num_indices, (int64_t) (indices_per_block * warp_size)),
      grid.y == 1 ? std::min<int>(at::sycl::getCurrentDeviceProperties()->maxGridSize[1], ceil_div(sliceSize, (int64_t) (warp_size))) : grid.y,
      grid.z);
      sycl::range<3> new_grid(ceil_div(num_indices, (int64_t) (indices_per_block * warp_size)), grid.y, grid.z);
...
#define KERNEL_SMEM 0
      ...
      if (sliceSize == 1) {
        ...
        AT_DISPATCH_V2(
          ...
          AT_WRAP([&] {
            indexing_backward_kernel_stride_1<scalar_t>/* SYCL: kernel launch with nd_range(KERNEL_GRID, block, KERNEL_SMEM, stream) */
            (
        ...
        if (sliceSize <= warp_size) {
          AT_DISPATCH_V2(
            ...
            AT_WRAP([&] {
              indexing_backward_kernel_small_stride<scalar_t>/* SYCL: kernel launch with nd_range(grid, block, 0, stream) */(
                sorted_indices.const_data_ptr<int64_t>(),
                orig_indices.const_data_ptr<int64_t>(),
                expandedValue.const_data_ptr<scalar_t>(),
                src_.mutable_data_ptr<scalar_t>(),
          ...
          if (num_indices >= 200000)
            AT_DISPATCH_V2(
              ...
              AT_WRAP([&] {
                indexing_backward_kernel_many_indices<scalar_t, UNROLL>/* SYCL: kernel launch with nd_range(new_grid_many_indices, block, smem_dups_size, stream) */(
                  sorted_indices.const_data_ptr<int64_t>(),
                  orig_indices.const_data_ptr<int64_t>(),
                  expandedValue.const_data_ptr<scalar_t>(),
                  src_.mutable_data_ptr<scalar_t>(),
          ...
          AT_DISPATCH_V2(
            ...
            "indexing_backward",
            AT_WRAP([&] {
              indexing_backward_kernel<scalar_t, UNROLL>/* SYCL: kernel launch with nd_range(grid, block, 0, stream) */(
                sorted_indices.const_data_ptr<int64_t>(),
                orig_indices.const_data_ptr<int64_t>(),
                expandedValue.const_data_ptr<scalar_t>(),
                src_.mutable_data_ptr<scalar_t>(),
...
REGISTER_OPENCL_DISPATCH(index_put_with_sort_stub, &index_put_with_sort_kernel)
...
void index_put_with_sort_quantized(Tensor & self, const c10::List<std::optional<Tensor>>& indices, const Tensor & value, double scale, int zero_point, bool unsafe) {
  ...
  if (num_indices > 0 && sliceSize > 0) {
      ...
      AT_DISPATCH_QINT_TYPES(
        ...
        indexing_backward_kernel_quantized<scalar_t, UNROLL>/* SYCL: kernel launch with nd_range(grid, block, 0, stream) */(
          sorted_indices.const_data_ptr<int64_t>(),
          orig_indices.const_data_ptr<int64_t>(),
...
REGISTER_OPENCL_DISPATCH(index_put_with_sort_quantized_stub, &index_put_with_sort_quantized)
} //anonymous
...
template <typename T, typename IndicesType, typename IndexType, int DstDim, int SrcDim, int IdxDim,
          typename func_t>
/* SYCL kernel */ void indexFuncSmallIndex(sycl::detail::TensorInfo<T, IndexType> dst,
                                    sycl::detail::TensorInfo<const T, IndexType> src,
                                    sycl::detail::TensorInfo<const IndicesType, IndexType> indices,
                                    int dstAddDim,
                                    int srcAddDim,
                                    IndexType innerSize,
  ...
  for (IndexType srcIndex = 0; srcIndex < indices.sizes[0]; ++srcIndex) {
    ...
    IndexType dstIndex =
        indices.data[sycl::detail::IndexToOffset<const IndicesType, IndexType, IdxDim>::get(srcIndex, indices)];
    SYCL_KERNEL_ASSERT(dstIndex < static_cast<IndexType>(dstAddDimSize));

    // We stride over the output ignoring the indexed dimension
    // (innerSize), whose offset calculation is handled differently
    for (IndexType linearIndex = item.get_group(0) * item.get_local_range(0) + item.get_local_id(0);
         linearIndex < innerSize;
         linearIndex += item.get_group_range(0) * item.get_local_range(0)) {
      IndexType dstOffset =
          sycl::detail::IndexToOffset<T, IndexType, DstDim>::get(linearIndex, dst);
      dstOffset += dstIndex * dst.strides[dstAddDim];

      IndexType srcOffset =
          sycl::detail::IndexToOffset<const T, IndexType, SrcDim>::get(linearIndex, src);
...
/* SYCL kernel */ void indexFuncLargeIndex(sycl::detail::TensorInfo<T, IndexType> dst,
                                    sycl::detail::TensorInfo<const T, IndexType> src,
                                    sycl::detail::TensorInfo<const IndicesType, IndexType> indices,
                                    int dstAddDim,
                                    int srcAddDim,
                                    ...
                                    const func_t& op,
                                    T alpha,
                                    sycl::detail::IntDivider<IndexType> innerSizeDivider) {
  ...
  auto processElement = [&](IndexType linearIndex) {
    ...
    IndexType dstOffset =
        sycl::detail::IndexToOffset<T, IndexType, DstDim>::get(elementInSlice, dst);
    dstOffset += dstIndex * dst.strides[dstAddDim];

    IndexType srcOffset =
        sycl::detail::IndexToOffset<const T, IndexType, SrcDim>::get(elementInSlice, src);
    srcOffset += srcIndex * src.strides[srcAddDim];
  ...
  if constexpr (VecSize > 1) {
    ...
    for (IndexType vecIdx = item.get_group(0) * item.get_local_range(0) + item.get_local_id(0);
         vecIdx < totalVecs;
         vecIdx += item.get_group_range(0) * item.get_local_range(0)) {
...
template <typename scalar_t>
bool indexShouldBeMajor(sycl::detail::TensorInfo<scalar_t, unsigned int> &info,
                                    int sliceDim)
{
...
void index_add_opencl_impl(const Tensor& self, int64_t dim, const Tensor& index, const Tensor& source, const Scalar& alpha, const Tensor& result) {
  ...
  const int mpc = at::sycl::getCurrentDeviceProperties()->multiProcessorCount;

#define SMALL_INDEX(TENSOR_TYPE, INDICES_TYPE, TYPE, SELF_DIM, SOURCE_DIM, IDX_DIM)     \
  indexFuncSmallIndex<TENSOR_TYPE, INDICES_TYPE, TYPE, SELF_DIM, SOURCE_DIM, IDX_DIM>   \
    /* SYCL: kernel launch with nd_range(smallIndexGrid, smallIndexBlock, 0, stream) */(                                   \
...
#define LARGE_INDEX(TENSOR_TYPE, INDICES_TYPE, TYPE,                        \
  ...
  {                                                                          \
    ...
    sycl::detail::IntDivider<TYPE> innerSizeDivider(innerSizeVal);           \
    indexFuncLargeIndex<TENSOR_TYPE, INDICES_TYPE, TYPE,                     \
                        SELF_DIM, SOURCE_DIM, IDX_DIM,                       \
                        IDX_IS_MAJOR, (IDX_IS_MAJOR) ? 4 : 1>               \
      /* SYCL: kernel launch with nd_range(largeIndexGrid, largeIndexBlock, 0, stream) */(                     \
        selfInfo, sourceInfo, indexInfo,                                     \
  ...
  const sycl::range<3> smallIndexGrid(std::min(ceil_div(sliceSize, (uint64_t)128), (uint64_t)(mpc * 8)));
  const sycl::range<3> smallIndexBlock(std::min(sliceSize, (uint64_t)128));

  const sycl::range<3> largeIndexGrid(std::min(ceil_div(sourceTotalSize, (uint64_t)128), (uint64_t)(mpc * 8)));
  //On ROCm, std::min -> ::min did not work as expected on when outTotalSize>=2147483648
  sycl::range<3> largeIndexBlock( (sourceTotalSize < defaultMaxBlockThreads) ? sourceTotalSize : defaultMaxBlockThreads );

  if (sycl::detail::canUse32BitIndexMath(result) &&
      sycl::detail::canUse32BitIndexMath(source) &&
      sycl::detail::canUse32BitIndexMath(index)) {
    AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND4(at::ScalarType::Bool, at::ScalarType::Half, at::ScalarType::BFloat16, at::ScalarType::ComplexHalf, result.scalar_type(), "index_add", [&] {
      sycl::detail::TensorInfo<scalar_t, unsigned int> selfInfo =
          sycl::detail::getTensorInfo<scalar_t, unsigned int>(self_);
      ...
      AT_DISPATCH_INDEX_TYPES(index.scalar_type(), "index_add_cuda_", [&] () {
        auto sourceInfo =
          sycl::detail::getTensorInfo<const scalar_t, unsigned int>(source_);
        const int sourceAddDim = sourceInfo.collapseDims(dim);
        sourceInfo.reduceDim(sourceAddDim);

        auto indexInfo =
        sycl::detail::getTensorInfo<const index_t, unsigned int>(index);
    ...
    AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND3(at::ScalarType::Bool, at::ScalarType::Half, at::ScalarType::BFloat16, self.scalar_type(), "index_add", [&] {
      sycl::detail::TensorInfo<scalar_t, uint64_t> selfInfo =
        sycl::detail::getTensorInfo<scalar_t, uint64_t>(self_);
      const int selfAddDim = selfInfo.collapseDims(dim);
      selfInfo.reduceDim(selfAddDim);
      const auto alpha_value = alpha.to<scalar_t>();

      sycl::detail::TensorInfo<const scalar_t, uint64_t> sourceInfo =
        sycl::detail::getTensorInfo<const scalar_t, uint64_t>(source_);
      const int sourceAddDim = sourceInfo.collapseDims(dim);
      sourceInfo.reduceDim(sourceAddDim);

      AT_DISPATCH_INDEX_TYPES(index.scalar_type(), "index_add_cuda_", [&] () {
        sycl::detail::TensorInfo<const index_t, uint64_t> indexInfo =
          sycl::detail::getTensorInfo<const index_t, uint64_t>(index);
        indexInfo.collapseDims();
...
void index_reduce_func_opencl_impl(
  const Tensor& self,
  int64_t dim,
  const Tensor& index,
  const Tensor& source,
  ...
  const Tensor& result) {
  globalContext().alertNotDeterministic("index_reduce_opencl");
  ...
  int mpc = at::sycl::getCurrentDeviceProperties()->multiProcessorCount;
...
#define LARGE_INDEX(TENSOR_TYPE, INDICES_TYPE, TYPE,                                     \
  ...
  uint64_t defaultMaxBlockThreads = getDefaultMaxThreadsPerBlock();
  sycl::range<3> smallIndexGrid(std::min(ceil_div(sliceSize, (uint64_t)128), (uint64_t)(mpc * 8)));
  sycl::range<3> smallIndexBlock(std::min(sliceSize, (uint64_t)128));

  sycl::range<3> largeIndexGrid(std::min(ceil_div(sourceTotalSize, (uint64_t)128), (uint64_t)(mpc * 8)));
  //On ROCm, std::min -> ::min did not work as expected on when outTotalSize>=2147483648
  sycl::range<3> largeIndexBlock( (sourceTotalSize < defaultMaxBlockThreads) ? sourceTotalSize : defaultMaxBlockThreads );
  ...
  if (sycl::detail::canUse32BitIndexMath(result) &&
    ...
    AT_DISPATCH_ALL_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16, result.scalar_type(), "index_reduce", [&] {
      ...
      AT_DISPATCH_INDEX_TYPES(index.scalar_type(), "index_reduce_opencl", [&] () {
        auto sourceInfo =
          sycl::detail::getTensorInfo<const scalar_t, unsigned int>(source_);
...
(const Tensor& self, int64_t dim, const Tensor& index, const Tensor& source, const Scalar& alpha, const Tensor& result) {
  index_add_opencl_impl(self, dim, index, source, alpha, result);
}
...
(const Tensor& self,
 ...
 const Tensor& result) {
  ...
  if (reduce == "prod") {
    index_reduce_func_opencl_impl(self, dim, index, source, include_self, ReductionType::PROD, reduce_multiply, result);
  } else if (reduce == "mean") {
    index_reduce_func_opencl_impl(self, dim, index, source, include_self, ReductionType::MEAN, reduce_add, result);
    ...
    index_reduce_func_opencl_impl(self, dim, index, source, include_self, ReductionType::MAX, reduce_maximum, result);
  } else if (reduce == "amin") {
    index_reduce_func_opencl_impl(self, dim, index, source, include_self, ReductionType::MIN, reduce_minimum, result);
  } else {
...
// parallelism.
template <typename T, typename IndicesType, typename IndexType, int DstDim, int SrcDim, int IdxDim>
/* SYCL kernel */ void indexSelectSmallIndex(sycl::detail::TensorInfo<T, IndexType> dst,
                                      sycl::detail::TensorInfo<const T, IndexType> src,
                                      sycl::detail::TensorInfo<const IndicesType, IndexType> indices,
                                      int dstSelectDim,
                                      int srcSelectDim,
  ...
  for (IndexType dstIndex = 0; dstIndex < indices.sizes[0]; ++dstIndex) {
    IndexType srcIndex =
      indices.data[sycl::detail::IndexToOffset<const IndicesType, IndexType, IdxDim>::get(dstIndex, indices)];
...
sycl::detail::TensorInfo<T, IndexType>
tensorInfoLegacyIfScalar(sycl::detail::TensorInfo<T, IndexType> ti) {
  if (ti.dims == 0) {
    ti.dims = 1;
    ti.sizes[0] = 1;
    ti.strides[0] = 1;
...
template <typename scalar_t>
void index_select_out_opencl_impl(
    Tensor& out,
    const Tensor& self,
    int64_t dim,
    const Tensor& index) {
...
#define SMALL_INDEX(TENSOR_TYPE, INDICES_TYPE, TYPE, DST_DIM, SRC_DIM, IDX_DIM)         \
  ...
  uint64_t defaultMaxBlockThreads = getDefaultMaxThreadsPerBlock();
  sycl::range<3> smallIndexGrid(std::min(ceil_div(sliceSize, defaultMaxBlockThreads), (uint64_t) (mpc * 8)));
  sycl::range<3> smallIndexBlock(std::min(sliceSize, defaultMaxBlockThreads));
  ...
  if (sycl::detail::canUse32BitIndexMath(out) &&
      sycl::detail::canUse32BitIndexMath(self) &&
      sycl::detail::canUse32BitIndexMath(index) &&
      numIndices <= 16
      ) {
    auto outInfo = tensorInfoLegacyIfScalar(sycl::detail::getTensorInfo<scalar_t, unsigned int>(out));
    int outSelectDim = outInfo.collapseDims(dim);
    outInfo.reduceDim(outSelectDim);

    auto  selfInfo = tensorInfoLegacyIfScalar(sycl::detail::getTensorInfo<const scalar_t, unsigned int>(self));
    int selfSelectDim = selfInfo.collapseDims(dim);
    selfInfo.reduceDim(selfSelectDim);

    AT_DISPATCH_INDEX_TYPES(index.scalar_type(), "index_select_out_opencl_impl", [&] () {
      auto indicesInfo = tensorInfoLegacyIfScalar(sycl::detail::getTensorInfo<const index_t, unsigned int>(index));
...
Tensor& index_select_out_opencl(
    const Tensor& self,
    int64_t dim,
    const Tensor& index,
    Tensor& out) {
  ...
  TORCH_CHECK(
      at::sycl::check_device({out, self, index}),
  ...
  if (self.is_quantized()) {
    ...
    AT_DISPATCH_QINT_TYPES(out.scalar_type(), "index_select_quant_opencl", [&] {
      index_select_out_opencl_impl<scalar_t>(out, self, dim, index);
    });
  } else {
    AT_DISPATCH_V2(
        out.scalar_type(),
...
Tensor index_select_cuda(const Tensor& self, int64_t dim, const Tensor& index) {
  ...
  at::native::index_select_out_opencl(self, dim, index, out);
  return out;
}
...
void masked_fill_kernel(TensorIterator& iter, const Scalar& value) {
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND4(
      kBool, kHalf, kBFloat16, kComplexHalf, iter.common_dtype(), "masked_fill_", [&]() {
        ...
        sycl_kernel(
            iter, [value_] SYCL_LAMBDA(scalar_t self, bool mask) -> scalar_t {
              if (mask) {
                return value_;
              }
              return self;
...
void cuda_masked_fill_kernel_quantized(TensorIterator& iter, scalar_t quantized_val) {
    sycl_kernel(
        iter, [quantized_val] SYCL_LAMBDA(scalar_t self, bool mask) -> scalar_t {
          if (mask) {
...
REGISTER_OPENCL_DISPATCH(masked_fill_kernel_quantized_stub, &masked_fill_kernel_quantized)
...
Tensor index_select_sparse_cuda(const Tensor& self, int64_t dim, const Tensor& index) {
  ...
  if (dim < sparse_dim) {
    ...
    const auto nneg_index = [&index, size]() -> Tensor {
      ...
      AT_DISPATCH_INDEX_TYPES(index.scalar_type(), "index_select_sparse_opencl", [&]() {
          sycl_kernel(iter, [size] SYCL_LAMBDA (index_t idx) -> index_t {
              SYCL_KERNEL_ASSERT(idx >= -size && idx < size
    ...
    std::tie(intrsc_counts_nneg_index, intrsc_first_match_nneg_index) = [&]() -> std::tuple<Tensor, Tensor> {
      ...
      AT_DISPATCH_INDEX_TYPES(nneg_index.scalar_type(), "index_select_sparse_opencl", [&]() {
          ...
          sycl_kernel(
              iter,
              [ptr_intrsc_counts_nneg_index, ptr_sorted_dim_indices, nnz] SYCL_LAMBDA (
                index_t idx_val, index_t idx_idx
              ) -> index_t {
                auto* lb = at::sycl::detail::find_bound<const index_t*, index_t, true>(
                  ptr_sorted_dim_indices,
                  ptr_sorted_dim_indices + nnz,
                  idx_val
                );
                auto* ub = at::sycl::detail::find_bound<const index_t*, index_t, false>(
                  ptr_sorted_dim_indices,
                  ptr_sorted_dim_indices + nnz,
                  idx_val
                );
    ...
    auto [selected_dim_indices, res_dim_indices] = [&]() -> std::tuple<Tensor, Tensor> {
      ...
      AT_DISPATCH_INDEX_TYPES(nneg_index.scalar_type(), "index_select_sparse_opencl", [&]() {
          ...
          sycl_kernel(
              iter,
              [ptr_res_dim_indices, ptr_selected_dim_indices, ptr_argsort_dim_indices] SYCL_LAMBDA (
