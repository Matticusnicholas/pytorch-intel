...
#include <ATen/native/sycl/TensorModeKernel.h>
#include <ATen/native/sycl/TensorModeKernel.h>
...
struct ModeImpl {
  std::tuple<scalar_t, int64_t> operator()(
      scalar_t *iter_begin,
      scalar_t *iter_end) {
    at::sycl::ThrustAllocator thrust_allocator;
    auto stream = at::sycl::getCurrentSYCLStream();
    auto policy = oneapi::dpl::cuda::par(thrust_allocator).on(stream);

    const auto n_element = iter_end - iter_begin;
    auto cuda_allocator = at::sycl::getSYCLDeviceAllocator();
    auto sort_buffer = c10::DeviceArray<int64_t>(*cuda_allocator, n_element);
    auto sort_buffer_ptr = oneapi::dpl::device_pointer_cast(sort_buffer.get());
    auto count_from_zero_iter = oneapi::dpl::make_counting_iterator(int64_t{0});
    oneapi::dpl::copy_n(policy, count_from_zero_iter, n_element, sort_buffer_ptr);


    // Sort the input data. The original indices of the data are stored in
    // sort_buffer_ptr
    oneapi::dpl::sort_by_key(policy, iter_begin, iter_end, sort_buffer_ptr);

    // Count # of unique elements via an inner product between adjacent elements.
    // Add 1 if two neighboring element are not equal.
    int unique = 1 +
        oneapi::dpl::inner_product(
                    policy,
                    iter_begin,
                    iter_end - 1,
                    iter_begin + 1,
                    0,
                    oneapi::dpl::plus<int>(),
                    oneapi::dpl::not_equal_to<scalar_t>());

    // Count frequency of each element
    auto keys = c10::DeviceArray<scalar_t>(*cuda_allocator, unique);
    auto counts = c10::DeviceArray<int64_t>(*cuda_allocator, unique);

    auto keys_ptr = oneapi::dpl::device_pointer_cast(keys.get());
    auto counts_ptr = oneapi::dpl::device_pointer_cast(counts.get());

    oneapi::dpl::reduce_by_key(
        policy,
        iter_begin,
        iter_end,
        oneapi::dpl::constant_iterator<int>(1),
        keys_ptr,
        counts_ptr);

    // Find index of maximum count
    auto it = oneapi::dpl::max_element(policy, counts_ptr, counts_ptr + unique);
    scalar_t mode = keys_ptr[it - counts_ptr];

    // Find first index within which it occurs
    auto position_iter = oneapi::dpl::find(policy, iter_begin, iter_end, mode);
...
struct ModeImpl<bool> {
  std::tuple<bool, int64_t> operator()(
    ...
    const auto num_true = oneapi::dpl::count_if(
        policy,
        first_bytes,
        last_bytes,
        [] SYCL_LAMBDA (uint8_t x) {
    ...
    // Find first index within which it occurs
    const auto position_iter = oneapi::dpl::find_if(
...
void calculate_mode(
  ...
  at::sycl::memcpy_and_sync(indices_data, &index, sizeof(int64_t), cudaMemcpyHostToDevice, stream);
}
...
void handle_fused_mode(
    sycl::range<3> grid,
    const TensorBase& self,
    sycl::detail::TensorInfo<scalar_t, unsigned int>& ti_values,
    sycl::detail::TensorInfo<int64_t, unsigned int>& ti_indices,
    int64_t slice_size,
    int64_t slices) {
  ...
  compute_mode<scalar_t, size>
      /* SYCL: launch with nd_range(grid, num_threads, memsize, at::sycl::getCurrentSYCLStream()) */(
...
void fused_mode(
  ...
  auto ti_values = sycl::detail::getTensorInfo<scalar_t, unsigned int>(values);
  auto ti_indices = sycl::detail::getTensorInfo<int64_t, unsigned int>(indices);
...
void launch_fused_mode_kernel(
  ...
  AT_DISPATCH_ALL_TYPES_AND3(kBool, kBFloat16, kHalf, self.scalar_type(), "opencl_mode", [&] {
