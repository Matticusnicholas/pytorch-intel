...
C10_LAUNCH_BOUNDS_1(1024)
// SYCL kernel
void upsample_bilinear2d_nhwc_out_frame(
    const accscalar_t rheight,
    const accscalar_t rwidth,
    const bool align_corners,
    const int channels,
  ...
  const int index = item.get_group(0) * item.get_local_range(0) + item.get_local_id(0);
...
#if 0 // SYCL: USE_ROCM not applicable
...
void upsample_bilinear2d_backward_out_frame(
...
#if 0 // SYCL: USE_ROCM not applicable
  for (size_t index = item.get_local_range(0) * item.get_group(0) + item.get_local_id(0); index < i_numel;
  ...
  for (size_t index = item.get_local_range(0) * item.get_group(0) + item.get_local_id(0); index < o_numel;
       index += item.get_local_range(0) * item.get_group_range(0)) {
    size_t index_temp = index;
...
C10_LAUNCH_BOUNDS_1(1024)
// SYCL kernel
void upsample_bilinear2d_backward_nhwc_out_frame(
    const int height1,
    const int width1,
    const int height2,
    const int width2,
...
static void upsample_bilinear2d_out_opencl_template(
    const Tensor& output,
    const Tensor& input,
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
    ...
    if (memory_format == at::MemoryFormat::ChannelsLast && channels >= 16 && \
      ...
      const int num_threads = std::min(
          at::sycl::getCurrentDeviceProperties()->maxThreadsPerBlock, 1024);
      ...
      upsample_bilinear2d_nhwc_out_frame<scalar_t, accscalar_t>
        /* SYCL: launch with nd_range(ceil_div(num_kernels, num_threads), num_threads, 0, at::sycl::getCurrentSYCLStream()) */(
      ...
      upsample_bilinear2d_out_frame<scalar_t, accscalar_t>
          /* SYCL: launch with nd_range(ceil_div(num_kernels, num_threads),
             num_threads,
             0,
             stream) */(
...
static void upsample_bilinear2d_backward_out_opencl_template(
  ...
  // element, and sycl::atomic_ref_add /* sycl::atomic_ref */ to input gradient. Given a sparse sampling case, our
...
// SYCL kernel
void upsample_gen2d_aa_out_frame(
  ...
  const int output_x = item.get_local_id(0) + item.get_group(0) * item.get_local_range(0);
  const int output_y = item.get_local_id(1) + item.get_group(1) * item.get_local_range(1);
  ...
  // SYCL: use sycl::local_accessor for dynamic shared memory int smem[];
  scalar_t* wx = reinterpret_cast<scalar_t*>(smem) + interp_width * item.get_local_id(0);
  scalar_t* wy = reinterpret_cast<scalar_t*>(smem) + interp_width * item.get_local_range(0) + interp_height * item.get_local_id(1);
  const int offset = interp_width * item.get_local_range(0) + interp_height * item.get_local_range(1);
  scalar_t *buffer2 = reinterpret_cast<scalar_t*>(smem) + offset + \
      interp_height * (item.get_local_id(0) + item.get_local_id(1) * item.get_local_range(0));
  ...
  {
    // All item.get_local_id(1) have the same wx weights
  ...
  {
    // All item.get_local_id(0) have the same wy weights
    upsample_antialias::_compute_weights<scalar_t, accscalar_t>(
        wy,
        height_scale,
        interp_height,
...
C10_LAUNCH_BOUNDS_1(256) // 256 performs better then 1024
// SYCL kernel
void upsample_gen2d_aa_backward_out_frame(
    const accscalar_t height_scale,
    const accscalar_t width_scale,
...
static void upsample_gen2d_aa_out_opencl_template(
    const Tensor& output,
    const Tensor& input_,
    IntArrayRef output_size,
    bool align_corners,
  ...
  checkAllSameGPU("upsample_gen2d_aa_out_opencl", {input_arg, output_arg});
  ...
  sycl::queue* stream = at::sycl::getCurrentSYCLStream();
  size_t sharedMemPerBlock = at::sycl::getCurrentDeviceProperties()->sharedMemPerBlock;
  int* maxThreadsDim = at::sycl::getCurrentDeviceProperties()->maxThreadsDim;
  int maxThreadsPerBlock = std::min(at::sycl::getCurrentDeviceProperties()->maxThreadsPerBlock, 256);
  int* maxGridSize = at::sycl::getCurrentDeviceProperties()->maxGridSize;
  int block_x = std::min<int>(maxThreadsDim[0], at::sycl::warp_size());
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
      ...
      input.scalar_type(), "upsample_bilinear2d_out_frame", [&] {
        ...
        const sycl::range<3> block(block_x, block_y);
        ...
        const sycl::range<3> grid(grid_x, grid_y, grid_z);
        ...
        upsample_gen2d_aa_out_frame<scalar_t, accscalar_t>
            /* SYCL: launch with nd_range(grid,
               block,
               shmem_size,
...
// InterpFilter as BicubicFilterFunctor <--> bicubic
template<typename InterpFilter>
static void upsample_gen2d_aa_backward_out_opencl_template(
    const Tensor& grad_input,
    const Tensor& grad_output_,
    IntArrayRef output_size,
    IntArrayRef input_size,
  ...
  // Inspired from UpSampleBicubic2d.cu::upsample_bicubic2d_backward_out_opencl_template
  TensorArg grad_input_arg{grad_input, "grad_input", 1},
      grad_output_arg{grad_output_, "grad_output_", 2};
  checkAllSameGPU(
      "upsample_gen2d_backward_out_opencl", {grad_output_arg, grad_input_arg});
  ...
  const int num_threads = std::min(at::sycl::getCurrentDeviceProperties()->maxThreadsPerBlock, 256);
  sycl::queue* stream = at::sycl::getCurrentSYCLStream();
...
TORCH_IMPL_FUNC(upsample_bilinear2d_out_opencl) (
    const Tensor& input,
    IntArrayRef output_size,
    bool align_corners,
    std::optional<double> scales_h,
    std::optional<double> scales_w,
    const Tensor& output) {
  upsample_bilinear2d_out_opencl_template(output, input, output_size, align_corners, scales_h, scales_w);
}

TORCH_IMPL_FUNC(upsample_bilinear2d_backward_out_opencl) (
  ...
  // Nondeterministic because of sycl::atomic_ref_add /* sycl::atomic_ref */ usage
  globalContext().alertNotDeterministic("upsample_bilinear2d_backward_out_opencl");
  upsample_bilinear2d_backward_out_opencl_template(
      grad_input, grad_output, output_size, input_size, align_corners, scales_h, scales_w);
}

TORCH_IMPL_FUNC(_upsample_bilinear2d_aa_out_opencl) (
    const Tensor& input,
    IntArrayRef output_size,
    bool align_corners,
    std::optional<double> scales_h,
  ...
  upsample_gen2d_aa_out_opencl_template<upsample_antialias::BilinearFilterFunctor>(
      output, input, output_size, align_corners, scales_h, scales_w);
}

TORCH_IMPL_FUNC(_upsample_bilinear2d_aa_backward_out_opencl) (
    const Tensor& grad_output,
    IntArrayRef output_size,
    IntArrayRef input_size,
    bool align_corners,
  ...
  // See Note [Writing Nondeterministic Operations]
  // Nondeterministic because of sycl::atomic_ref_add /* sycl::atomic_ref */ usage
  globalContext().alertNotDeterministic("upsample_bilinear2d_aa_backward_out_opencl");
  upsample_gen2d_aa_backward_out_opencl_template<upsample_antialias::BilinearFilterFunctor>(
      grad_input, grad_output, output_size, input_size, align_corners, scales_h, scales_w);
}
...
TORCH_IMPL_FUNC(_upsample_bicubic2d_aa_out_opencl) (
    const Tensor& input,
    IntArrayRef output_size,
    bool align_corners,
    std::optional<double> scales_h,
    std::optional<double> scales_w,
    const Tensor& output) {
  upsample_gen2d_aa_out_opencl_template<upsample_antialias::BicubicFilterFunctor>(
      output, input, output_size, align_corners, scales_h, scales_w);
}

TORCH_IMPL_FUNC(_upsample_bicubic2d_aa_backward_out_opencl) (
    const Tensor& grad_output,
    IntArrayRef output_size,
    IntArrayRef input_size,
    bool align_corners,
  ...
  // See Note [Writing Nondeterministic Operations]
  // Nondeterministic because of sycl::atomic_ref_add /* sycl::atomic_ref */ usage
  globalContext().alertNotDeterministic("upsample_bicubic2d_aa_backward_out_opencl");
  upsample_gen2d_aa_backward_out_opencl_template<upsample_antialias::BicubicFilterFunctor>(
      grad_input, grad_output, output_size, input_size, align_corners, scales_h, scales_w);
}
