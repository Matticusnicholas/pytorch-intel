...
// Source: aten/src/ATen/native/cuda/LinearAlgebra.cu
...
void addr_kernel_opencl(TensorIterator &iter, const Scalar& beta, const Scalar& alpha) {
...
/* SYCL kernel */ void _elementwise_kernel(int total_n_elems, func_t f) {
...
static void _launch_kernel(int total_n_elems, func_t f) {
  ...
  _elementwise_kernel<n_threads, n_elems_per_thread, func_t>
    /* SYCL: kernel launch with nd_range(grid, block, 0, stream) */(total_n_elems, f);
  // SYCL: kernel launch check handled by SYCL runtime;
}

void unpack_pivots_opencl_kernel(TensorIterator& iter, const int64_t dim_size, const int64_t max_pivot) {
  if (iter.numel() == 0) {
    return;
  }
  ...
  if (!iter.can_use_32bit_indexing()) {
    for (auto& sub_iter : iter.with_32bit_indexing()) {
      unpack_pivots_opencl_kernel(sub_iter, dim_size, max_pivot);
    }
    return;
  }
...
REGISTER_DISPATCH(unpack_pivots_stub, &unpack_pivots_opencl_kernel)
REGISTER_DISPATCH(addr_stub, &addr_kernel_opencl)
} // namespace at::native
