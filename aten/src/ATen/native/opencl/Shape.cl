...
inline bool getCatGrid(ptrdiff_t nTensors, dim3& grid) {
  const int numSM = at::sycl::getCurrentDeviceProperties()->multiProcessorCount;
  ...
  grid = sycl::range<3>( 2LL * numSM, (long long) nTensors );
...
inline std::tuple<dim3, dim3> getCatGridRocm(unsigned int max_elements_per_tensor,
  ...
  const unsigned int num_sm = at::sycl::getCurrentDeviceProperties()->multiProcessorCount;
  thread_blocks = std::min(num_sm * max_tb_per_sm, thread_blocks);

  sycl::range<3> block = sycl::range<3>(threads_per_block);
  sycl::range<3> grid = sycl::range<3>(thread_blocks, (long long)nTensors);
...
/**
  ...
  * grid-stride loop based off of the item.get_group(0), item.get_local_id(0) for each input to
...
void CatArrayBatchedCopy(
    ...
    IndexType tid = item.get_group(0) * item.get_local_range(0) + item.get_local_id(0);
    IndexType nElements = inputs.nElements[item.get_group(1)];
    TensorSizeStride<IndexType, CAT_ARRAY_MAX_INPUT_DIMS> ins = stride_size > 1 ? inputs.tensorStride[item.get_group(1)] : inputs.tensorStride[0];
    bool isContig = inputs.isContiguous[item.get_group(1)];

    if(tid >= nElements) return;

    const T* data = inputs.input[item.get_group(1)];
    IndexType offset = inputs.offset[item.get_group(1)];
    IndexType dimSize = inputs.dimSize[item.get_group(1)];
...
template <typename T, typename IndexType, int Dims, int batch_size, int stride_size>
// SYCL kernel
void CatArrayBatchedCopy_contig(
    T* output,
    CatArrInputTensorMetadata<T, IndexType, batch_size, stride_size> inputs,
    TensorSizeStride<IndexType, CAT_ARRAY_MAX_INPUT_DIMS> os,
    const int concatDim,
...
// SYCL kernel
void CatArrayBatchedCopy_vectorized(
    char* output,
    CatArrInputTensorMetadata<T, IndexType, batch_size, stride_size> inputs,
    TensorSizeStride<IndexType, CAT_ARRAY_MAX_INPUT_DIMS> os,
    const int concatDim,
    ...
    IndexType tid = item.get_group(0) * item.get_local_range(0) + item.get_local_id(0);
    IndexType nElements = inputs.nElements[item.get_group(1)] / elems_per_vec;

    if(tid >= nElements) return;

    const char * data = (char*)inputs.input[item.get_group(1)];
    IndexType offset = inputs.offset[item.get_group(1)] * trailingSize / elems_per_vec;
    IndexType dimSize = inputs.dimSize[item.get_group(1)] * trailingSize / elems_per_vec;
...
// SYCL kernel
void CatArrayBatchedCopy_alignedK_contig(
    T* output,
    CatArrInputTensorMetadata<T, IndexType, batch_size, stride_size> inputs,
    TensorSizeStride<IndexType, CAT_ARRAY_MAX_INPUT_DIMS> os,
    const int concatDim,
    ...
    IndexType inputOffset = (item.get_group(0) * item.get_local_range(0) + item.get_local_id(0)) * kILP;
    IndexType inputStride = item.get_group_range(0) * item.get_local_range(0) * kILP;
...
void parallel_cat(const Tensor &out, const MaterializedITensorListRef& inputs, int64_t dimension,
  ...
  at::sycl::CUDAStream stream = at::sycl::getCurrentSYCLStream();
...
#else
    sycl::range<3> applyBlock, catGrid;
    if (isInOutAligned) {
...
#define HANDLE_CASE(DIMS) \
    if (isInOutAligned) {\
      constexpr auto elems_per_vec = alignment / sizeof(scalar_t); \
      CatArrayBatchedCopy_vectorized<scalar_t, unsigned int, DIMS, batch_size, stride_size, alignment, elems_per_vec>/* SYCL: launch with nd_range(\
      ...
      CatArrayBatchedCopy_alignedK_contig<scalar_t, unsigned int, DIMS, batch_size, stride_size, ALIGNED_VEC_LOAD_BYTES_16>/* SYCL: launch with nd_range(\
          catGrid, applyBlock, 0, stream.stream()) */(\
              data, catMetaData, outputParam, cat_dim, outputParam.tensorStride[cat_dim]);\
    } else if (isContig && isAligned && sizeof(scalar_t) == 2) { \
      CatArrayBatchedCopy_alignedK_contig<scalar_t, unsigned int, DIMS, batch_size, stride_size, ALIGNED_VEC_LOAD_BYTES_8>/* SYCL: launch with nd_range(\
          catGrid, applyBlock, 0, stream.stream()) */(\
              data, catMetaData, outputParam, cat_dim, outputParam.tensorStride[cat_dim]);\
    } else if (isContig) {\
      CatArrayBatchedCopy_contig<scalar_t, unsigned int, DIMS, batch_size, stride_size>/* SYCL: launch with nd_range(\
          catGrid, applyBlock, 0, stream.stream()) */(\
              data, catMetaData, outputParam, cat_dim, outputParam.tensorStride[cat_dim]);\
    } else {\
      CatArrayBatchedCopy<scalar_t, unsigned int, DIMS, batch_size, stride_size>/* SYCL: launch with nd_range(\
          catGrid, applyBlock, 0, stream.stream()) */(\
              data, catMetaData, outputParam, cat_dim, outputParam.tensorStride[cat_dim]);\
    }\
...
TORCH_IMPL_FUNC(cat_out_opencl)
(const ITensorListRef& tensors,
 ...
 const Tensor& result) {
  ...
  const bool all32BitIndexable = std::all_of(materialized.begin(), materialized.end(),
    [] (const Tensor& t) {
      return at::sycl::detail::canUse32BitIndexMath(t);
    });
  ...
  if (materialized.size() > 1 &&
      ...
      at::sycl::detail::canUse32BitIndexMath(result) &&
      all_contiguous &&
      all32BitIndexable &&
      all_same_dtype) {
      if (isBitsType(result.scalar_type())) {
        AT_DISPATCH_BIT_TYPES(result.scalar_type(), "cat_opencl", [&]() {
