...
// Source: aten/src/ATen/native/cuda/ActivationGeluKernel.cu
...
void GeluSYCLKernelImpl(TensorIteratorBase& it, GeluType approximate) {
  if (approximate == GeluType::Tanh) {
    AT_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16, it.dtype(), "GeluSYCLKernelImpl", [&]() {
      sycl_kernel(it, [] SYCL_LAMBDA(scalar_t x) -> scalar_t {
        ...
        return opmath_t(0.5) * static_cast<opmath_t>(x) * (opmath_t(1) + sycl::tanh(inner));
      });
    });
  } else {
    AT_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16, it.dtype(), "GeluSYCLKernelImpl", [&]() {
...
void GeluBackwardSYCLKernelImpl(TensorIteratorBase& it, GeluType approximate) {
  if (approximate == GeluType::Tanh) {
    AT_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16,
        it.dtype(), "GeluBackwardSYCLKernelImpl", [&]() {
          sycl_kernel(it, [] SYCL_LAMBDA(scalar_t dy, scalar_t x) -> scalar_t {
            using opmath_t = at::opmath_type<scalar_t>;
            constexpr opmath_t kBeta = M_SQRT2 * M_2_SQRTPI * opmath_t(0.5);
            constexpr opmath_t kKappa = 0.044715;
            ...
            auto inner = kBeta * (static_cast<opmath_t>(x) + kKappa * x_cube);
            auto tanh_inner = sycl::tanh(inner);
