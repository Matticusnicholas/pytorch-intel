...
// Source: aten/src/ATen/native/cuda/LossCTC.cu
...
ctc_loss_log_alpha_gpu_kernel(scalar_t* __restrict__ log_alpha_data,
  ...
  // bookkeeping
  int64_t b = item.get_local_id(1) + item.get_group(1) * item.get_local_range(1);
  ...
  for (int64_t block_s = 0; block_s < 2*max_target_length+1; block_s += item.get_local_range(0)) {
    int64_t s = item.get_local_id(0) + block_s;
    scalar_t la;
    switch (s) {
    case 0:
  ...
  for (int64_t block_s = 0; block_s < 2*max_target_length+1; block_s += item.get_local_range(0)) {
    ...
    for (int64_t t=1; t < max_input_length; t++) {
      item.barrier(sycl::access::fence_space::local_space); // on cuda 9 we might use partial synchronization of only the threads within the same batch
...
std::tuple<Tensor, Tensor> ctc_loss_gpu_template(const Tensor& log_probs, const Tensor& targets, IntArrayRef input_lengths, IntArrayRef target_lengths, int64_t BLANK) {
  ...
  if (at::sycl::getCurrentDeviceProperties()->major >= 12) {
  ...
  sycl::range<3> block(threads_target, threads_batch);
  sycl::range<3> grid(1, (batch_size+threads_batch-1)/threads_batch);
  sycl::queue* stream = at::sycl::getCurrentSYCLStream();

  ctc_loss_log_alpha_gpu_kernel<scalar_t, target_t>/* SYCL: kernel launch with nd_range(grid, block, 0, stream) */(
...
ctc_loss_backward_log_beta_gpu_kernel(scalar_t* __restrict__ log_beta_data,
  ...
  for (int64_t block_s = 2*max_target_length - (2*max_target_length % item.get_local_range(0)); block_s >= 0; block_s -= item.get_local_range(0)) {
    int64_t s = item.get_local_id(0) + block_s;
    scalar_t lb;
  ...
  for (int64_t block_s = 2*max_target_length - (2*max_target_length % item.get_local_range(0)); block_s >= 0; block_s -= item.get_local_range(0)) {
    ...
    for (int64_t t=max_input_length-2; t>=0; t--) {
      item.barrier(sycl::access::fence_space::local_space); // on cuda 9 we might use partial synchronization of only the threads within the same batch item
...
// Internally this uses sycl_atomic_add because different threads may write to the same
// gradient position.
...
ctc_loss_backward_collect_nonblank_gpu_kernel(scalar_t* __restrict__ gradient_data,
  ...
  int64_t b = item.get_local_id(1) + item.get_group(1) * item.get_local_range(1);
  int64_t s = item.get_local_id(0) + ((int64_t) item.get_group(0)) * item.get_local_range(0); // note, this directly indexes into targets, not targets prime!
  ...
  for (int64_t t = 0; t < input_length; t++) {
    ...
    sycl_atomic_add_no_return(&gradient_data[gr_batch_offset + t * gr_input_stride + gr_char_stride * target],
...
ctc_loss_backward_collect_gpu_kernel(scalar_t* __restrict__ gradient_data,
  ...
  constexpr scalar_t neginf = -INFINITY;
  int64_t b = item.get_local_id(1) + item.get_group(1) * item.get_local_range(1);
  int64_t t = item.get_local_id(0) + ((int64_t) item.get_group(0)) * item.get_local_range(0);
...
Tensor ctc_loss_backward_gpu_template(const Tensor& grad_out, const Tensor& log_probs, const Tensor& targets, IntArrayRef input_lengths, IntArrayRef target_lengths,
  ...
  if (at::sycl::getCurrentDeviceProperties()->major >= 10) {
    max_threads = 512;
  }
  int threads_target = max_threads;
  while (threads_target / 2 >= 2*max_target_length+1) {
...

  {
    sycl::range<3> block(threads_target, threads_batch);
    sycl::range<3> grid(1, (batch_size+threads_batch-1)/threads_batch);
    ctc_loss_backward_log_beta_gpu_kernel<scalar_t, target_t>/* SYCL: kernel launch with nd_range(grid, block, 0, stream) */
  ...
  if (is_large) { // large alphabet, large batch
    ...
    sycl::range<3> grid(
        ...
        1);
    ctc_loss_backward_collect_nonblank_gpu_kernel<scalar_t, target_t>/* SYCL: kernel launch with nd_range(grid, block, 0, stream) */
    ...
    sycl::range<3> block(threads_input, threads_batch);
    sycl::range<3> grid((log_probs.size(0) + threads_input-1)/threads_input, (batch_size+threads_batch-1)/threads_batch);
    ctc_loss_backward_collect_gpu_kernel<scalar_t, target_t>/* SYCL: kernel launch with nd_range(grid, block, 0, stream) */
      (grad.mutable_data_ptr<scalar_t>(),
       grad_out.const_data_ptr<scalar_t>(), grad_out.stride(0),
       log_alpha.const_data_ptr<scalar_t>(), log_beta.const_data_ptr<scalar_t>(),
       log_probs.const_data_ptr<scalar_t>(), input_lengths_t.template const_data_ptr<int64_t>(), log_probs.size(0),
    ...
    // SYCL: kernel launch check handled by SYCL runtime; // catch launch errors
  }
  ...
  // zero those invalid graident elements due to padding
  {
    ...
    sycl::range<3> grid(
      ...
      (batch_size+threads_batch-1)/threads_batch);
    ctc_loss_zero_padded_gradients<scalar_t>/* SYCL: kernel launch with nd_range(grid, block, 0, stream) */(
      grad.mutable_data_ptr<scalar_t>(),
...
std::tuple<Tensor, Tensor> ctc_loss_gpu(const Tensor& log_probs, const Tensor& targets, IntArrayRef input_lengths, IntArrayRef target_lengths, int64_t BLANK, bool zero_infinity) {
  ...
  return AT_DISPATCH_FLOATING_TYPES(log_probs.scalar_type(), "ctc_loss_opencl", [&] {
...
Tensor ctc_loss_backward_gpu(const Tensor& grad, const Tensor& log_probs, const Tensor& targets, IntArrayRef input_lengths, IntArrayRef target_lengths,
  ...
  return AT_DISPATCH_FLOATING_TYPES(log_probs.scalar_type(), "ctc_loss_backward_opencl", [&] {
      if (targets.scalar_type() == kLong) {
