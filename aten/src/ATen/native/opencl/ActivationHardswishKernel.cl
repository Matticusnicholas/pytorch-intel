...
// Source: aten/src/ATen/native/cuda/ActivationHardswishKernel.cu
...
void hardswish_kernel(TensorIterator& iter) {
  AT_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16, iter.dtype(), "hardswish_opencl", [&]() {
    using opmath_t = at::opmath_type<scalar_t>;
    const opmath_t zero(0.0f);
    const opmath_t one_sixth(1.0f / 6.0f);
    const opmath_t three(3.0f);
    const opmath_t six(6.0f);
    sycl_kernel(iter, [zero, one_sixth, three, six] SYCL_LAMBDA(scalar_t self_val) -> scalar_t {
...
void hardswish_backward_kernel(TensorIterator& iter) {
  AT_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16, iter.dtype(), "hardswish_backward_opencl", [&]() {
    using opmath_t = at::opmath_type<scalar_t>;
    const opmath_t zero(0.0f);
    const opmath_t three(3.0f);
    ...
    sycl_kernel(
      iter,
      [zero, three, neg_three, one_half] SYCL_LAMBDA(scalar_t grad_val_, scalar_t self_val_) -> scalar_t {
