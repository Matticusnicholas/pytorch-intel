...
void where_kernel_impl(TensorIterator &iter) {
  AT_DISPATCH_V2(iter.dtype(), "where_opencl", [&] {
      sycl_kernel(
        iter,
        [=] SYCL_LAMBDA (bool cond_val, scalar_t self_val, scalar_t other_val) -> scalar_t {
...
void isposinf_kernel_impl(TensorIteratorBase &iter) {
  AT_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16, iter.input_dtype(), "isposinf_opencl", [&]() {
    sycl_kernel(
      iter,
      [] SYCL_LAMBDA (scalar_t a) -> bool { return a == std::numeric_limits<scalar_t>::infinity(); }
    );
  });
}

void isneginf_kernel_impl(TensorIteratorBase &iter) {
  AT_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16, iter.input_dtype(), "isneginf_opencl", [&]() {
    sycl_kernel(
      iter,
      [] SYCL_LAMBDA (scalar_t a) -> bool { return a == -std::numeric_limits<scalar_t>::infinity(); }
    );
...
void clamp_kernel_impl(TensorIteratorBase& iter) {
  AT_DISPATCH_ALL_TYPES_AND2(kHalf, kBFloat16, iter.common_dtype(), "clamp_opencl", [&] {
    sycl_kernel(iter, []SYCL_LAMBDA(scalar_t v, scalar_t lower, scalar_t upper) -> scalar_t {
...
void inline launch_clamp_scalar(TensorIteratorBase& iter, Scalar lim0, Scalar lim1, at::native::detail::ClampLimits minmax){
  AT_DISPATCH_ALL_TYPES_AND2(kHalf, kBFloat16, iter.common_dtype(), "clamp_scalar_opencl", [&] {
...
// SYCL kernel
void _assert_async_opencl_kernel(const scalar_t* input, Msg msg) {
  SYCL_KERNEL_ASSERT_MSG(input[0] != 0, msg.msg);
}

// SYCL kernel
void _assert_async_opencl_kernel(const c10::complex<float>* input, Msg msg) {
  SYCL_KERNEL_ASSERT_MSG(input[0] != c10::complex<float>(0, 0), msg.msg);
}
// SYCL kernel
void _assert_async_opencl_kernel(const c10::complex<double>* input, Msg msg) {
  SYCL_KERNEL_ASSERT_MSG(input[0] != c10::complex<double>(0, 0), msg.msg);
}

void _assert_async_msg_opencl(const Tensor& self_tensor, std::string_view assert_msg) {
  ...
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND3(at::ScalarType::Half, at::ScalarType::Bool, at::ScalarType::BFloat16, self.scalar_type(), "_assert_async_opencl", [&] {
    _assert_async_opencl_kernel/* SYCL: launch with nd_range(1, 1, 0, stream) */(self.const_data_ptr<scalar_t>(), msg);
    // SYCL: kernel launch check handled by SYCL runtime;
  });
}

void _assert_async_opencl(const Tensor& self_tensor) {
  _assert_async_msg_opencl(self_tensor, "");
}
