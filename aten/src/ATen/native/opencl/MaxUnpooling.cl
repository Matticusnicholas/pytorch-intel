...
// Source: aten/src/ATen/native/cuda/MaxUnpooling.cu
...
template <typename T>
/* SYCL kernel */ void max_unpooling2d_forward_kernel(
  ...
  for (int64_t linearIndex = item.get_global_id(0); linearIndex < numInputElements; linearIndex += item.get_global_range(0)) {
...
template <typename T>
/* SYCL kernel */ void max_unpooling3d_forward_kernel(
  ...
  int64_t iColumn = ((int64_t) item.get_group(0)) * item.get_local_range(0) + item.get_local_id(0);
  int64_t iRow = item.get_group(1) * item.get_local_range(1) + item.get_local_id(1);
  int64_t iFrame = (item.get_group(2) + offsetZ) % input.size(1); // input frame/time
  int64_t slice = (item.get_group(2) + offsetZ) / input.size(1); // input slice/feature
...
template <typename T>
/* SYCL kernel */ void max_unpooling2d_backward_kernel(
    const int64_t numInputElements,
    const T* input,
    const int64_t* indices,
    const int64_t numChannels,
...
template <typename T>
/* SYCL kernel */ void max_unpooling3d_backward_kernel(
    ...
    int offsetZ) {
  int iColumn = item.get_group(0) * item.get_local_range(0) + item.get_local_id(0);
  int iRow = item.get_group(1) * item.get_local_range(1) + item.get_local_id(1);
  int iFrame = (item.get_group(2) + offsetZ) % gradInput.size(1); // output frame/time
  int slice =
      (item.get_group(2) + offsetZ) / gradInput.size(1); // output slice/feature
...
Tensor& max_unpooling2d_forward_out_opencl(const Tensor& self_,
  ...
  checkAllSameGPU(
      "max_unpooling2d_forward_out_opencl", {output_arg, self_arg, indices_arg});

  for (int64_t i = 1; i < self_.ndimension(); ++i) {
    TORCH_CHECK(self_.size(i) > 0, "max_unpooling2d_forward_out_opencl(): ",
  ...
  if (count != 0 && oheight != 0 && owidth != 0) {
    AT_DISPATCH_ALL_TYPES_AND2(kHalf, kBFloat16,
        self.scalar_type(), "max_unpooling2d_forward_kernel", ([&] {
          max_unpooling2d_forward_kernel/* SYCL: kernel launch with nd_range(
              SYCL_GET_BLOCKS(count),
              SYCL_NUM_THREADS,
              0,
...
Tensor max_unpooling2d_forward_opencl(
    const Tensor& self,
    const Tensor& indices,
    IntArrayRef output_size) {
  auto output = at::empty({0}, self.options());
  at::native::max_unpooling2d_forward_out_opencl(self, indices, output_size, output);
  return output;
}
...
Tensor& max_unpooling3d_forward_out_opencl(const Tensor& self_,
    const Tensor& indices_,
    IntArrayRef output_size,
    IntArrayRef stride,
    IntArrayRef padding,
  ...
  max_unpooling3d_shape_check(
    self_, Tensor(), indices_, output_size, stride, padding, "max_unpooling3d_forward_out_opencl()");
  ...
  TensorArg output_arg{output, "output", 1}, self_arg{self_, "self_", 2},
      indices_arg{indices_, "indices_", 3};
  checkAllSameGPU(
      "max_unpooling3d_forward_out_opencl", {output_arg, self_arg, indices_arg});
  ...
  AT_DISPATCH_ALL_TYPES_AND2(kHalf, kBFloat16,
      self.scalar_type(), "max_unpooling3d_forward_kernel", ([&] {
        while (totalZ > 0) {
          ...
          max_unpooling3d_forward_kernel/* SYCL: kernel launch with nd_range(
              grid,
              block,
              0,
              at::sycl::getCurrentSYCLStream()) */(
...
Tensor max_unpooling3d_forward_opencl(
    const Tensor& self,
    const Tensor& indices,
    IntArrayRef output_size,
    IntArrayRef stride,
    IntArrayRef padding) {
  auto output = at::empty({0}, self.options());
  at::native::max_unpooling3d_forward_out_opencl(
      self, indices, output_size, stride, padding, output);
  return output;
}

at::Tensor& max_unpooling2d_backward_out_opencl(const Tensor& grad_output_,
    const Tensor& self_,
    const Tensor& indices_,
    IntArrayRef output_size,
    Tensor& grad_input) {
  ...
  checkAllSameGPU(
      "max_unpooling2d_backward_out_opencl",
  ...
  AT_DISPATCH_ALL_TYPES_AND2(kHalf, kBFloat16,
      self.scalar_type(), "max_unpooling2d_backward_kernel", ([&] {
        max_unpooling2d_backward_kernel/* SYCL: kernel launch with nd_range(
            SYCL_GET_BLOCKS(count),
            SYCL_NUM_THREADS,
            0,
            at::sycl::getCurrentSYCLStream()) */(
...
at::Tensor max_unpooling2d_backward_opencl(
    const Tensor& grad_output,
    const Tensor& self,
    const Tensor& indices,
    IntArrayRef output_size) {
  auto grad_input = at::empty_like(self, LEGACY_CONTIGUOUS_MEMORY_FORMAT);
  at::native::max_unpooling2d_backward_out_opencl(
      grad_output, self, indices, output_size, grad_input);
  return grad_input;
}

at::Tensor& max_unpooling3d_backward_out_opencl(const Tensor& grad_output_,
    const Tensor& self_,
    const Tensor& indices_,
    IntArrayRef output_size,
    IntArrayRef stride,
  ...
  max_unpooling3d_shape_check(
    self_, grad_output_, indices_, output_size, stride, padding, "max_unpooling3d_backward_out_opencl()");
  ...
  checkAllSameGPU(
      "max_unpooling3d_backward_out_opencl",
      ...
      self.scalar_type(), "max_unpooling3d_backward_kernel", ([&] {
        while (totalZ > 0) {
          sycl::range<3> grid(
              ...
              ceilDiv(inputHeight, static_cast<int64_t>(block.y)),
              totalZ > 65535 ? 65535 : totalZ);
          max_unpooling3d_backward_kernel/* SYCL: kernel launch with nd_range(
              grid,
              block,
              0,
              at::sycl::getCurrentSYCLStream()) */(
...
at::Tensor max_unpooling3d_backward_opencl(
    const Tensor& grad_output,
    const Tensor& self,
    const Tensor& indices,
    IntArrayRef output_size,
    ...
    IntArrayRef padding) {
  auto grad_input = at::empty_like(self, LEGACY_CONTIGUOUS_MEMORY_FORMAT);
  at::native::max_unpooling3d_backward_out_opencl(
