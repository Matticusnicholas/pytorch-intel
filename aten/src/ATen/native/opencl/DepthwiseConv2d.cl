...
// Source: aten/src/ATen/native/cuda/DepthwiseConv2d.cu
// SYCL TODO: needs_review - complex file (766 lines), kernel launch patterns need manual verification
#define TORCH_ASSERT_ONLY_METHOD_OPERATORS
#include <ATen/core/Tensor.h>
#include <ATen/Dispatch.h>
#include <ATen/AccumulateType.h>
#include <ATen/div_rtn.h>
#include <ATen/sycl/CUDABlas.h>
...
namespace at::native {
namespace {
using SYCL_NUM_THREADS;
using SYCL_GET_BLOCKS;
...
/* SYCL kernel */ void
#if 1 // SYCL: always use unroll hints
/* SYCL: reqd_work_group_size(SYCL_NUM_THREADS) */
#endif
conv_depthwise2d_forward_kernel_generic(
  ...
  for (index_t linearIndex = item.get_global_id(0); linearIndex < totalElements; linearIndex += item.get_global_range(0)) {
...
/* SYCL: reqd_work_group_size(SYCL_NUM_THREADS) */
#endif
/* SYCL kernel */ void conv_depthwise2d_backward_kernel(
...
/* SYCL kernel */ void conv_depthwise2d_grad_weight_kernel(
    const PackedTensorAccessor32<const scalar_t, 4, DefaultPtrTraits> grad_output,
    const PackedTensorAccessor32<const scalar_t, 4, DefaultPtrTraits> input,
  ...
  const int laneId = item.get_local_id(0) % SYCL_SUB_GROUP_SIZE;
  const int batch = item.get_local_id(0) / SYCL_SUB_GROUP_SIZE;
  const int nwarps = item.get_local_range(0) / SYCL_SUB_GROUP_SIZE;
  ...
  for (int batchIdx = batch; batchIdx < batchSize; batchIdx += nwarps){
    ...
    for (index_t idx = laneId; idx < imageElements; idx += SYCL_SUB_GROUP_SIZE) {
  ...
  /* SYCL: use sycl::local_accessor for shared memory */ char smem[];
  acc_t* buf = reinterpret_cast<acc_t*>(smem);
  acc_t tval = sycl_utils::BlockReduceSum(grad, buf);
...
void conv_depthwise2d_forward_out(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(kHalf, kBFloat16, input.scalar_type(),
    ...
    if (kW == 5 && kH == 5) {
      conv_depthwise2d_forward_kernel<5> /* SYCL: submit kernel with nd_range({grid=grid, block=block, smem=0, stream=stream}) */(
      ...
      conv_depthwise2d_forward_kernel<3> /* SYCL: submit kernel with nd_range({grid=grid, block=block, smem=0, stream=stream}) */(
        input_a, output_a, weight_a, bias_a, has_bias, n, outputChannels, depthwiseMultiplier,
        width, height, outputWidth, outputHeight,
        kW, kH, dW, dH, padW, padH, dilationW, dilationH);
      /* SYCL: kernel launch check handled by SYCL runtime */;
    } else if (kW == 1 && kH == 1) {
      conv_depthwise2d_forward_kernel<1> /* SYCL: submit kernel with nd_range({grid=grid, block=block, smem=0, stream=stream}) */(
        input_a, output_a, weight_a, bias_a, has_bias, n, outputChannels, depthwiseMultiplier,
        width, height, outputWidth, outputHeight,
        kW, kH, dW, dH, padW, padH, dilationW, dilationH);
      /* SYCL: kernel launch check handled by SYCL runtime */;
    } else {
      conv_depthwise2d_forward_kernel_generic/* SYCL: submit kernel with nd_range({grid=grid, block=block, smem=0, stream=stream}) */(
        input_a, output_a, weight_a, bias_a, has_bias, n, outputChannels, depthwiseMultiplier,
        width, height, outputWidth, outputHeight,
        kW, kH, dW, dH, padW, padH, dilationW, dilationH);
      /* SYCL: kernel launch check handled by SYCL runtime */;
...
void conv_depthwise2d_backward_out(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(kHalf, kBFloat16, grad_output.scalar_type(),
                                  "conv_depthwise2d_backward_opencl", [&] {
    ...
    if (kW == 5 && kH == 5) {
      if (dW == 1 && dH == 1){
        conv_depthwise2d_backward_kernel<5, 1>/* SYCL: submit kernel with nd_range({grid=grid, block=block, smem=0, stream=stream}) */(
        ...
        conv_depthwise2d_backward_kernel<5, 2>/* SYCL: submit kernel with nd_range({grid=grid, block=block, smem=0, stream=stream}) */(
            grad_output_a, grad_input_a, weight_a, n, inputChannels, depthwiseMultiplier, outputChannels, width,
            height, outputWidth, outputHeight, kW, kH, dW, dH, padW, padH, dilationW, dilationH);
        /* SYCL: kernel launch check handled by SYCL runtime */;
      } else {
        conv_depthwise2d_backward_kernel<5, 0>/* SYCL: submit kernel with nd_range({grid=grid, block=block, smem=0, stream=stream}) */(
            grad_output_a, grad_input_a, weight_a, n, inputChannels, depthwiseMultiplier, outputChannels, width,
            height, outputWidth, outputHeight, kW, kH, dW, dH, padW, padH, dilationW, dilationH);
        /* SYCL: kernel launch check handled by SYCL runtime */;
      }
      ...
      if (dW == 1 && dH == 1){
        conv_depthwise2d_backward_kernel<3, 1>/* SYCL: submit kernel with nd_range({grid=grid, block=block, smem=0, stream=stream}) */(
            grad_output_a, grad_input_a, weight_a, n, inputChannels, depthwiseMultiplier, outputChannels, width,
            height, outputWidth, outputHeight, kW, kH, dW, dH, padW, padH, dilationW, dilationH);
        /* SYCL: kernel launch check handled by SYCL runtime */;
      } else if (dW == 2 && dH == 2) {
        conv_depthwise2d_backward_kernel<3, 2>/* SYCL: submit kernel with nd_range({grid=grid, block=block, smem=0, stream=stream}) */(
            grad_output_a, grad_input_a, weight_a, n, inputChannels, depthwiseMultiplier, outputChannels, width,
            height, outputWidth, outputHeight, kW, kH, dW, dH, padW, padH, dilationW, dilationH);
        /* SYCL: kernel launch check handled by SYCL runtime */;
      } else {
        conv_depthwise2d_backward_kernel<3, 0>/* SYCL: submit kernel with nd_range({grid=grid, block=block, smem=0, stream=stream}) */(
            grad_output_a, grad_input_a, weight_a, n, inputChannels, depthwiseMultiplier, outputChannels, width,
            height, outputWidth, outputHeight, kW, kH, dW, dH, padW, padH, dilationW, dilationH);
        /* SYCL: kernel launch check handled by SYCL runtime */;
      }
      ...
      if (dW == 1 && dH == 1){
        conv_depthwise2d_backward_kernel<1, 1>/* SYCL: submit kernel with nd_range({grid=grid, block=block, smem=0, stream=stream}) */(
            grad_output_a, grad_input_a, weight_a, n, inputChannels, depthwiseMultiplier, outputChannels, width,
            height, outputWidth, outputHeight, kW, kH, dW, dH, padW, padH, dilationW, dilationH);
        /* SYCL: kernel launch check handled by SYCL runtime */;
      } else if (dW == 2 && dH == 2) {
        conv_depthwise2d_backward_kernel<1, 2>/* SYCL: submit kernel with nd_range({grid=grid, block=block, smem=0, stream=stream}) */(
            grad_output_a, grad_input_a, weight_a, n, inputChannels, depthwiseMultiplier, outputChannels, width,
            height, outputWidth, outputHeight, kW, kH, dW, dH, padW, padH, dilationW, dilationH);
        /* SYCL: kernel launch check handled by SYCL runtime */;
      } else {
        conv_depthwise2d_backward_kernel<1, 0>/* SYCL: submit kernel with nd_range({grid=grid, block=block, smem=0, stream=stream}) */(
            grad_output_a, grad_input_a, weight_a, n, inputChannels, depthwiseMultiplier, outputChannels, width,
            height, outputWidth, outputHeight, kW, kH, dW, dH, padW, padH, dilationW, dilationH);
        /* SYCL: kernel launch check handled by SYCL runtime */;
      }
    } else if (dW == 1 && dH == 1) {
      conv_depthwise2d_backward_kernel<0, 1>/* SYCL: submit kernel with nd_range({grid=grid, block=block, smem=0, stream=stream}) */(
          grad_output_a, grad_input_a, weight_a, n, inputChannels, depthwiseMultiplier, outputChannels, width,
          height, outputWidth, outputHeight, kW, kH, dW, dH, padW, padH, dilationW, dilationH);
      /* SYCL: kernel launch check handled by SYCL runtime */;
    } else if (dW == 2 && dH == 2) {
      conv_depthwise2d_backward_kernel<0, 2>/* SYCL: submit kernel with nd_range({grid=grid, block=block, smem=0, stream=stream}) */(
          grad_output_a, grad_input_a, weight_a, n, inputChannels, depthwiseMultiplier, outputChannels, width,
          height, outputWidth, outputHeight, kW, kH, dW, dH, padW, padH, dilationW, dilationH);
      /* SYCL: kernel launch check handled by SYCL runtime */;
    } else {
      conv_depthwise2d_backward_kernel<0, 0>/* SYCL: submit kernel with nd_range({grid=grid, block=block, smem=0, stream=stream}) */(
          grad_output_a, grad_input_a, weight_a, n, inputChannels, depthwiseMultiplier, outputChannels, width,
          height, outputWidth, outputHeight, kW, kH, dW, dH, padW, padH, dilationW, dilationH);
      /* SYCL: kernel launch check handled by SYCL runtime */;
    }
...
int getGradParamsNumThreads(int batchSize) {
  ...
  return std::min(batchSize * at::sycl::sub_group_size(), MAX_BLOCK_SIZE);
}
...
void conv_depthwise2d_grad_weight_out(
  ...
  sycl::range<3> grid(blocks);
  sycl::range<3> block(getGradParamsNumThreads(batchSize));
  const auto stream = c10::sycl::getCurrentSYCLStream();

  AT_DISPATCH_FLOATING_TYPES_AND2(kHalf, kBFloat16, grad_output.scalar_type(),
                                  "conv_depthwise2d_grad_weight_opencl", [&] {
    ...
    conv_depthwise2d_grad_weight_kernel/* SYCL: submit kernel with nd_range({grid=grid, block=block, smem=smem, stream=stream}) */(
...
Tensor& conv_depthwise2d_opencl_out(
    const Tensor &input_,
...
Tensor conv_depthwise2d_opencl(
    const Tensor &input,
    const Tensor &weight,
    IntArrayRef kernel_size,
  ...
  return conv_depthwise2d_opencl_out(input, weight, kernel_size, bias,
                                   stride, padding, dilation, out);
}

std::tuple<Tensor&, Tensor&> conv_depthwise2d_backward_opencl_out(
...
std::tuple<Tensor, Tensor> conv_depthwise2d_backward_opencl(
    const Tensor& grad_output,
    const Tensor& self,
    const Tensor& weight,
    IntArrayRef kernel_size,
  ...
  return conv_depthwise2d_backward_opencl_out(
      grad_output,
      self,
      weight,
      kernel_size,
...
REGISTER_OPENCL_DISPATCH(conv_depthwise2d_backward_stub, &conv_depthwise2d_backward_opencl)
