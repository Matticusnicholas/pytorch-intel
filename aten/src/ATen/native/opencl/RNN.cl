...
// Source: aten/src/ATen/native/cuda/RNN.cu
...
using at::sycl::detail::TensorInfo;
using at::sycl::detail::getTensorInfo;
using at::sycl::detail::IndexToOffset;
...
void getLaunchConfig(sycl::range<3>* block, sycl::range<3>* grid, int64_t numel) {
...
/* SYCL kernel */ void lstm_cell_forward(
    ...
    for (index_type linearIndex = item.get_group(0) * item.get_local_range(0) + item.get_local_id(0);
...
// SYCL: launch bounds
C10_LAUNCH_BOUNDS_2(512, 4)
/* SYCL kernel */ void lstm_cell_backward(
...
// SYCL: launch bounds
C10_LAUNCH_BOUNDS_2(512, 4)
/* SYCL kernel */ void gru_cell_forward(
            TensorInfo<scalar_t, index_type> Input,
            TensorInfo<scalar_t, index_type> Hidden,
            TensorInfo<scalar_t, index_type> Bias1,
            TensorInfo<scalar_t, index_type> Bias2,
...
// SYCL: launch bounds
C10_LAUNCH_BOUNDS_2(512, 4)
/* SYCL kernel */ void gru_cell_backward(
...
void lstm_forward_impl(const Tensor& input_gates, const Tensor& hidden_gates,
  ...
  if (allContiguous({input_gates, hidden_gates, input_bias, hidden_bias, cx, hy, cy, workspace})) {
    ...
    kernel::lstm_cell_forward<scalar_t, accscalar_t, index_type, 1>
      /* SYCL: kernel launch with nd_range(grid, block, 0, stream) */
...
std::tuple<Tensor, Tensor, Tensor> _thnn_fused_lstm_cell_cuda(
  ...
  checkSizes("_thnn_fused_lstm_cell_opencl",
...
std::tuple<Tensor, Tensor> _thnn_fused_gru_cell_cuda(
  ...
  checkSizes("_thnn_fused_gru_cell_opencl",
             {input_gates, "input_gates", 1}, {hidden_gates, "hidden_gates", 2},
             {input_bias, "input_bias", 3}, {hidden_bias, "hidden_bias", 4},
...
std::tuple<Tensor, Tensor, Tensor, Tensor, Tensor> _thnn_fused_gru_cell_backward_opencl(
