...
// Source: aten/src/ATen/native/cuda/PowKernel.cu
...
void pow_scalar_tensor_impl(TensorIteratorBase& iter, scalar_t base) {
  sycl_kernel(iter, [=]SYCL_LAMBDA(scalar_t exp) -> scalar_t {
...
void pow_scalar_tensor_impl(TensorIteratorBase& iter, c10::complex<value_t> base) {
  ...
  sycl_kernel(iter, [=]SYCL_LAMBDA(c10::complex<value_t> exp) -> c10::complex<value_t> {
...
#else
  sycl_kernel(iter, [=] SYCL_LAMBDA(scalar_t base) -> scalar_t {
...
// SYCL: JIT path disabled, using direct dispatch
...
#else
      sycl_kernel(iter, [=] SYCL_LAMBDA(scalar_t base, scalar_t exp) -> scalar_t {
            using opmath_t = at::opmath_type<scalar_t>;
    ...
    AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND2(
        kHalf, kBFloat16, iter.common_dtype(), "pow_opencl", [&] {
      if (iter.is_cpu_scalar(1)) {
        const auto base = iter.scalar_value<scalar_t>(1);
        iter.remove_operand(1);
        pow_scalar_tensor_impl(iter, base);
...
void pow_tensor_scalar_kernel_impl(TensorIteratorBase& iter,
  ...
  if (d_exp == 2) {
    sycl_kernel(iter, [=]SYCL_LAMBDA(Base_type base) -> Base_type {
      return base * base;
    });
  } else if (d_exp == 3) {
    sycl_kernel(iter, [=]SYCL_LAMBDA(Base_type base) -> Base_type {
...
void pow_tensor_scalar_kernel(TensorIteratorBase& iter, const Scalar& exp_scalar) {
  ...
  if (!exp_scalar.isComplex()) {
    if (exp_scalar.equal(.5)) {
      return sqrt_kernel_opencl(iter);
    } else if (exp_scalar.equal(-0.5)) {
      return rsqrt_kernel_opencl(iter);
    } else if (exp_scalar.equal(-1.0)) {
      return reciprocal_kernel_opencl(iter);
    }
  }
  if (isComplexType(iter.common_dtype()) || exp_scalar.isComplex()) {
    ...
    AT_DISPATCH_COMPLEX_TYPES(iter.common_dtype(), "pow_opencl", [&]() {
    ...
    AT_DISPATCH_ALL_TYPES_AND2(kHalf, kBFloat16, iter.common_dtype(), "pow_opencl", [&]() {
      const auto exp = exp_scalar.to<scalar_t>();
