...
#include <ATen/native/sycl/IndexKernelUtils.h>
...
// SYCL kernel
void _scatter_gather_elementwise_kernel(int N, func_t f) {
...
static void _launch_scatter_gather_kernel(int64_t N, const func_t& f) {
  ...
  const sycl::range<3> block(nt);
  const sycl::range<3> grid((N + block.x * vt - 1) / (block.x * vt));
  const auto stream = at::sycl::getCurrentSYCLStream();
  _scatter_gather_elementwise_kernel<nt, vt, func_t>/* SYCL: launch with nd_range(grid, block, 0, stream) */(N, f);
  // SYCL: kernel launch check handled by SYCL runtime;
}

template <bool is_scatter_like, typename scalar_t, typename index_t>
struct _opencl_scatter_gather_internal_kernel {
  ...
  void operator() (
    ...
    if (!iter.can_use_32bit_indexing()) {
      for (auto& sub_iter : iter.with_32bit_indexing()) {
        _opencl_scatter_gather_internal_kernel<is_scatter_like, scalar_t, index_t>()(
    ...
    auto loop = [=]C10_DEVICE(int i) {
      ...
      SYCL_KERNEL_ASSERT_VERBOSE(idx_dim >= 0 && idx_dim < index_size
...
struct opencl_scatter_gather_base_kernel {
  void operator()(
    ...
    AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND3(
      ...
      iter.dtype(),
      "opencl_scatter_gather_base_kernel_func", [&] {
        using dtype = typename std::conditional<cast_to_opaque,
          OpaqueType<sizeof(scalar_t)>, scalar_t>::type;

        AT_DISPATCH_INDEX_TYPES(index.scalar_type(), "opencl_scatter_gather_base_kernel_func", [&] () {
          _opencl_scatter_gather_internal_kernel<is_scatter_like, dtype, index_t>()(
  ...
  void operator()(
    ...
    if (self.is_quantized()) {
      ...
      AT_DISPATCH_QINT_TYPES(iter.dtype(), "gather_quant_opencl", [&] {
        using dtype = typename std::conditional<cast_to_opaque,
            OpaqueType<sizeof(scalar_t)>, scalar_t>::type;
        AT_DISPATCH_INDEX_TYPES(index.scalar_type(), "opencl_scatter_gather_base_kernel_func", [&] () {
          _opencl_scatter_gather_internal_kernel<is_scatter_like, dtype, index_t>()(
...
struct _opencl_scatter_fill_internal_kernel {
  template <typename func_t>
  void operator()(
    TensorIterator& iter,
    scalar_t src_val,
    ...
    if (!iter.can_use_32bit_indexing()) {
      for (auto& sub_iter : iter.with_32bit_indexing()) {
        _opencl_scatter_fill_internal_kernel<scalar_t, index_t>()(
...
struct opencl_scatter_fill_base_kernel {
  template <typename func_t>
  void operator()(
    const Tensor& self, int64_t dim,
    ...
    AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND3(
      at::ScalarType::Half, at::ScalarType::Bool, at::ScalarType::BFloat16,
      iter.dtype(),
      "opencl_scatter_fill_base_kernel_func", [&] {
        using dtype = typename std::conditional<cast_to_opaque,
          OpaqueType<sizeof(scalar_t)>, scalar_t>::type;
        ...
        AT_DISPATCH_INDEX_TYPES(index.scalar_type(), "opencl_scatter_fill_base_kernel_func", [&] () {
          _opencl_scatter_fill_internal_kernel<dtype, index_t>()(
  ...
  void operator()(
    ...
    AT_DISPATCH_ALL_TYPES_AND2(
      at::ScalarType::Half, at::ScalarType::BFloat16,
      iter.dtype(),
      "opencl_scatter_fill_base_kernel_reduce_multiply", [&] {
        using dtype = typename std::conditional<cast_to_opaque,
          OpaqueType<sizeof(scalar_t)>, scalar_t>::type;
        ...
        AT_DISPATCH_INDEX_TYPES(index.scalar_type(), "opencl_scatter_fill_base_kernel_reduce_multiply", [&] () {
          _opencl_scatter_fill_internal_kernel<dtype, index_t>()(
            iter, src_val, index_size, index_stride, self.numel(), f
          );
        });
...
void gather_opencl_kernel(const Tensor& result, const Tensor& self, int64_t dim, const Tensor& index) {
  opencl_scatter_gather_base_kernel</*is_scatter_like=*/false>()(
    result, dim, index, self,
    "gather_out_opencl", tensor_assign);
}

void scatter_opencl_kernel(const Tensor& self, int64_t dim, const Tensor& index, const Tensor& src) {
  // When indices are not unique, the behavior is non-deterministic
  globalContext().alertNotDeterministic("scatter_opencl_");
  opencl_scatter_gather_base_kernel<>()(
    self, dim, index, src,
    "scatter_opencl_", tensor_assign);
}

void scatter_fill_opencl_kernel(const Tensor& self, int64_t dim, const Tensor& index, const Scalar& src) {
  opencl_scatter_fill_base_kernel<>()(
    self, dim, index, src,
    "scatter_fill_opencl_", tensor_assign);
}

void scatter_add_opencl_kernel(const Tensor& self, int64_t dim, const Tensor& index, const Tensor& src) {
  opencl_scatter_gather_base_kernel</*is_scatter_like=*/true, /*cast_to_opaque=*/false>()(
    self, dim, index, src,
    "scatter_add_opencl_", reduce_add);
}

void scatter_reduce_opencl_kernel(const Tensor& self, const int64_t dim, const Tensor& index,
                               const Tensor& src, const ReductionType& reduce) {
  // See Note [Writing Nondeterministic Operations]
  // Nondeterministic because of sycl::atomic_ref_add /* sycl::atomic_ref *//AtomicMul usage
  globalContext().alertNotDeterministic("scatter_reduce_opencl_kernel");
  switch (reduce) {
  case ReductionType::SUM :
    opencl_scatter_gather_base_kernel<true, false>()(self, dim, index, src,
                                       "scatter_reduce_opencl_add_", reduce_add);
    break;
  case ReductionType::PROD :
    opencl_scatter_gather_base_kernel<true, false>()(self, dim, index, src,
                                       "scatter_reduce_opencl_multiply_", reduce_multiply);
    break;
  default :
    break;
  }
...
void scatter_reduce_two_opencl_kernel(const Tensor& self, const int64_t dim, const Tensor& index,
                                    const Tensor& src, const ReductionType& reduce) {
  switch (reduce) {
  case ReductionType::SUM :
    opencl_scatter_gather_base_kernel<true, false>()(self, dim, index, src,
            "scatter_reduce_opencl_sum_", reduce_add);
    break;
  case ReductionType::PROD :
    globalContext().alertNotDeterministic("scatter_reduce_opencl_prod_");
    opencl_scatter_gather_base_kernel<true, false>()(self, dim, index, src,
            "scatter_reduce_opencl_prod_", reduce_multiply);
    break;
  case ReductionType::MAX :
    opencl_scatter_gather_base_kernel<true, false>()(self, dim, index, src,
            "scatter_reduce_opencl_amax_", reduce_maximum);
    break;
  case ReductionType::MIN :
    opencl_scatter_gather_base_kernel<true, false>()(self, dim, index, src,
            "scatter_reduce_opencl_amin_", reduce_minimum);
    break;
  case ReductionType::MEAN :
    opencl_scatter_gather_base_kernel<true, false>()(self, dim, index, src,
            "scatter_reduce_opencl_mean_", reduce_mean);
    break;
  }
}

void scatter_scalar_reduce_opencl_kernel(const Tensor& self, const int64_t dim, const Tensor& index,
                               const Scalar& value, const ReductionType& reduce) {
  switch (reduce) {
  case ReductionType::SUM :
    opencl_scatter_fill_base_kernel<false>()(self, dim, index, value,
                                      "scatter_fill_opencl_add_", reduce_add);
    break;
  case ReductionType::PROD :
    opencl_scatter_fill_base_kernel<false>()(self, dim, index, value,
                                      "scatter_fill_opencl_multiply_", reduce_multiply);
    break;
  default :
    break;
  }
...
REGISTER_DISPATCH(gather_stub, &gather_opencl_kernel)
REGISTER_DISPATCH(scatter_stub, &scatter_opencl_kernel)
REGISTER_DISPATCH(scatter_fill_stub, &scatter_fill_opencl_kernel)
REGISTER_DISPATCH(scatter_add_stub, &scatter_add_opencl_kernel)
REGISTER_DISPATCH(scatter_reduce_stub, &scatter_reduce_opencl_kernel)
REGISTER_DISPATCH(scatter_scalar_reduce_stub, &scatter_scalar_reduce_opencl_kernel)
REGISTER_DISPATCH(scatter_reduce_two_stub, &scatter_reduce_two_opencl_kernel)
