...
// Source: aten/src/ATen/native/cuda/GridSampler.cpp

#define TORCH_ASSERT_ONLY_METHOD_OPERATORS
#include <ATen/native/opencl/GridSampler.h>
...
Tensor grid_sampler_2d_opencl(const Tensor& input, const Tensor& grid,
...
Tensor grid_sampler_3d_opencl(const Tensor& input, const Tensor& grid,
                            int64_t interpolation_mode, int64_t padding_mode,
                            bool align_corners) {
  auto in_size = input.sizes();
  auto grid_size = grid.sizes();
...
grid_sampler_2d_backward_opencl(const Tensor& grad_output, const Tensor& input,
...
std::tuple<Tensor, Tensor>
grid_sampler_3d_backward_opencl(const Tensor& grad_output, const Tensor& input,
                              const Tensor& grid, int64_t interpolation_mode, int64_t padding_mode,
                              bool align_corners, std::array<bool,2> output_mask) {
  auto input_requires_grad = output_mask[0];
  Tensor grad_input = ([&]() {
