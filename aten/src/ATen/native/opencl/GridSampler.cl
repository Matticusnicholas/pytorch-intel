...
// Source: aten/src/ATen/native/cuda/GridSampler.cu
// SYCL TODO: needs_review - complex file (962 lines), kernel launch patterns need manual verification
#define TORCH_ASSERT_NO_OPERATORS
#include <ATen/OpMathType.h>
#include <ATen/native/sycl/GridSampler.h>
...
namespace {
  ...
  /* SYCL kernel */ void grid_sampler_2d_kernel(
  ...
  template <typename scalar_t, typename index_t>
  /* SYCL: reqd_work_group_size(512) */
  /* SYCL kernel */ void grid_sampler_3d_kernel(
      const index_t nthreads,
      TensorInfo<const scalar_t, index_t> input,
      TensorInfo<const scalar_t, index_t> grid,
      TensorInfo<scalar_t, index_t> output,
...
// including batch * channel offset (NC_offset).
  ...
  template <typename scalar_t, typename index_t>
  /* SYCL: reqd_work_group_size(256) */
  /* SYCL kernel */ void grid_sampler_2d_backward_kernel(
      const index_t nthreads,
  ...
  template <typename scalar_t, typename index_t>
  /* SYCL: reqd_work_group_size(256) */
  /* SYCL kernel */ void grid_sampler_3d_backward_kernel(
      const index_t nthreads,
      TensorInfo<const scalar_t, index_t> grad_output,
      TensorInfo<const scalar_t, index_t> input,
      TensorInfo<const scalar_t, index_t> grid,
...
void launch_grid_sampler_2d_forward_kernel(
  ...
  if (count > 0) {
    AT_DISPATCH_FLOATING_TYPES_AND2(
      ...
      input.scalar_type(), "grid_sampler_2d_opencl", [&] {
      if (canUse32BitIndexMath(input) && canUse32BitIndexMath(grid) &&
          canUse32BitIndexMath(output)) {
        grid_sampler_2d_kernel<scalar_t>
          /* SYCL: submit kernel with nd_range({grid=GET_BLOCKS(count, block=256), smem=256, stream=0, at::sycl::getCurrentSYCLStream()}) */(
...
void launch_grid_sampler_3d_forward_kernel(
  ...
  if (count > 0) {
    AT_DISPATCH_FLOATING_TYPES_AND2(
      ScalarType::Half, ScalarType::BFloat16,
      input.scalar_type(), "grid_sampler_3d_opencl", [&] {
      if (canUse32BitIndexMath(input) && canUse32BitIndexMath(grid) &&
          canUse32BitIndexMath(output)) {
        grid_sampler_3d_kernel<scalar_t>
          /* SYCL: submit kernel with nd_range({grid=GET_BLOCKS(count, block=512), smem=512, stream=0, at::sycl::getCurrentSYCLStream()}) */(
            static_cast<int>(count),
            getTensorInfo<const scalar_t, int>(input),
            getTensorInfo<const scalar_t, int>(grid),
            getTensorInfo<scalar_t, int>(output),
...
void launch_grid_sampler_2d_backward_kernel(
  ...
  globalContext().alertNotDeterministic("grid_sampler_2d_backward_opencl");
  auto N = input.size(0);
  auto H = grid.size(1);
  auto W = grid.size(2);
  ...
  if (count > 0) {
    AT_DISPATCH_FLOATING_TYPES_AND2(
      ScalarType::Half, ScalarType::BFloat16,
      input.scalar_type(), "grid_sampler_2d_backward_opencl", [&] {
      if (canUse32BitIndexMath(input) && canUse32BitIndexMath(grid) &&
...
void launch_grid_sampler_3d_backward_kernel(
  ...
  // See Note [Writing Nondeterministic Operations]
  // Nondeterministic because of atomicAdd usage
  globalContext().alertNotDeterministic("grid_sampler_3d_backward_opencl");
  auto N = input.size(0);
  auto D = grid.size(1);
  auto H = grid.size(2);
  auto W = grid.size(3);
  ...
  if (count > 0) {
    AT_DISPATCH_FLOATING_TYPES_AND2(
      ScalarType::Half, ScalarType::BFloat16,
      input.scalar_type(), "grid_sampler_3d_backward_opencl", [&] {
      if (canUse32BitIndexMath(input) && canUse32BitIndexMath(grid) &&
          canUse32BitIndexMath(grad_output)) {
