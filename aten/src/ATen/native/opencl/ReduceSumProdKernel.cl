...
// Source: aten/src/ATen/native/cuda/ReduceSumProdKernel.cu
...
struct sum_functor {
  void operator()(TensorIterator& iter) {
    const auto sum_combine = [] SYCL_LAMBDA(acc_t a, acc_t b) -> acc_t {
      return a + b;
    };
    constexpr bool is_16_bits = sizeof(scalar_t) == 2;
    if constexpr (is_16_bits) {
      sycl_reduce_kernel<scalar_t, out_t, /*vt0=*/4, /*input_vec_size=*/8>(
        iter, func_wrapper<out_t>(sum_combine)
      );
    } else {
      sycl_reduce_kernel<scalar_t, out_t>(
        iter, func_wrapper<out_t>(sum_combine)
      );
    }
  }
...
// SYCL: JIT path disabled, using direct dispatch
#if 0 /* AT_USE_JITERATOR - disabled for SYCL */ && !defined(_MSC_VER)
  void operator()(TensorIterator& iter) {
  ...
  void operator()(TensorIterator& iter) {
    ...
    sycl_reduce_kernel<scalar_t, scalar_t>(
        iter, func_wrapper<scalar_t>([] SYCL_LAMBDA(acc_t a, acc_t b) -> acc_t {
          return a + b;
        }), acc_t{0.});
  }
#endif
...
struct prod_functor {
  ...
  void operator()(TensorIterator& iter) {
    sycl_reduce_kernel<scalar_t, out_t>(
        iter, func_wrapper<out_t>([] SYCL_LAMBDA(acc_t a, acc_t b) -> acc_t {
          return a * b;
        }), 1.);
  }
  #endif
...
struct prod_functor<bool> {
  void operator()(TensorIterator& iter) {
    sycl_reduce_kernel<bool, bool>(
        iter, func_wrapper<bool>([] SYCL_LAMBDA(bool a, bool b) -> bool {
          return a && b;
        }), 1);
  }
};
  ...
  void operator()(TensorIterator& iter) {
    ...
    sycl_reduce_kernel<scalar_t, scalar_t>(
        ...
        func_wrapper<scalar_t>(
            [] SYCL_LAMBDA(acc_t a, acc_t b) -> acc_t { return a * b; }),
        acc_t{1.});
  }
#endif
};
...
struct xor_sum_functor {
  void operator()(TensorIterator& iter) {
    sycl_reduce_kernel<scalar_t, uint64_t>(
        iter,
        func_wrapper<uint64_t>(
            [] SYCL_LAMBDA(uint64_t a, uint64_t b) -> uint64_t {
              return a ^ b;
            }));
  }
};
...
struct xor_sum_functor<scalar_t, std::enable_if_t<!std::is_integral_v<scalar_t>>> {
  void operator()(TensorIterator& iter) {
    sycl_reduce_kernel<scalar_t, double>(
        iter,
        // implicitly upcast scalar_t to double
        func_wrapper<double>([] SYCL_LAMBDA(double a, double b) -> double {
          union {
            double d;
            uint64_t u;
...
struct xor_sum_functor<scalar_t, std::enable_if_t<std::is_same_v<scalar_t, bool>>>  {
  void operator()(TensorIterator& iter) {
    sycl_reduce_kernel<bool, uint64_t>(
        iter, func_wrapper<uint64_t>([] SYCL_LAMBDA(bool a, bool b) -> uint64_t {
...
static void sum_kernel_opencl(TensorIterator& iter){
  auto general_dispatcher = [](TensorIterator& iter) {
    AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND2(
        kBool, kComplexHalf, iter.dtype(), "sum_opencl", [&]() {
...
static void nansum_kernel_opencl(TensorIterator& iter) {
  auto general_dispatcher = [](TensorIterator& iter) {
    auto dtype = iter.dtype();
    if (at::isComplexType(dtype)) {
        AT_DISPATCH_COMPLEX_TYPES_AND(kComplexHalf, dtype, "nansum_opencl", [&]() {
          nansum_functor_complex<scalar_t>{}(iter);
        });
    } else {
        AT_DISPATCH_FLOATING_TYPES(iter.dtype(), "nansum_opencl", [&]() {
...
static void prod_kernel_opencl(TensorIterator& iter) {
  auto general_dispatcher = [](TensorIterator& iter) {
    AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND2(kComplexHalf, kBool, iter.dtype(), "prod_opencl", [&]() {
...
static void xor_sum_kernel_opencl(TensorIterator& iter) {
  // Use iter.dtype(1) to dispatch based on the type of the input tensor
  AT_DISPATCH_ALL_TYPES_AND3(
      kHalf, kBFloat16, kBool, iter.dtype(1), "xor_sum_opencl", [&]() {
        xor_sum_functor<scalar_t>{}(iter);
      });
}

REGISTER_DISPATCH(sum_stub, &sum_kernel_opencl)
REGISTER_DISPATCH(nansum_stub, &nansum_kernel_opencl)
REGISTER_DISPATCH(prod_stub, &prod_kernel_opencl)
REGISTER_DISPATCH(xor_sum_stub, &xor_sum_kernel_opencl)
