...
// Original: ATen/native/cuda/fused_adam_utils.cuh
#pragma once
...
struct FusedAdamMathFunctor {
  ...
  inline void operator()(
      ...
      const float* found_inf_ptr,
      sycl::nd_item<1> item) {
    // SYCL: item.get_group(0) replaces blockIdx.x
    ...
    const auto [bias_correction1, bias_correction2_sqrt] =
      ...
      const opmath_t bc1 = 1 - at::native::pow_(beta1_opmath, step_count);
      const opmath_t bc2 = 1 - at::native::pow_(beta2_opmath, step_count);
      return {bc1, std::sqrt(bc2)};
    }();
...
#pragma unroll
        ...
        adam_math<scalar_type, opmath_t, depth, adam_mode, amsgrad>(
            r_args, lr_opmath, beta1_opmath, beta2_opmath,
            weight_decay_opmath, eps_opmath, maximize,
            grad_scale_ptr, found_inf_ptr, bias_correction1, bias_correction2_sqrt);
#pragma unroll
        for (int i = 0; i < depth; i++) {
