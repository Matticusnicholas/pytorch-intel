...
// Original: ATen/native/cuda/fused_adagrad_utils.cuh
#pragma once
...
struct FusedAdagradMathFunctor {
  ...
  inline void operator()(
    ...
    if ((n % kILP == 0) && (chunk_size % kILP == 0) && all_aligned) {
      // SYCL: item.get_local_id(0) replaces threadIdx.x
      for (int64_t i_start = item.get_local_id(0);
        ...
        adagrad_math<scalar_t, opmath_t>(
            r_args, corrected_lr, weight_decay, eps,
            maximize, grad_scale_ptr, found_inf_ptr);
